...
/**
 * Revalúa las TRCONT Titos de agosto que Eduardo pidió en la planilla
 * (las COM ya están corregidas; no se tocan).
 *
 *   BSA TM#740 y #819 → 10.216233  (promedio post COM 166531 @ 1530)
 *   KSA TM#884        → 10.181767  (promedio post COM 166532/166614)
 *   RSA TM#842        → 9.983433   (promedio sin COM 166530 anulada)
 *
 * El resto de TRA agosto ya coincide con la planilla y no se toca.
 *
 * Uso:
 *   php scripts/fix_titos_agosto_promedio_planilla.php
 *   php scripts/fix_titos_agosto_promedio_planilla.php apply
 */
...
/** @var array<int, float> tm.id => precio unitario pedido */
$preciosPorTm = [
    740 => 10.216233, // BSA 21-08
    819 => 10.216233, // BSA 25-08
    884 => 10.181767, // KSA 28-08
    842 => 9.983433,  // RSA 26-08
];

$fmt = static fn (float $v): string => number_format($v, 6, '.', '');
$fmt2 = static fn (float $v): string => number_format($v, 2, ',', '.');
$labelEmp = static fn (int $emp): string => match ($emp) {
    1 => 'BSA',
    2 => 'KSA',
    3 => 'RSA',
    default => (string) $emp,
...
echo '=== Revaluación Titos agosto a precios planilla ('.($apply ? 'APLICAR' : 'DRY-RUN').") ===\n\n";
