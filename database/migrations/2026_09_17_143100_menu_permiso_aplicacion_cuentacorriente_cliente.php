...
{
    private const MENU_URL = 'ventas/aplicacion-cuentacorriente';

    private const MENU_NOMBRE = 'Aplicar cuenta corriente';

    private const PADRE_URL = '#cuenta-corriente-ventas';
    ...
    private const PERMISOS = [
        ['nombre' => 'Aplicar cuenta corriente de cliente', 'slug' => 'aplicar-cuentacorriente-cliente'],
        ['nombre' => 'Desaplicar cuenta corriente de cliente', 'slug' => 'desaplicar-cuentacorriente-cliente'],
    ];
    ...
    public function up(): void
    {
        ...
        if ($padreId <= 0) {
            $padreId = (int) (DB::table('menu')->where('nombre', 'Módulo de Ventas')->where('menu_id', 0)->value('id') ?? 0);
        }
        if ($padreId <= 0) {
            return;
        }
