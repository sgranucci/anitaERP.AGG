...
return new class extends Migration
{
    private const PERMISO = [
        ...
        'nombre' => 'Listar todos los ingresos y egresos de caja',
        'slug' => 'listar-todos-ingresos-egresos-caja',
    ];
    ...
    public function up(): void
    {
        ...
        foreach ($this->resolverRolesCajaContaduria() as $rolId) {
    ...
    /** @return list<int> */
    private function resolverRolesCajaContaduria(): array
    {
        return DB::table('rol')
            ->where(function ($q) {
                $q->whereRaw('LOWER(nombre) LIKE ?', ['%contadur%'])
                    ->orWhereRaw('LOWER(nombre) LIKE ?', ['%caja%'])
                    ->orWhereRaw('LOWER(nombre) LIKE ?', ['%tesorer%'])
