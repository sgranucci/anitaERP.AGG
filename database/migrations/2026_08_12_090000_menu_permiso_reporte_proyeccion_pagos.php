...
{
    private const MENU = [
        'url' => 'compras/proyeccion-pagos',
        'nombre' => 'Proyección de pagos',
        'icono' => 'fa-calendar-check',
        'permisos' => [
            ['slug' => 'listar-reporte-proyeccion-pagos', 'nombre' => 'Listar proyección de pagos a proveedores'],
        ],
    ];

    public function up(): void
    {
        $parentMenuId = (int) (DB::table('menu')->where('url', 'compras/comprobante-proveedor')->value('menu_id') ?? 0);
        if ($parentMenuId === 0) {
            $parentMenuId = (int) (DB::table('menu')->where('url', 'compras/requisicion')->value('menu_id') ?? 0);
        }
        if ($parentMenuId === 0) {
        ...
        $refMenuId = (int) (DB::table('menu')->where('url', 'compras/requisicion-reporte')->value('id') ?? 0);
        $refPermisoId = (int) (DB::table('permiso')->where('slug', 'listar-reporte-requisicion-compras')->value('id') ?? 0);
        if ($refPermisoId === 0) {
