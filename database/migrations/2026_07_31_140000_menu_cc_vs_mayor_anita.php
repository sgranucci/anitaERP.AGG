...
{
    private const MENU_URL = 'contable/cc-vs-mayor-anita';

    private const MENU_NOMBRE = 'Control CC vs mayor Anita';

    private const PERMISO_SLUG = 'listar-cc-vs-mayor-anita';

    private const PERMISO_NOMBRE = 'Listar control CC vs mayor Anita';
    ...
    public function up(): void
    {
        ...
        $menuId = $this->upsertMenu(self::MENU_URL, self::MENU_NOMBRE, $padreId, $orden, 'fa-balance-scale');
    ...
    {
        $id = (int) (DB::table('menu')->where('nombre', 'Módulo Contable')->whereNull('menu_id')->value('id') ?? 0);
        if ($id > 0) {
            return $id;
        }

        return (int) (DB::table('menu')->where('url', 'contable')->value('id') ?? 0);
    }
