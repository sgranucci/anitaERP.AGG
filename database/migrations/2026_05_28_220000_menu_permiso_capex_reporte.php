...
{
    private const MENU_URL = 'presupuesto/capex-reporte';

    private const MENU_PADRE_ID = 214;

    private const MENU_REF_ROLES_ID = 216;

    public function up(): void
    {
        $orden = (int) (DB::table('menu')->where('menu_id', self::MENU_PADRE_ID)->max('orden') ?? 0) + 1;
        $menuId = $this->upsertMenu(self::MENU_URL, 'Reporte CAPEX', self::MENU_PADRE_ID, $orden, 'fa-file-invoice-dollar');
