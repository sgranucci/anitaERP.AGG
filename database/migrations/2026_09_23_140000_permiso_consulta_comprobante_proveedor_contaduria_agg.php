...
/**
 * AGG: Contaduría (egalarza Enc / Op / Sup) consulta comprobantes de proveedor
 * de toda la empresa, mismo criterio que Control de Gestión: listar + editar (sin alta/escritura).
 */
return new class extends Migration
{
    ...
    /** Consulta / apertura de formulario (igual que OC). */
    private const PERMISOS_CONSULTA = [
    ...
    public function up(): void
    {
        ...
        foreach (self::PERMISOS_CONSULTA as $slug) {
    ...
    public function down(): void
    {
        ...
        $permisoIds = DB::table('permiso')
            ->whereIn('slug', self::PERMISOS_CONSULTA)
            ->pluck('id')
            ->map(fn ($id) => (int) $id)
            ->all();
    ...
    private function resolverRolIds(): array
    {
        ...
        foreach (self::ROLES as $nombre) {
            ...
            if ($id <= 0) {
                $like = match (true) {
                    str_starts_with($nombre, 'Enc-') => 'Enc-contadur%',
                    str_starts_with($nombre, 'Op-') => 'Op-contadur%',
                    default => 'Sup-contadur%',
                };
