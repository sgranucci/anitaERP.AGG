...
{
    private const MENU_URL = 'caja/interbanking/movimientos-persistidos';

    private const PERMISO_LISTAR = 'listar-interbanking-movimientos-persistidos';

    private const PERMISO_SINCRONIZAR = 'sincronizar-interbanking-movimientos';

    /** Permiso cuyos roles y menú se replican (saldos históricos Interbanking). */
    private const PERMISO_REFERENCIA = 'listar-saldos-interbanking-historico';
    ...
    public function up(): void
    {
        ...
        if ($menuId === 0) {
            $menuId = (int) DB::table('menu')->insertGetId([
                ...
                'nombre' => 'Interbanking movimientos persistidos',
                'url' => self::MENU_URL,
                'orden' => 21,
        ...
        $permisoListarId = $this->asegurarPermiso(
            self::PERMISO_LISTAR,
            'Listar movimientos Interbanking persistidos',
            $menuId
        );
        $permisoSyncId = $this->asegurarPermiso(
            self::PERMISO_SINCRONIZAR,
            'Sincronizar movimientos Interbanking desde API',
            $menuId
        );
