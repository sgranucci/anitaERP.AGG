...
/**
 * Ferli: menú/permisos reportes de comisiones de vendedores (detalle + resumen).
 */
return new class extends Migration
{
    /** @var list<array{url: string, nombre: string, slug: string, permiso_nombre: string, orden: int}> */
    private const ITEMS = [
        [
            'url' => 'ventas/comision-vendedor-detalle',
            'nombre' => 'Comisiones vendedor detalle',
            'slug' => 'listar-comision-vendedor-detalle',
            'permiso_nombre' => 'Listar reporte comisiones vendedor detalle',
            'orden' => 20,
        ],
        [
            'url' => 'ventas/comision-vendedor',
            'nombre' => 'Comisiones vendedor resumen',
            'slug' => 'listar-comision-vendedor',
            'permiso_nombre' => 'Listar reporte comisiones vendedor resumen',
            'orden' => 21,
        ],
    ];
    ...
    public function up(): void
    {
        ...
        foreach (self::ITEMS as $idx => $item) {
            $orden = max($ordenBase + $idx + 1, (int) $item['orden']);
            $menuId = $this->upsertMenu($item['url'], $item['nombre'], $padreId, $orden, 'fa-percentage');
            $permisoId = $this->upsertPermiso($menuId, $item['slug'], $item['permiso_nombre']);
    ...
    public function down(): void
    {
        ...
        foreach (array_reverse(self::ITEMS) as $item) {
            ...
            $permisoId = (int) (DB::table('permiso')->where('slug', $item['slug'])->value('id') ?? 0);
            if ($permisoId > 0) {
    ...
    private function upsertPermiso(int $menuId, string $slug, string $nombre): int
    {
    ...
    private function resolverMenuReportesVentasId(): int
    {
        ...
        if ($ventasId > 0) {
            $reportesId = (int) (DB::table('menu')
                ...
                ->whereIn('nombre', ['Reportes', 'Reportes de ventas'])
                ->where('url', '#')
                ->value('id') ?? 0);
            if ($reportesId > 0) {
                return $reportesId;
        ...
        foreach (['ventas/reppedido', 'ventas/ventas-por-concepto', 'ventas/iva-ventas'] as $url) {
