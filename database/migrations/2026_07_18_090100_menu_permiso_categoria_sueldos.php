...
return new class extends Migration
{
    ...
    private const MENU_NOMBRE = 'Categorías';

    /** @var list<array{nombre: string, slug: string}> */
    private const PERMISOS = [
        ['nombre' => 'Listar categoría sueldos', 'slug' => 'listar-categoria-sueldos'],
        ['nombre' => 'Crear categoría sueldos', 'slug' => 'crear-categoria-sueldos'],
        ['nombre' => 'Editar categoría sueldos', 'slug' => 'editar-categoria-sueldos'],
        ['nombre' => 'Actualizar categoría sueldos', 'slug' => 'actualizar-categoria-sueldos'],
        ['nombre' => 'Borrar categoría sueldos', 'slug' => 'borrar-categoria-sueldos'],
    ];
    ...
    public function up(): void
    {
        ...
        $existente = (int) (DB::table('menu')->where('url', self::MENU_URL)->value('id') ?? 0);
        if ($existente > 0) {
            $orden = (int) (DB::table('menu')->where('id', $existente)->value('orden') ?? 0);
        } else {
            $orden = (int) (DB::table('menu')->where('menu_id', $submenuId)->max('orden') ?? 0) + 1;
        }
        $menuId = $this->upsertMenuHijo(self::MENU_URL, self::MENU_NOMBRE, $submenuId, $orden);
