...
{
    private const MENU = [
        'url' => 'compras/historial-precios-articulo',
        'nombre' => 'Historial de precios por artículo/proveedor',
        'icono' => 'fa-chart-line',
        'slug' => 'listar-reporte-historial-precios-compra',
        'permiso_nombre' => 'Listar historial de precios por artículo/proveedor',
        'orden' => 3,
    ];

    /** Roles que deben recibir el permiso aunque no tengan el menú OC (pedido Enc-compras). */
    private const ROLES_EXPLICITOS = ['Enc-compras', 'Op-Compras', 'administrador'];
    ...
    public function up(): void
    {
        ...
        $refMenuId = (int) (DB::table('menu')->where('url', 'compras/ordencompra-reporte')->value('id') ?? 0);
        if ($refMenuId === 0) {
            $refMenuId = (int) (DB::table('menu')->where('url', 'compras/ordencompra')->value('id') ?? 0);
        }
        $refPermisoId = (int) (DB::table('permiso')->where('slug', 'listar-reporte-ordencompra')->value('id') ?? 0);
        if ($refPermisoId === 0) {
