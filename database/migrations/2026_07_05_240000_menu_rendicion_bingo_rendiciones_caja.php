...
return new class extends Migration
{
    ...
    private const MENU_URL_LEGACY = 'caja/bingo/rendicion';
    ...
    public function up(): void
    {
        ...
        $menuId = $this->upsertMenu(self::MENU_URL, 'Rendiciones bingo', $padreId, $orden, 'fa-ticket');

        $legacyId = (int) (DB::table('menu')->where('url', self::MENU_URL_LEGACY)->value('id') ?? 0);
        if ($legacyId > 0 && $legacyId !== $menuId) {
            DB::table('permiso')->where('menu_id', $legacyId)->update(['menu_id' => $menuId]);
            foreach (DB::table('menu_rol')->where('menu_id', $legacyId)->pluck('rol_id') as $rolId) {
                if (! DB::table('menu_rol')->where('menu_id', $menuId)->where('rol_id', $rolId)->exists()) {
                    DB::table('menu_rol')->insert(['menu_id' => $menuId, 'rol_id' => $rolId]);
                }
            }
            DB::table('menu_rol')->where('menu_id', $legacyId)->delete();
            DB::table('menu')->where('id', $legacyId)->delete();
        }

        $slugs = [
            ['nombre' => 'Listar rendiciones bingo caja', 'slug' => 'listar-rendicion-bingo-caja'],
            ['nombre' => 'Ingresar rendición bingo caja', 'slug' => 'crear-rendicion-bingo-caja'],
    ...
    private function upsertMenu(string $url, string $nombre, int $padre, int $orden, string $icono): int
    {
        ...
        if ($id === 0) {
            $id = (int) (DB::table('menu')->where('url', self::MENU_URL_LEGACY)->value('id') ?? 0);
        }
