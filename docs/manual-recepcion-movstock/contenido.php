    ...
    'secciones' => [
        ...
        [
            ...
            'items' => [
                ...
                'Revertir confirmada (listado Movimientos de Stock): deshace el movimiento de salida y el de entrada por separado (mismo artículo, depósito y cantidad de cada uno, signo invertido). Si hubo conversión de fórmula no se vuelve a convertir.',
            ],
        ],
        [
            ...
            'parrafos' => [
                'Menú: Stock → Transferencia. Diseño pensado para depósito, tablet o celular: cabecera con TRA y depósitos (por defecto 1 → 8 gastronomía, modificables), caja de pickeo, lista de ítems y barra inferior Transferir.',
                'Rutina de pickeo: botón Cámara (en HTTPS lee en vivo como un QR; en HTTP el celular no permite la cámara continua y hay que sacar foto), lector Bluetooth o SKU a mano y Enter. Resuelve el artículo por SKU, código de barras del maestro o código de barras / código de artículo de la solapa Proveedores. Trae el stock del depósito de salida, suma cantidad 1 (si el ítem ya está, incrementa 1 hasta el saldo). Puede corregir la cantidad a mano. Al Transferir se genera la TRA en el ERP.',
                'Complete empresa, depósito salida, depósito entrada (o bienes de uso según tipo), tipo de transacción transferencia y centro de costo destino si el tipo lleva contabilidad (etiqueta «contabilidad» en el desplegable). Empresa, depósitos y tipo se recuerdan al cambiarlos (por usuario); no hace falta Transferir para que queden.',
                'TRCONT: si el artículo es de otros activos y el depósito de salida tiene COM de ese SKU, la pantalla pasa a TRCONT. Si no hubo COM en ese depósito, vuelve a TRA y no aborta: se registra el movimiento de stock. TITO: el depósito de salida debe ser el de la última recepción. «Cargar stock» con TRCONT lista solo artículos válidos; los demás se omiten con aviso.',
                ...
                'Transferir (N): envía la transferencia; N indica cuántos ítems tienen cantidad > 0. Al confirmar aparece un aviso de «Grabando…» (el botón se bloquea para no grabar dos veces) y después el código generado (TR-…). Hay que tocar Entendido. Si falta centro de costo destino en TRCONT, el sistema rechaza la operación.',
        ...
        [
            ...
            'items' => [
                ...
                'Transferencia inmediata (pickeo): Transferencia → TRA y depósitos 1→8 ya vienen cargados → pickear SKU, barra del artículo o barra/código de la solapa Proveedores (cantidad 1) → Transferir.',
