...
/**
 * Genera Manual de Usuario Pedidos y Facturación (Word + PDF) con capturas.
 * Ejecutar: php docs/manual-ventas/generar.php
 */
...
$outDir = __DIR__;
$manual = app(ManualVentasService::class);
$contenido = $manual->meta();
$capturas = config('manual_ventas.capturas', []);

$baseName = 'Manual_Usuario_AnitaERP_Pedidos_Facturacion';
$docxPath = $outDir . '/' . $baseName . '.docx';
$pdfPath = $outDir . '/' . $baseName . '.pdf';
$htmlPath = $outDir . '/' . $baseName . '_preview.html';
$imgDir = public_path('docs/manual-ventas/img');

function capturaImagenPath(array $cap): ?string
{
    global $imgDir;
    $archivo = $cap['archivo'] ?? '';
    $base = preg_replace('/\.(svg|png)$/i', '', $archivo);
...
function esSvg(string $path): bool
{
...
if (class_exists(PhpWord::class)) {
    ...
    foreach ($contenido['secciones'] as $sec) {
        ...
        foreach (capturasParaSeccion($sec, $capturas) as $cap) {
            $imgPath = capturaImagenPath($cap);
            if ($imgPath && ! esSvg($imgPath)) {
        ...
        if (! empty($sec['nota'])) {
            $section->addText($sec['nota'], ['size' => 11, 'italic' => true], ['spaceAfter' => 120, 'alignment' => Jc::BOTH]);
        }
        if (! empty($sec['flujo'])) {
            $section->addText($sec['flujo'], ['size' => 9, 'name' => 'Courier New'], ['spaceAfter' => 120]);
        }
        if (! empty($sec['items'])) {
    ...
    echo "Aviso: PhpWord no instalado; se omite .docx\n";
}
...
function buildHtml(array $contenido, array $capturasCfg): string
{
    ...
    foreach ($contenido['secciones'] as $sec) {
        ...
        if (! empty($sec['nota'])) {
            $h .= '<div class="highlight">' . e($sec['nota']) . '</div>';
        }
        if (! empty($sec['flujo'])) {
            $h .= '<div class="flow-box">' . e($sec['flujo']) . '</div>';
        }
        if (! empty($sec['items'])) {
            $h .= '<ul>';
            foreach ($sec['items'] as $item) {
        ...
        foreach (['tabla', 'tabla2'] as $tablaKey) {
            ...
            $tabla = $sec[$tablaKey];
            if (! empty($tabla['caption'])) {
                $h .= '<p class="table-caption">' . e($tabla['caption']) . '</p>';
            }
            $h .= '<table><thead><tr>';
            foreach ($tabla['headers'] as $hd) {
                $h .= '<th>' . e($hd) . '</th>';
            }
            $h .= '</tr></thead><tbody>';
            foreach ($tabla['rows'] as $row) {
