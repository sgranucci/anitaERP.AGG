...
/**
 * Genera Manual Canjes Marketing (Word + PDF) con capturas.
 * Ejecutar: php docs/manual-canjes-marketing/generar.php
 */
...
use App\Services\Ventas\ManualCanjesMarketingService;
...
foreach ($contenido['secciones'] as $sec) {
    ...
    if (! empty($sec['flujo'])) {
        $section->addText($sec['flujo'], ['name' => 'Courier New', 'size' => 9], ['spaceAfter' => 120]);
    }
    if (! empty($sec['items'])) {
...
function buildHtml(array $contenido, array $capturasCfg): string
{
    ...
    $h .= '<p class="meta">Versión ' . e($contenido['version']) . ' — ' . e($contenido['fecha']) . '</p></div>';
    ...
    foreach ($contenido['secciones'] as $sec) {
        ...
        if (! empty($sec['flujo'])) {
            $h .= '<pre class="mc-flow-box">' . e($sec['flujo']) . '</pre>';
        }
        if (! empty($sec['items'])) {
            $h .= '<ul>';
            foreach ($sec['items'] as $item) {
