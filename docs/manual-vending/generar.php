...
/**
 * Genera Manual Vending (Word + PDF) con capturas.
 * Ejecutar: php docs/manual-vending/generar.php
 */
...
$outDir = __DIR__;
$manual = app(ManualVendingService::class);
$contenido = $manual->meta();
$capturas = config('manual_vending.capturas', []);

$baseName = 'Manual_Usuario_AnitaERP_Vending';
$docxPath = $outDir . '/' . $baseName . '.docx';
$pdfPath = $outDir . '/' . $baseName . '.pdf';
$htmlPath = $outDir . '/' . $baseName . '_preview.html';
$imgDir = public_path('docs/manual-vending/img');
...
function capturasParaSeccion(array $sec, array $capturasCfg): array
{
    ...
    if (! empty($sec['captura_id']) && isset($capturasCfg[$sec['captura_id']]) && ! isset($seen[$sec['captura_id']])) {
