    ...
    <div class="cliente-premio-uif-archivos-preview row">
        @foreach ($archivosList as $arch)
            @php
                ...
                $pathSub = 'storage/archivos/clientes_premios_uif/'.$premioId.'/'.$safeName;
                $pathFlat = 'storage/archivos/clientes_premios_uif/'.$safeName;
                $publicSub = public_path($pathSub);
                $publicFlat = public_path($pathFlat);
                $urlInline = file_exists($publicSub)
                    ? asset($pathSub)
                    : (file_exists($publicFlat) ? asset($pathFlat) : asset($pathSub));
