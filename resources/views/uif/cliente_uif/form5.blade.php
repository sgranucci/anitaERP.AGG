    ...
    <div class="card-body">
    	<table class="table" id="archivo-table">
    		...
    		<tbody id="tbody-tabla-archivo">
		 		@if ($data->cliente_archivos_uif ?? '') 
					@foreach (old('archivos', $data->cliente_archivos_uif->count() ? $data->cliente_archivos_uif : ['']) as $archivo)
            			<tr class="item-archivo">
                			<td>
								<input type="file" name="nombrearchivos[]" class="form-control nombrearchivos" 
									onchange='actualizaArchivo(this)'
									data-initial-preview="{{isset($archivo->nombrearchivo) ? asset("storage/archivos/clientes_uif/$archivo->nombrearchivo") : ''}}" \>
                			...
                			<td>
								@if ($archivo->nombrearchivo ?? '')
                                	...
                                	@else
										<img heigh=100px width=100px src="{{ asset("storage/archivos/clientes_uif/$archivo->nombrearchivo") }}" alt="image">
									@endif
                			...
                			<td>
								@if ($archivo->nombrearchivo ?? '')
									<a download="{{$archivo->nombrearchivo}}" href="{{ asset("storage/archivos/clientes_uif/$archivo->nombrearchivo") }}" title='Descargar' /><i class="fa fa-download"></i>
