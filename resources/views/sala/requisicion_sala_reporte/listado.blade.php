...
<head>
    ...
    <title>{{ $titulo ?? 'Requisiciones de SALA' }}</title>
    <style>
        body { font-family: DejaVu Sans, Helvetica, Arial, sans-serif; font-size: 7px; color: #1a1a1a; line-height: 1.3; }
        table.data {
    ...
    <table class="listado-header">
        <tr>
            ...
            <td style="width: 46%; text-align: center;">
                <h2 style="margin: 0; font-size: 16px; font-weight: bold;">{{ $titulo ?? 'Requisiciones de SALA' }}</h2>
                ...
                @if (! empty($totales))
                    <div class="meta">
                        ...
                        · Cantidad: {{ number_format((float) ($totales['total_cantidad'] ?? 0), 0, ',', '.') }}
                        · Pendiente: {{ number_format((float) ($totales['total_pendiente'] ?? 0), 0, ',', '.') }}
                    </div>
                @endif
            </td>
