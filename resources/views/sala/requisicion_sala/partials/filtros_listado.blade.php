@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? RequisicionSalaListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'numerorequisicion';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (RequisicionSalaListadoFiltros::CAMPOS as $key => $meta) {
...
@endphp
<div class="collapse border-bottom{{ $tieneCriteriosPanel ? ' show' : '' }}" id="panel-filtros-requisicion-sala" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ RequisicionSalaListadoFiltros::MODO_TODOS }}" {{ $modo === RequisicionSalaListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ RequisicionSalaListadoFiltros::MODO_CAMPO }}" {{ $modo === RequisicionSalaListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== RequisicionSalaListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? RequisicionSalaListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(RequisicionSalaListadoFiltros::operadoresParaCampo($modo === RequisicionSalaListadoFiltros::MODO_CAMPO ? $campoActivo : 'numerorequisicion') as $opKey => $opLabel)
