@php
    $archivosList = isset($data) && $data && ($data->capex_archivos ?? null)
        ? $data->capex_archivos
        : collect();
...
@if ($archivosList->count())
    <div class="capex-archivos-preview row">
        @foreach ($archivosList as $arch)
            @php
                $safeName = $arch->nombrearchivo;
                $ext = strtolower(pathinfo($safeName, PATHINFO_EXTENSION));
                $urlInline = \App\Support\Archivos\ArchivoAdjuntoCacheSupport::urlStoragePublico('archivos/capex/'.$data->id.'/'.$safeName);
                $urlDescarga = $urlInline;
                $esImagen = in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp'], true);
                $esPdf = $ext === 'pdf';
            @endphp
            <div class="col-md-6 col-lg-4 mb-3 capex-archivo-item">
                <div class="card card-outline card-secondary h-100 mb-0">
                    <div class="card-body p-2 d-flex flex-column">
                        ...
                        @if (! $ocultarInputsConservar)
                            ...
                            <button type="button" class="btn btn-sm btn-outline-danger mt-2 eliminar-archivo-capex" title="Quitar de la lista; se elimina del Capex al guardar">
