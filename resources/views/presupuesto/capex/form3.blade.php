@php
    $tieneCapex = isset($data) && $data && ($data->id ?? null);
    $puedeEditarArchivos = ! isset($visualizar) && (empty($soloConsulta) || ! empty($puedeActualizarCapex));
    $cantArchivos = $tieneCapex ? ($data->capex_archivos?->count() ?? 0) : 0;
@endphp
...
<div class="card card-outline card-info mb-3">
    ...
    <div class="card-body">
        <p class="text-muted small mb-3">
            Documentación del Capex (presupuestos, planos, cotizaciones, etc.). PDF, imágenes u otros.
            Los cambios se confirman al guardar.
        </p>
        @if ($tieneCapex)
            <p class="text-muted small mb-2 font-weight-bold">Archivos actuales</p>
            @include('presupuesto.capex.partials.archivos_adjuntos', [
                'data' => $data,
                'ocultarInputsConservar' => ! $puedeEditarArchivos,
            ])
        @else
            <div class="text-center text-muted py-3 bg-light rounded mb-0">
                Guarde el Capex para ver archivos asociados en esta solapa. Puede adjuntar archivos nuevos abajo.
            </div>
        @endif
    </div>
</div>
    ...
    <div class="card card-outline card-primary mb-0">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-sm table-bordered mb-2" id="capex-archivo-table">
                    ...
                    <tbody id="capex-tbody-tabla-archivo">
            ...
            <div class="text-right">
                <button id="capex-agrega-renglon-archivo" type="button" class="btn btn-outline-primary btn-sm">
