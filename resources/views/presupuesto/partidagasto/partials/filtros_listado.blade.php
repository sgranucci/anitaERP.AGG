@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? PartidagastoListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'detalle';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (PartidagastoListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = PartidagastoListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = PartidagastoListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('consultar_partidagasto');
@endphp
<div class="collapse border-bottom" id="panel-filtros-partidagasto" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ PartidagastoListadoFiltros::MODO_TODOS }}" {{ $modo === PartidagastoListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ PartidagastoListadoFiltros::MODO_CAMPO }}" {{ $modo === PartidagastoListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== PartidagastoListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? PartidagastoListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(PartidagastoListadoFiltros::operadoresParaCampo($modo === PartidagastoListadoFiltros::MODO_CAMPO ? $campoActivo : 'detalle') as $opKey => $opLabel)
