@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? PresupuestoListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (PresupuestoListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = PresupuestoListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = PresupuestoListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('consultar_presupuesto');
@endphp
<div class="collapse border-bottom" id="panel-filtros-presupuesto" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ PresupuestoListadoFiltros::MODO_TODOS }}" {{ $modo === PresupuestoListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ PresupuestoListadoFiltros::MODO_CAMPO }}" {{ $modo === PresupuestoListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== PresupuestoListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? PresupuestoListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(PresupuestoListadoFiltros::operadoresParaCampo($modo === PresupuestoListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
