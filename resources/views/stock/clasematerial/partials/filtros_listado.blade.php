@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? ClasematerialListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (ClasematerialListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = ClasematerialListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = ClasematerialListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('clasematerial');
@endphp
<div class="collapse border-bottom" id="panel-filtros-clasematerial" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ ClasematerialListadoFiltros::MODO_TODOS }}" {{ $modo === ClasematerialListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ ClasematerialListadoFiltros::MODO_CAMPO }}" {{ $modo === ClasematerialListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== ClasematerialListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? ClasematerialListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(ClasematerialListadoFiltros::operadoresParaCampo($modo === ClasematerialListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
