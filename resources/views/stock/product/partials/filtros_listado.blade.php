@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? ArticuloFerliListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'descripcion';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (ArticuloFerliListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = ArticuloFerliListadoFiltros::operadoresParaCampo($key);
    }
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ ArticuloFerliListadoFiltros::MODO_TODOS }}" {{ $modo === ArticuloFerliListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ ArticuloFerliListadoFiltros::MODO_CAMPO }}" {{ $modo === ArticuloFerliListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== ArticuloFerliListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach(ArticuloFerliListadoFiltros::CAMPOS as $key => $meta)
                        <option value="{{ $key }}" {{ $campoActivo === $key ? 'selected' : '' }}>{{ $meta['label'] }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(ArticuloFerliListadoFiltros::operadoresParaCampo($modo === ArticuloFerliListadoFiltros::MODO_CAMPO ? $campoActivo : 'descripcion') as $opKey => $opLabel)
