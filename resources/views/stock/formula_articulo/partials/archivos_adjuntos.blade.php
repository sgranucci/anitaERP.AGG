@php
    $archivosList = isset($data) && $data && $data->formula_articulo_archivos
        ? $data->formula_articulo_archivos
        : collect();
    $ocultarInputsConservar = $ocultarInputsConservar ?? false;
@endphp

@if ($archivosList->count())
    <div class="formula-archivos-preview row">
        @foreach ($archivosList as $arch)
            @php
                ...
                $esPdf = $ext === 'pdf';
            @endphp
            <div class="col-md-6 col-lg-4 mb-3 formula-archivo-item">
                    ...
                    <div class="card-body p-2 d-flex flex-column">
                        ...
                        @if ($esImagen)
                            <div class="text-center bg-light rounded mb-2" style="min-height: 120px;">
                                <a href="{{ $urlPublica }}" target="_blank" rel="noopener noreferrer" title="Abrir imagen">
                                    <img src="{{ $urlPublica }}" alt="" class="img-fluid rounded" style="max-height: 180px; object-fit: contain;">
                                </a>
                            </div>
                        @elseif ($esPdf)
                            <div class="flex-grow-1 mb-2" style="min-height: 200px;">
                                <iframe src="{{ $urlPublica }}" class="w-100 rounded border-0 bg-secondary" style="height: 220px;" title="Vista previa PDF"></iframe>
                            </div>
                        @else
                        ...
                        <div class="mt-auto pt-1">
                            ...
                            <a href="{{ $urlPublica }}" class="btn btn-sm btn-outline-secondary ml-1" target="_blank" rel="noopener noreferrer" title="Abrir en nueva pestaña">
                        ...
                        @if (! $ocultarInputsConservar)
                            ...
                            <button type="button" class="btn btn-sm btn-outline-danger mt-2 eliminar-archivo-formula" title="Quitar de la lista; se elimina al guardar">
