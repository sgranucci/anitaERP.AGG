@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? DepmaeListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (DepmaeListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = DepmaeListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = DepmaeListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('depmae');
@endphp
<div class="collapse border-bottom" id="panel-filtros-depmae" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ DepmaeListadoFiltros::MODO_TODOS }}" {{ $modo === DepmaeListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ DepmaeListadoFiltros::MODO_CAMPO }}" {{ $modo === DepmaeListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== DepmaeListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? DepmaeListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(DepmaeListadoFiltros::operadoresParaCampo($modo === DepmaeListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
