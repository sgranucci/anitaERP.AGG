...
<div class="alert alert-secondary mb-0">
    El tipo de transacción seleccionado no genera asiento contable.
</div>
@elseif(! empty($preview['error']))
<div class="alert alert-danger">
    <strong>No se puede generar el asiento:</strong> {{ $preview['error'] }}
    @if(str_contains((string) $preview['error'], 'tipo de asiento') || str_contains((string) $preview['error'], 'STK'))
    <div class="mt-2 small">
        Verifique en Contable &rarr; Tipos de asiento que exista la abreviatura <strong>STK</strong> (Stock).
    </div>
    @endif
</div>
@else
...
<div class="alert alert-info py-2">
    Vista previa: el asiento se grabará al guardar el movimiento (si el cuadre contable es válido).
</div>
@else
<div class="d-flex flex-wrap align-items-center mb-3" style="gap: 8px;">
    ...
    @if($urlAsiento)
    <a href="{{ $urlAsiento }}" class="btn btn-sm btn-outline-primary" target="_blank" rel="noopener" title="Consultar asiento contable">
...
<div class="alert alert-warning py-2">
    <ul class="mb-0 pl-3">
        @foreach($preview['advertencias'] as $aviso)
        <li>{{ $aviso }}</li>
        @endforeach
    </ul>
</div>
...
<div class="table-responsive">
    <table class="table table-bordered table-sm" id="tabla-asiento-movimientostock">
        ...
        <tfoot>
            ...
            @if(! empty($preview['es_preview']) && isset($preview['total_movimiento']))
            <tr>
                <td colspan="2" class="text-right text-muted">Total movimiento (contable)</td>
                <td colspan="3" class="text-muted">{{ number_format((float) $preview['total_movimiento'], 2, ',', '.') }}</td>
            </tr>
            @endif
        </tfoot>
        @endif
