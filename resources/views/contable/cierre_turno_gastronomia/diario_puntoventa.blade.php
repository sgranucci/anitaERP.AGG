...
@section('titulo')
    Diario gastronom&iacute;a por PV (Contable)
@endsection
    ...
    <div class="col-lg-12">
        ...
        <div class="card card-info">
            <div class="card-header d-flex align-items-center flex-wrap">
                <h3 class="card-title mb-0">Diario por punto de venta y medios de pago</h3>
            ...
            <div class="card-body">
                <div class="alert alert-info py-2 small mb-3">
                    ...
                    <ul class="mb-0 pl-3">
                        <li>Una secci&oacute;n por <strong>jornada</strong>; dentro, una fila por <strong>punto de venta</strong> (sin estacionamiento).</li>
                        <li><strong>Venta bruta</strong> = facturas; <strong>NC</strong> = notas de cr&eacute;dito; <strong>Venta neta</strong> = bruto &minus; NC.</li>
                        <li>Expand&iacute; cada PV para ver <strong>medios de pago</strong>: cobros de facturas, devoluciones por NC y neto.</li>
                        <li>Fuente: emisiones gastronom&iacute;a ERP + cobranzas / cuentas de caja (misma l&oacute;gica que cierres de turno).</li>
                    </ul>
                </div>

                <form method="get" action="{{ route('cierres_turno_gastronomia_contable_diario_puntoventa') }}" class="mb-4">
                ...
                @if ($consultar && empty($error_reporte) && $resultado !== null)
                        ...
                        @if (can('exportar-cierres-turno-gastronomia-contable', false))
                            <div class="mr-2 mb-1">
                                @include('includes.exportar-tabla-queryparams', [
                                    'ruta' => 'listar_cierres_turno_gastronomia_contable_diario_puntoventa',
                    ...
                    @if ($dias === [])
                        <p class="text-muted text-center py-4">Sin facturaci&oacute;n gastronom&iacute;a en el rango indicado.</p>
                    @else
                                ...
                                <div class="card-header py-2" style="background:#d6eaf8;color:#17202A;">
                                    <div class="d-flex flex-wrap justify-content-between align-items-center">
                                        ...
                                        <span class="small">
                                            ...
                                            Bruto {{ number_format((float) ($totDia['venta_bruta'] ?? 0), 2, ',', '.') }}
                                            · NC {{ number_format((float) ($totDia['total_nc'] ?? 0), 2, ',', '.') }}
                                            · Neto <strong>{{ number_format((float) ($totDia['venta_neta'] ?? 0), 2, ',', '.') }}</strong>
                                        </span>
                                    </div>
                                </div>
                                        ...
                                        <table class="table table-bordered table-hover mb-0" style="font-size: 0.95rem;">
                                            <thead style="background:#85C1E9;color:#17202A;">
                                                <tr>
                                                    ...
                                                    <th class="text-right">Notas cr&eacute;dito</th>
                                            ...
                                            <tbody>
                                                @foreach ($dia['puntoventas'] ?? [] as $idxPv => $pv)
                                                    @php
                                                        ...
                                                        $pcs = implode(', ', $pv['identificadores_pc'] ?? []);
                                                    @endphp
                                                    <tr class="{{ empty($pv['cuadre_ok']) ? 'table-warning' : '' }}">
                                                        ...
                                                        <td class="align-middle small"><code>{{ $pcs !== '' ? $pcs : '—' }}</code></td>
                                                        <td class="text-center align-middle">{{ (int) ($pv['cantidad_facturas'] ?? 0) }}</td>
                                                        <td class="text-right align-middle font-weight-bold">{{ number_format((float) ($pv['venta_bruta'] ?? 0), 2, ',', '.') }}</td>
                                                        <td class="text-center align-middle">{{ (int) ($pv['cantidad_nc'] ?? 0) }}</td>
                                                        <td class="text-right align-middle">{{ number_format((float) ($pv['total_nc'] ?? 0), 2, ',', '.') }}</td>
                                                        <td class="text-right align-middle font-weight-bold">{{ number_format((float) ($pv['venta_neta'] ?? 0), 2, ',', '.') }}</td>
                                                                    ...
                                                                    <div class="table-responsive">
                                                                        <table class="table table-sm table-striped table-bordered mb-0 bg-white">
                                                                            <thead style="background:#85C1E9;color:#17202A;">
                                                                                <tr>
                                                                                    <th>Medio de pago</th>
                                                                                    <th class="text-right">Cobros (facturas)</th>
                                                                            ...
                                                                            <tbody>
                                                                                @forelse ($pv['medios'] ?? [] as $medio)
                                                                                    <tr>
                                                                                        ...
                                                                                        <td class="text-right">{{ number_format((float) ($medio['devoluciones_nc'] ?? 0), 2, ',', '.') }}</td>
                                                                                        <td class="text-right font-weight-bold">{{ number_format((float) ($medio['neto'] ?? 0), 2, ',', '.') }}</td>
                                                                                    </tr>
                                                                                @empty
                                                                                    <tr>
                                                                                        <td colspan="4" class="text-center text-muted py-2">Sin cobranza registrada en este PV.</td>
                                                                                    </tr>
                                                                                @endforelse
                                                                            </tbody>
                                                                        </table>
                                            ...
                                            <tfoot>
                                                <tr style="background:#eaf2f8;font-weight:bold;">
                                                    <td></td>
                                                    <td colspan="2">Total jornada</td>
                                                    <td class="text-center">{{ (int) ($totDia['cantidad_facturas'] ?? 0) }}</td>
                                                    <td class="text-right">{{ number_format((float) ($totDia['venta_bruta'] ?? 0), 2, ',', '.') }}</td>
                                                    <td class="text-center">{{ (int) ($totDia['cantidad_nc'] ?? 0) }}</td>
                                                    <td class="text-right">{{ number_format((float) ($totDia['total_nc'] ?? 0), 2, ',', '.') }}</td>
                                                    <td class="text-right">{{ number_format((float) ($totDia['venta_neta'] ?? 0), 2, ',', '.') }}</td>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <table class="table table-sm mb-0">
                                            <tr>
                                                <td>Facturas</td>
                                                <td class="text-right">{{ (int) ($resumen['cantidad_facturas'] ?? 0) }}</td>
                                            </tr>
                                            <tr>
                                                <td>Venta bruta</td>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right">{{ number_format((float) ($resumen['total_nc'] ?? 0), 2, ',', '.') }}
                                                    <small class="text-muted">({{ (int) ($resumen['cantidad_nc'] ?? 0) }})</small>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>Venta neta</td>
                                                <td class="text-right font-weight-bold">{{ number_format((float) ($resumen['venta_neta'] ?? 0), 2, ',', '.') }}</td>
                                            </tr>
                                            <tr>
                                                <td>Cobrado neto</td>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-sm table-striped table-bordered mb-0">
                                                ...
                                                <tbody>
                                                    ...
                                                    @empty
                                                        <tr>
                                                            <td colspan="4" class="text-center text-muted">Sin medios</td>
                                                        </tr>
                                                    @endforelse
                                                </tbody>
                                            </table>
