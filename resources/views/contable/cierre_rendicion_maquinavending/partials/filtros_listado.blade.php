@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? CierreRendicionMaquinavendingListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'codigo';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (CierreRendicionMaquinavendingListadoFiltros::CAMPOS as $key => $meta) {
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="estado_cierre" id="estado_cierre" class="form-control form-control-sm">
                    <option value="{{ CierreRendicionMaquinavendingListadoFiltros::ESTADO_TODOS }}" @selected($estadoCierre === CierreRendicionMaquinavendingListadoFiltros::ESTADO_TODOS)>Todos</option>
                    <option value="{{ CierreRendicionMaquinavendingListadoFiltros::ESTADO_PENDIENTE }}" @selected($estadoCierre === CierreRendicionMaquinavendingListadoFiltros::ESTADO_PENDIENTE)>Pendiente</option>
                    <option value="{{ CierreRendicionMaquinavendingListadoFiltros::ESTADO_CERRADA }}" @selected($estadoCierre === CierreRendicionMaquinavendingListadoFiltros::ESTADO_CERRADA)>Cerrada</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2">
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ CierreRendicionMaquinavendingListadoFiltros::MODO_TODOS }}" @selected($modo === CierreRendicionMaquinavendingListadoFiltros::MODO_TODOS)>Cualquier campo</option>
                    <option value="{{ CierreRendicionMaquinavendingListadoFiltros::MODO_CAMPO }}" @selected($modo === CierreRendicionMaquinavendingListadoFiltros::MODO_CAMPO)>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== CierreRendicionMaquinavendingListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? CierreRendicionMaquinavendingListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm" data-operadores='@json($operadoresJson)'>
                    @foreach(CierreRendicionMaquinavendingListadoFiltros::operadoresParaCampo($modo === CierreRendicionMaquinavendingListadoFiltros::MODO_CAMPO ? $campoActivo : 'codigo') as $opKey => $opLabel)
