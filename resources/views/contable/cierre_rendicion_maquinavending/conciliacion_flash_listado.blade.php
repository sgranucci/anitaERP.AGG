@php
    ...
    $filas = $filas ?? CierreRendicionMaquinavendingConciliacionFlashExport::aplanarFilas($resultado ?? []);
    ...
    foreach ($filas as $fSum) {
        ...
        $totFlash += (float) ($fSum['total_flash_vending'] ?? 0);
        $totRendg += (float) ($fSum['total_rendgastro_z'] ?? 0);
        $totAsientos += (float) ($fSum['total_asientos_debe'] ?? 0);
        $totDifFlash += (float) ($fSum['diferencia_flash'] ?? 0);
        $totDifRendg += (float) ($fSum['diferencia_rendgastro'] ?? 0);
...
<head>
    ...
    <title>Conciliacion flash vending</title>
    <style>
...
@else
    <table class="listado-header">
        <tr>
            ...
            <td style="width: 47%; text-align: center;">
                <h2 style="margin: 0; font-size: 13px; font-weight: bold;">Conciliaci&oacute;n flash vending</h2>
    ...
    <p style="font-size: 6.5px; color: #555; margin: 4px 0 8px;">
        Fact. neta = ventas netas (comparable a flash vending y rendgastro Z).
        Venta total = ventas del d&iacute;a (comparable a &Sigma; debe del asiento; vending no maneja notas de cr&eacute;dito).
        Dif. flash = fact. neta − flash_caja.vending.
        Dif. rendg. = fact. neta − rendgastro Z.
...
<table class="data">
    <thead>
        <tr>
            ...
            <th class="num">Flash vending</th>
            <th class="num">Rendg. Z</th>
            <th class="num">Asientos</th>
            <th class="num">Dif. fact.&minus;flash</th>
            <th class="num">Dif. fact.&minus;rendg.</th>
