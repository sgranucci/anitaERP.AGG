@php
    ...
    $resultado = $resultado ?? [];
    $matriz = $matriz ?? MaquinavendingDiarioPuntoventaExport::matrizAncha($resultado);
...
<head>
    ...
    <style>
        ...
        .th-venta, .th-nc { background-color: #5DADE2; }
...
@if ($esExcel)
    <table>
        ...
        <tbody>
            <tr>
                <td colspan="{{ $colspan }}"><strong style="font-size: 14pt;">Diario vending por PV / medios (Contable)</strong></td>
            </tr>
            <tr>
            ...
            <tr>
                <td colspan="{{ $colspan }}">
                    ...
                    — {{ count($bloquesPv) }} PV
                    — columnas: FECHA + por cada PV: medios y Venta total + TOTAL DÍA
                </td>
            </tr>
        </tbody>
    </table>
    ...
    <table class="listado-header">
        <tr>
            ...
            <td style="width: 56%; text-align: center;">
                <h2 style="margin: 0; font-size: 12px; font-weight: bold;">Diario vending por PV / medios (Contable)</h2>
...
@if ($bloquesPv === [])
    <p style="text-align:center;">Sin rendiciones vending en el rango indicado.</p>
@else
    <table class="data">
        <thead>
            <tr>
            ...
            <tr>
                @foreach ($bloquesPv as $bloque)
                    ...
                    @endforeach
                    <th class="th-venta {{ $esTotal ? 'th-total-dia' : '' }}">Venta total</th>
                @endforeach
            </tr>
        </thead>
        <tbody>
