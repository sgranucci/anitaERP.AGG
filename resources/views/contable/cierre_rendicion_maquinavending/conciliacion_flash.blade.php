...
<script>
    window.CIERRE_REND_VEND_CONC = {
        urlEjecutarJornada: @json(route('api_cierre_rendicion_maquinavending_ejecutar_jornada')),
        urlEjecutarPeriodo: @json(route('api_cierre_rendicion_maquinavending_ejecutar_rango')),
    };
</script>
<script src="{{ asset('assets/pages/scripts/contable/cierre_rendicion_maquinavending/conciliacion_flash.js') }}?v={{ @filemtime(public_path('assets/pages/scripts/contable/cierre_rendicion_maquinavending/conciliacion_flash.js')) ?: time() }}" type="text/javascript"></script>
@endif
@endsection

@section('contenido')
@php
    $tolConfig = (float) config('gastronomia.cierre_rendicion_vending_contable.conciliacion_flash_tolerancia', 0.02);
@endphp
<div class="row">
    <div class="col-lg-12">
        ...
        <div class="card card-info">
            <div class="card-header d-flex align-items-center flex-wrap">
                <h3 class="card-title mb-0">Conciliaci&oacute;n rendiciones vending vs flash / rendgastro</h3>
            ...
            <div class="card-body">
                <div class="alert alert-info py-2 small mb-3">
                    ...
                    <ul class="mb-0 pl-3">
                        <li><strong>Fact. neta</strong> se compara con el <strong>flash vending</strong> (campo <code>flash_caja.vending</code> del m&oacute;dulo flash ERP; puede estar en 0 mientras no haya fuente) y con la <strong>rendgastro Z</strong> del punto de venta (Anita).</li>
                        <li><strong>Venta total</strong> se compara con &Sigma; debe de asientos → diferencia &ldquo;venta total − asientos&rdquo; (vending no maneja notas de cr&eacute;dito).</li>
                        <li>El <strong>estado</strong> compara contra la fuente externa con dato (rendgastro Z primero, luego flash); si ninguna tiene dato, contra los asientos.</li>
                    </ul>
                    Tolerancia: {{ number_format($tolConfig, 2, ',', '.') }}.
                    Expand&iacute; cada jornada para ver el detalle por punto de venta.
                </div>
                <form method="get" action="{{ route('cierre_rendicion_maquinavending_conciliacion_flash') }}" class="mb-4">
                ...
                @if ($consultar && empty($error_flash) && $resultado !== null)
                    @php
                        ...
                        foreach ($dias as $dSum) {
                            ...
                            $totFlash += (float) ($dSum['total_flash_vending'] ?? 0);
                            $totRendg += (float) ($dSum['total_rendgastro_z'] ?? 0);
                            ...
                            @if (can('exportar-cierre-rendicion-maquinavending-contable', false))
                                <div class="mr-2 mb-1">
                                    @include('includes.exportar-tabla-queryparams', [
                                        'ruta' => 'listar_cierre_rendicion_maquinavending_conciliacion_flash',
                            ...
                            <table id="tabla-paginada" class="table table-bordered table-hover mb-0" style="font-size: 0.9rem;">
                                <thead style="background:#85C1E9;color:#17202A;">
                                    <tr>
                                        ...
                                        <th class="text-right">Flash vending</th>
                                        <th class="text-right">Rendg. Z</th>
                                        <th class="text-right">Asientos</th>
                                        <th class="text-right">Fact.&minus;Flash</th>
                                        <th class="text-right">Fact.&minus;Rendg.</th>
                                ...
                                <tbody>
                                    @foreach ($dias as $idx => $dia)
                                        @php
                                            ...
                                            $difRendg = (float) ($dia['diferencia_rendgastro'] ?? 0);
                                            $difVentaAsientos = (float) ($dia['diferencia_venta_total_asientos'] ?? 0);
                                            $flashVend = (float) ($dia['total_flash_vending'] ?? 0);
                                            $rendgZ = (float) ($dia['total_rendgastro_z'] ?? 0);
                                        ...
                                        <tr class="{{ $filaClase }}">
                                            ...
                                            <td class="text-right align-middle {{ abs($flashVend) > $tol && abs($difFlash) > $tol ? 'text-danger font-weight-bold' : 'text-muted' }}">
                                                {{ number_format($difFlash, 2, ',', '.') }}
                                            </td>
                                            <td class="text-right align-middle {{ abs($rendgZ) > $tol && abs($difRendg) > $tol ? 'text-danger font-weight-bold' : 'text-success' }}">
                                                {{ number_format($difRendg, 2, ',', '.') }}
                                            </td>
                                            <td class="text-right align-middle {{ abs($difVentaAsientos) > $tol ? 'text-danger font-weight-bold' : 'text-success' }}"
                                                title="Venta total menos Σ debe de asientos">
                                                        ...
                                                        <div class="table-responsive">
                                                            <table class="table table-sm table-striped table-bordered mb-0 bg-white">
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    @empty
                                                                        <tr>
                                                                            <td colspan="7" class="text-center text-muted py-2">Sin rendiciones en esta jornada.</td>
                                                                        </tr>
                                                                    @endforelse
                                                                </tbody>
