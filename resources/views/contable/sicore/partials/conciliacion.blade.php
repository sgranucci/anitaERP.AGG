@if (! empty($conciliacion['habilitada']))
    <div class="card card-outline card-warning mt-3" id="sicore-conciliacion-panel">
        <div class="card-header">
            <h3 class="card-title">Conciliación SICORE vs mayor contable</h3>
            @if ($periodo_texto ?? '')
                ...
                <table class="table table-sm mb-0">
                    <thead style="background-color:#85C1E9;color:#17202A;">
                        <tr>
                            ...
                            <th>Concepto</th>
                            <th>Cuentas</th>
                            <th class="text-right">Total SICORE</th>
                        ...
                        @foreach ($conciliacion['items'] ?? [] as $idx => $item)
                            ...
                            <tr @class([
                                ...
                                <td class="text-center align-middle">
                                    @if ($tieneDetalle)
                                        <button type="button"
                                            class="btn btn-xs btn-outline-secondary js-sicore-audit-toggle"
                                ...
                                <td>
                                    {{ $item['nombre'] ?? '' }}
                                    <div class="small text-muted">{{ $item['concilia_con'] ?? '' }}</div>
                                ...
                                <td class="text-right">
                                    {{ number_format((float) ($item['total_sicore'] ?? 0), 2, ',', '.') }}
                                </td>
                                    ...
                                    <td colspan="9" class="p-0 border-0">
                                        <div id="{{ $auditId }}" class="collapse border-top bg-light">
                                            <div class="px-3 py-3">
                                                ...
                                                @endphp
                                                @if (($resumen['coinciden'] ?? 0) > 0 || ($resumen['solo_mayor'] ?? 0) > 0)
                                                    <div class="alert alert-light border small mb-3 py-2">
                                                        <strong>Desglose de la diferencia</strong>
                                                        <ul class="mb-0 pl-3">
                                                            <li>
                                                                Operaciones que <strong>cuadran</strong> ({{ $resumen['coinciden'] ?? 0 }}):
                                                                SICORE {{ number_format((float) ($resumen['sum_coincidentes_sicore'] ?? 0), 2, ',', '.') }}
                                                            ...
                                                            @if (($resumen['solo_mayor'] ?? 0) > 0)
                                                                <li>
                                                                    ...
                                                                    <span class="text-muted">— no están en el SICORE a presentar</span>
                                                                </li>
                                                            @endif
                                                            @if (($resumen['solo_sicore'] ?? 0) > 0)
                                                                <li>
                                                                    Solo en SICORE ({{ $resumen['solo_sicore'] }}):
                                                                    {{ number_format((float) ($resumen['sum_solo_sicore'] ?? 0), 2, ',', '.') }}
                                                                </li>
                                                            @endif
                                                            <li>
                                                                <strong>Diferencia</strong> = SICORE − mayor neto =
                                                                {{ number_format((float) ($item['total_sicore'] ?? 0), 2, ',', '.') }}
                                                                − ({{ number_format((float) ($item['total_mayor'] ?? 0), 2, ',', '.') }})
                                                ...
                                                <div class="d-flex flex-wrap align-items-center mb-2">
                                                    ...
                                                    @if (($resumen['solo_sicore'] ?? 0) > 0)
                                                        <span class="badge badge-warning ml-1">{{ $resumen['solo_sicore'] }} solo SICORE</span>
                                                    @endif
                                                    @if (($resumen['solo_mayor'] ?? 0) > 0)
                                                    ...
                                                    <table class="table table-sm table-bordered mb-0 bg-white" style="font-size:0.78rem;">
                                                        <thead>
                                                            <tr style="background-color:#85C1E9;color:#17202A;">
                                                                <th>Estado</th>
                                                                <th>Fecha</th>
                                                                <th>Referencia SICORE</th>
                                                                <th>Proveedor</th>
                                                                <th class="text-right">Imp. SICORE</th>
                                                                <th>Mayor</th>
    ...
    <script>
    (function () {
        var panel = document.getElementById('sicore-conciliacion-panel');
