@php
    $tieneAsiento = isset($data) && $data && ($data->id ?? null);
    $archivosList = $tieneAsiento ? ($data->asiento_archivos ?? collect()) : collect();
...
<div class="card card-outline card-info mb-3">
    ...
    <div class="card-body">
        <p class="text-muted small mb-3">
            Documentación del asiento (comprobantes, notas, etc.). PDF o imágenes.
            Los cambios se confirman al guardar el asiento.
        </p>
        @if ($tieneAsiento)
            @include('contable.asiento.partials.archivos_adjuntos', [
                'data' => $data,
                'ocultarInputsConservar' => false,
            ])
        @else
            <div class="text-center text-muted py-3 bg-light rounded mb-0">
                Puede adjuntar archivos al crear; también podrá gestionarlos al editar el asiento.
            </div>
        @endif
    </div>
</div>
...
<div class="card card-outline card-primary mb-0">
    ...
    <div class="card-body">
        <p class="text-muted small mb-2">
            Seleccione un archivo por renglón o use <strong>+ Agrega renglón</strong>.
            @if ($tieneAsiento)
