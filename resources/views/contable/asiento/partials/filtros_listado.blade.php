@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? AsientoListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'numeroasiento';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (AsientoListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = AsientoListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = AsientoListadoFiltros::tieneCriteriosTexto($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('asiento', AsientoListadoFiltros::paraQueryStringEmpresa($f));
    $fScope = $f['empresa_scope'] ?? 'una';
    $fEmp = (int) ($f['empresa_id'] ?? 0);
@endphp
<div class="collapse border-bottom" id="panel-filtros-asiento" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ AsientoListadoFiltros::MODO_TODOS }}" {{ $modo === AsientoListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ AsientoListadoFiltros::MODO_CAMPO }}" {{ $modo === AsientoListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== AsientoListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? AsientoListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(AsientoListadoFiltros::operadoresParaCampo($modo === AsientoListadoFiltros::MODO_CAMPO ? $campoActivo : 'numeroasiento') as $opKey => $opLabel)
            ...
            <div class="form-group col-md-3 col-sm-6 mb-2">
                ...
                <input type="text"
                       ...
                       value="{{ $f['valor'] ?? '' }}"
                       placeholder="Texto o fecha d/m/Y"
                       autocomplete="off">
            </div>
