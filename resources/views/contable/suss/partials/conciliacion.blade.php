@if (! empty($conciliacion['habilitada']))
    <div class="card card-outline card-warning mt-3" id="suss-conciliacion-panel">
        ...
        <div class="card-body p-0">
                        ...
                        @foreach ($conciliacion['items'] ?? [] as $idx => $item)
                            ...
                            <tr @class([
                                ...
                                <td class="text-center align-middle">
                                    @if ($tieneDetalle)
                                        <button type="button"
                                            class="btn btn-xs btn-outline-secondary js-suss-audit-toggle"
                                        ...
                                        <div id="{{ $auditId }}" class="collapse border-top bg-light">
                                            <div class="px-3 py-3">
                                                    ...
                                                    <div class="alert alert-light border small mb-3 py-2">
                                                        ...
                                                        <ul class="mb-0 pl-3">
                                                            <li>
                                                                ...
                                                                SUSS {{ number_format((float) ($resumen['sum_coincidentes_sicore'] ?? 0), 2, ',', '.') }}
                                                            ...
                                                            @if (($resumen['solo_mayor'] ?? 0) > 0)
                                                                <li>
                                                                    ...
                                                                    <span class="text-muted">— no están en el SUSS a presentar</span>
                                                                </li>
                                                            @endif
                                                            @if (($resumen['solo_sicore'] ?? 0) > 0)
                                                                <li>
                                                                    Solo en SUSS ({{ $resumen['solo_sicore'] }}):
                                                                    {{ number_format((float) ($resumen['sum_solo_sicore'] ?? 0), 2, ',', '.') }}
                                                                </li>
                                                            @endif
                                                            <li>
                                                                <strong>Diferencia</strong> = SUSS − mayor neto =
                                                                {{ number_format((float) $totalIibb, 2, ',', '.') }}
                                                                − ({{ number_format((float) ($item['total_mayor'] ?? 0), 2, ',', '.') }})
                                                        ...
                                                        @if (! empty($exp['diferencia_explicada_por_solo_mayor']))
                                                            <p class="mb-0 mt-2 text-muted">
                                                                Todo el SUSS cuadra operación por operación; la diferencia total coincide con los movimientos <em>solo mayor</em>
                                                ...
                                                <div class="d-flex flex-wrap align-items-center mb-2">
                                                    ...
                                                    @if (($resumen['solo_sicore'] ?? 0) > 0)
                                                        <span class="badge badge-warning ml-1">{{ $resumen['solo_sicore'] }} solo SUSS</span>
                                                    @endif
                                                    @if (($resumen['solo_mayor'] ?? 0) > 0)
                                                    ...
                                                    <table class="table table-sm table-bordered mb-0 bg-white" style="font-size:0.78rem;">
                                                        <thead>
                                                            <tr style="background-color:#85C1E9;color:#17202A;">
                                                                <th>Estado</th>
                                                                <th>Fecha</th>
                                                                <th>Referencia SUSS</th>
                                                                <th>Proveedor</th>
                                                                <th class="text-right">Imp. SUSS</th>
                                                                <th>Mayor</th>
            ...
            <p class="small text-muted px-3 py-2 mb-0 border-top">
                ...
                La diferencia es SUSS − mayor comparable.
            </p>
        </div>
    </div>
    ...
    <script>
    (function () {
        var panel = document.getElementById('suss-conciliacion-panel');
