@php
    $archivosList = isset($data) && $data && $data->requisicion_archivos
        ? $data->requisicion_archivos
        : collect();
    $hashParam = request()->route('hash');
    $ocultarInputsConservar = $ocultarInputsConservar ?? false;
@endphp

@if ($archivosList->count())
    <div class="requisicion-archivos-preview row">
        @foreach ($archivosList as $arch)
            @php
                ...
                $urlDescarga = route('requisicion_archivo', ['id' => $data->id, 'archivo' => $arch->id]);
                if ($qsBase !== []) {
