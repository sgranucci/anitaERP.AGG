...
<script>
$(function () {
    var baseUrl = @json(url('compras/requisicion'));
    ...
    $(document).on('click', '.js-requisicion-comprobantes', function () {
        ...
        $titulo.text('Comprobantes asociados — Requisición ' + num);
        ...
        $.getJSON(baseUrl + '/' + id + '/comprobantes-asociados')
            .done(function (data) {
                ...
                if (filas.length === 0) {
                    $cuerpo.html('<p class="text-muted mb-0">No hay comprobantes asociados por ahora.</p>');
                } else {
                    var html = '<div class="table-responsive"><table class="table table-sm table-striped table-bordered mb-0">';
                    html += '<thead><tr><th>Tipo</th><th>Número</th><th>Fecha</th><th>Estado</th><th class="text-center" style="min-width:9rem">Acciones</th></tr></thead><tbody>';
                    filas.forEach(function (r) {
                        ...
                        if (r.url_ver) {
                            html += '<a href="' + r.url_ver + '" class="btn btn-outline-secondary btn-sm mr-1 mb-1" title="Visualizar" target="_blank" rel="noopener noreferrer"><i class="fa fa-eye"></i></a>';
                        }
                        ...
                        if (!r.url_ver && !r.url_imprimir_vertical) {
