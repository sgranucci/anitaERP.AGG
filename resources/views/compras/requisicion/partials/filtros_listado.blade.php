@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? RequisicionListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'numerorequisicion';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (RequisicionListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = RequisicionListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = RequisicionListadoFiltros::tieneCriteriosTexto($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('consultar_requisicion', RequisicionListadoFiltros::paraQueryStringEmpresa($f));
    $fScope = $f['empresa_scope'] ?? 'una';
    $fEmp = $f['empresa_id'] ?? null;
@endphp
<div class="collapse border-bottom" id="panel-filtros-requisicion" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ RequisicionListadoFiltros::MODO_TODOS }}" {{ $modo === RequisicionListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ RequisicionListadoFiltros::MODO_CAMPO }}" {{ $modo === RequisicionListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== RequisicionListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? RequisicionListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(RequisicionListadoFiltros::operadoresParaCampo($modo === RequisicionListadoFiltros::MODO_CAMPO ? $campoActivo : 'numerorequisicion') as $opKey => $opLabel)
