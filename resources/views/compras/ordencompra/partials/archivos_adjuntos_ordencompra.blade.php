@php
    $archivosList = isset($data) && $data && $data->ordencompra_archivos
        ? $data->ordencompra_archivos
        : collect();
    $hashParam = request()->query('hash');
    $ocultarInputsConservar = $ocultarInputsConservar ?? false;
@endphp

@if ($archivosList->count())
    <div class="ordencompra-archivos-preview row">
        @foreach ($archivosList as $arch)
            @php
                ...
                $urlDescarga = route('ordencompra_archivo', ['id' => $data->id, 'archivo' => $arch->id]);
                if ($qsBase !== []) {
                ...
                $urlInline = \App\Support\Archivos\ArchivoAdjuntoCacheSupport::conVersion(
                    route('ordencompra_archivo', ['id' => $data->id, 'archivo' => $arch->id]).'?'.http_build_query($qsInline),
                    public_path('storage/archivos/ordencompras/'.$data->id.'/'.basename($safeName))
                );
                $esImagen = in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp'], true);
                $esPdf = $ext === 'pdf';
            @endphp
            <div class="col-md-6 col-lg-4 mb-3 ordencompra-archivo-item">
                <div class="card card-outline card-secondary h-100 mb-0">
                    <div class="card-body p-2 d-flex flex-column">
                        ...
                        @if (! $ocultarInputsConservar)
                            ...
                            <button type="button" class="btn btn-sm btn-outline-danger mt-2 eliminar-archivo-ordencompra" title="Quitar de la lista; se elimina al guardar la orden">
