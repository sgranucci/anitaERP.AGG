@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? SuscripcionListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'suscripcion_nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (SuscripcionListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = SuscripcionListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = SuscripcionListadoFiltros::tieneCriteriosTexto($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('consultar_suscripcion', SuscripcionListadoFiltros::paraQueryStringExternos($f));
    $fEmp = $f['empresa_id'] ?? null;
    $fEstado = $f['estado'] ?? '';
@endphp
<div class="collapse border-bottom" id="panel-filtros-suscripcion" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ SuscripcionListadoFiltros::MODO_TODOS }}" {{ $modo === SuscripcionListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ SuscripcionListadoFiltros::MODO_CAMPO }}" {{ $modo === SuscripcionListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== SuscripcionListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach (SuscripcionListadoFiltros::operadoresParaCampo($modo === SuscripcionListadoFiltros::MODO_CAMPO ? $campoActivo : 'suscripcion_nombre') as $opKey => $opLabel)
