@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? ConceptoIvacompraListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (ConceptoIvacompraListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = ConceptoIvacompraListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = ConceptoIvacompraListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('concepto_ivacompra');
@endphp
<div class="collapse border-bottom" id="panel-filtros-concepto-ivacompra" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ ConceptoIvacompraListadoFiltros::MODO_TODOS }}" {{ $modo === ConceptoIvacompraListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ ConceptoIvacompraListadoFiltros::MODO_CAMPO }}" {{ $modo === ConceptoIvacompraListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== ConceptoIvacompraListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach ($camposFiltro ?? ConceptoIvacompraListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach (ConceptoIvacompraListadoFiltros::operadoresParaCampo($modo === ConceptoIvacompraListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
