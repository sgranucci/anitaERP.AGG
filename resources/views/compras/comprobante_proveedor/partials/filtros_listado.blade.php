@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? ComprobanteProveedorListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombreproveedor';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (ComprobanteProveedorListadoFiltros::CAMPOS as $key => $meta) {
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ ComprobanteProveedorListadoFiltros::MODO_TODOS }}" {{ $modo === ComprobanteProveedorListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ ComprobanteProveedorListadoFiltros::MODO_CAMPO }}" {{ $modo === ComprobanteProveedorListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== ComprobanteProveedorListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? ComprobanteProveedorListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(ComprobanteProveedorListadoFiltros::operadoresParaCampo($modo === ComprobanteProveedorListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombreproveedor') as $opKey => $opLabel)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-valor-hasta-wrap" style="{{ ($operadorActivo === 'entre' && ($f['modo'] ?? '') === ComprobanteProveedorListadoFiltros::MODO_CAMPO) ? '' : 'display:none' }}">
