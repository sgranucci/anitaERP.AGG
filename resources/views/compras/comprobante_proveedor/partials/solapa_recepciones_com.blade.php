@php
    ...
    $bloqueado = ($esEdicion ?? false) && ! empty($bloqueado_edicion);
    ...
    // importe_comparacion ya viene en moneda de la factura (no convertir otra vez a pesos).
    ...
    $monedaFacturaNombre = $data->monedas->nombre
        ?? ($esMe ? 'ME#'.$monedaId : 'PESOS');
    ...
    <div class="card card-outline card-info mb-3">
        ...
        <div class="card-body">
            ...
            <p class="small mb-2">
                ...
                en {{ $monedaFacturaNombre }}:
                <strong>{{ number_format($importeRef, 2, ',', '.') }}</strong>
            </p>
            @endif
            ...
            @if ($bloqueado)
                <ul class="list-unstyled mb-0">
                    @foreach ($data->comprobante_proveedor_recepciones ?? [] as $vinculo)
                        @php
                            $recepcionBloqueada = $vinculo->recepcion_proveedores;
                            $numeroBloqueado = $recepcionBloqueada->numerorecepcion ?? null;
                            $etiquetaBloqueada = filled($numeroBloqueado)
                                ? 'Nº '.$numeroBloqueado.' (ID '.$vinculo->recepcion_proveedor_id.')'
                                : 'Recepción #'.$vinculo->recepcion_proveedor_id;
                        @endphp
                        <li>
                            <small>{{ $etiquetaBloqueada }}</small>
                    ...
                    <tbody>
                        @forelse ($mostrarIds as $recepcionId)
                            @php
                                ...
                                $importeComFactura = (float) ($recepcion->importe_provision_com_factura ?? $importeComMe);
                                ...
                                $coincide = $importeRef > 0 && abs($importeComFactura - $importeRef) <= 0.05;
                            ...
                            <tr class="cp-com-fila {{ $coincide ? 'table-success' : '' }}"
                                ...
                                <td class="align-middle"><strong>{{ $recepcion->numerorecepcion ?? '' }}</strong></td>
                                ...
                                <td class="text-right align-middle">
                                    @if ($monedaComId !== $monedaId && abs($importeComFactura - $importeComMe) > 0.05)
                                        <small>
                                            {{ number_format($importeComMe, 2, ',', '.') }} {{ $monedaComNombre }}
                                            <br><strong>{{ number_format($importeComFactura, 2, ',', '.') }} {{ $monedaFacturaNombre }}</strong>
                                        </small>
                                    @else
                                        <small><strong>{{ number_format($importeComFactura, 2, ',', '.') }}</strong></small>
                                    @endif
                                </td>
                                <td class="text-right align-middle">
                                ...
                                <td class="text-center align-middle">
                                    ...
                                    <a href="{{ route('editar_recepcion_proveedor', ['id' => $recepcion->id, 'origen' => 'modal_consulta', 'vista' => 'consulta']) }}"
                                       ...
                                       title="Ver recepción Nº {{ $recepcion->numerorecepcion ?? $recepcion->id }}"
                                       target="_blank"
                ...
                <div id="cp-com-articulos-wrap">
                    @foreach ($mostrarIds as $recepcionId)
                        @php
                            ...
                            $etiquetaArticulos = filled($recepcion?->numerorecepcion ?? null)
                                ? 'COM Nº '.$recepcion->numerorecepcion.' (ID '.$recepcion->id.')'
                                : 'COM #'.($recepcion->id ?? $recepcionId);
                        @endphp
                        @if ($recepcion)
