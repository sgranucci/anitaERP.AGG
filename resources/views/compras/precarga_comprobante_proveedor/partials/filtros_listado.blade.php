@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? PrecargaComprobanteProveedorListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombreproveedor';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (PrecargaComprobanteProveedorListadoFiltros::CAMPOS as $key => $meta) {
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ PrecargaComprobanteProveedorListadoFiltros::MODO_TODOS }}" {{ $modo === PrecargaComprobanteProveedorListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ PrecargaComprobanteProveedorListadoFiltros::MODO_CAMPO }}" {{ $modo === PrecargaComprobanteProveedorListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== PrecargaComprobanteProveedorListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? PrecargaComprobanteProveedorListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(PrecargaComprobanteProveedorListadoFiltros::operadoresParaCampo($modo === PrecargaComprobanteProveedorListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombreproveedor') as $opKey => $opLabel)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-valor-hasta-wrap" style="{{ ($operadorActivo === 'entre' && ($f['modo'] ?? '') === PrecargaComprobanteProveedorListadoFiltros::MODO_CAMPO) ? '' : 'display:none' }}">
