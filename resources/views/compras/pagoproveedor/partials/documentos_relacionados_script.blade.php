@once
<script>
$(function () {
    var urlTpl = @json(route('pagoproveedor_documentos_relacionados', ['id' => '__ID__']));
    var $modal = $('#modalPagoDocumentosRelacionados');
    var $titulo = $('#modalPagoDocumentosRelacionadosTitulo');
    var $cuerpo = $('#pagoDocumentosRelacionadosCuerpo');

    function esc(s) {
        return $('<div>').text(s == null ? '' : String(s)).html();
    }

    function btnGrupo(doc, labelCorto) {
        if (!doc) {
            return '<span class="text-muted small">—</span>';
        }
        var html = '<div class="d-inline-flex flex-wrap align-items-center">';
        html += '<span class="mr-1 small font-weight-bold">' + esc(labelCorto || doc.etiqueta || doc.numero || '') + '</span>';
        if (doc.url_ver) {
            html += '<a href="' + doc.url_ver + '" class="btn btn-outline-secondary btn-sm mr-1 mb-1" title="Consultar" target="_blank" rel="noopener noreferrer"><i class="fa fa-eye"></i></a>';
        }
        if (doc.url_pdf) {
            html += '<a href="' + doc.url_pdf + '" class="btn btn-outline-danger btn-sm mr-1 mb-1" title="PDF" target="_blank" rel="noopener noreferrer"><i class="fa fa-file-pdf-o"></i></a>';
        }
        if (doc.url_pdf_apaisado) {
            html += '<a href="' + doc.url_pdf_apaisado + '" class="btn btn-outline-secondary btn-sm mb-1" title="PDF apaisado" target="_blank" rel="noopener noreferrer"><i class="fa fa-arrows-alt-h"></i></a>';
        }
        if (!doc.url_ver && !doc.url_pdf) {
            html += '<span class="text-muted small">Sin permiso</span>';
        }
        html += '</div>';
        return html;
    }

    function celdaComs(coms) {
        if (!coms || !coms.length) {
            return '<span class="text-muted small">—</span>';
        }
        var html = '';
        coms.forEach(function (c, idx) {
            if (idx > 0) {
                html += '<hr class="my-1">';
            }
            html += btnGrupo(c, 'COM ' + (c.numero || ''));
        });
        return html;
    }

    $(document).on('click', '.js-op-documentos-relacionados', function () {
        var id = $(this).data('id');
        var num = $(this).data('numero') || '';
        $titulo.text('Documentos relacionados — ' + (num || ('OP #' + id)));
        $cuerpo.html('<p class="text-muted mb-0">Cargando…</p>');
        $modal.modal('show');

        $.getJSON(urlTpl.replace('__ID__', encodeURIComponent(id)))
            .done(function (data) {
                var filas = data.filas || [];
                if (filas.length === 0) {
                    $cuerpo.html('<p class="text-muted mb-0">Esta OP no tiene comprobantes aplicados.</p>');
                    return;
                }
                var html = '<div class="table-responsive"><table class="table table-sm table-striped table-bordered mb-0">';
                html += '<thead><tr>';
                html += '<th>Factura / comprobante</th>';
                html += '<th class="text-right">Aplicado</th>';
                html += '<th>Orden de compra</th>';
                html += '<th>Requisición</th>';
                html += '<th>Recepción COM</th>';
                html += '</tr></thead><tbody>';
                filas.forEach(function (r) {
                    html += '<tr>';
                    html += '<td>';
                    html += '<div class="small mb-1">' + esc(r.etiqueta || '') + (r.fecha ? ' <span class="text-muted">(' + esc(r.fecha) + ')</span>' : '') + '</div>';
                    if (r.factura) {
                        html += btnGrupo(r.factura, '');
                    } else if (r.es_opa) {
                        html += '<span class="badge badge-secondary">OPA / anticipo</span>';
                    } else {
                        html += '<span class="text-muted small">Sin factura CxP</span>';
                    }
                    html += '</td>';
                    html += '<td class="text-right text-nowrap"><small>' + esc(r.monto_aplicado_fmt || '') + '</small></td>';
                    html += '<td>' + (r.ordencompra ? btnGrupo(r.ordencompra, 'OC ' + (r.ordencompra.numero || '')) : '<span class="text-muted small">—</span>') + '</td>';
                    html += '<td>' + (r.requisicion ? btnGrupo(r.requisicion, 'REQ ' + (r.requisicion.numero || '')) : '<span class="text-muted small">—</span>') + '</td>';
                    html += '<td>' + celdaComs(r.coms) + '</td>';
                    html += '</tr>';
                });
                html += '</tbody></table></div>';
                html += '<p class="text-muted small mb-0 mt-2">Ojo / PDF abren consulta o el archivo en una pestaña nueva.</p>';
                $cuerpo.html(html);
            })
            .fail(function (xhr) {
                var msg = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : 'No se pudo cargar la información.';
                $cuerpo.html('<p class="text-danger mb-0">' + esc(msg) + '</p>');
            });
    });
});
</script>
@endonce
