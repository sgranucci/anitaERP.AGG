@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? ListaprecioProveedorListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (ListaprecioProveedorListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = ListaprecioProveedorListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = ListaprecioProveedorListadoFiltros::tieneCriteriosTexto($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('consultar_listaprecio_proveedor', [
        'estado' => $f['estado'] ?? null,
    ]);
    $estadoPersist = $f['estado'] ?? '';
@endphp
<div class="collapse border-bottom" id="panel-filtros-listaprecio-proveedor" data-listado-filtros-panel>
    <input type="hidden" name="filtro_busqueda_rapida" id="filtro_busqueda_rapida" value="">
    @if ($estadoPersist !== '')
        <input type="hidden" name="estado" value="{{ $estadoPersist }}">
    @endif
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ ListaprecioProveedorListadoFiltros::MODO_TODOS }}" {{ $modo === ListaprecioProveedorListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ ListaprecioProveedorListadoFiltros::MODO_CAMPO }}" {{ $modo === ListaprecioProveedorListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== ListaprecioProveedorListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach ($camposFiltro ?? ListaprecioProveedorListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach (ListaprecioProveedorListadoFiltros::operadoresParaCampo($modo === ListaprecioProveedorListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
