            			...
            			<tr class="item-archivo">
                			<td>
								<input type="file" name="nombrearchivos[]" class="form-control nombrearchivos" 
									...
									data-initial-preview="{{isset($archivo->nombrearchivo) ? \App\Support\Archivos\ArchivoAdjuntoCacheSupport::urlStoragePublico('archivos/tickets/'.$data->id.'/'.$archivo->nombrearchivo) : ''}}" \>
                			...
                			<td>
								@if ($archivo->nombrearchivo ?? '')
                                	...
                                	@else
										<img heigh=100px width=100px src="{{ \App\Support\Archivos\ArchivoAdjuntoCacheSupport::urlStoragePublico('archivos/tickets/'.$data->id.'/'.$archivo->nombrearchivo) }}" alt="image">
									@endif
                			...
                			<td>
								@if ($archivo->nombrearchivo ?? '')
									<a download="{{$archivo->nombrearchivo}}" href="{{ \App\Support\Archivos\ArchivoAdjuntoCacheSupport::urlStoragePublico('archivos/tickets/'.$data->id.'/'.$archivo->nombrearchivo) }}" title='Descargar' /><i class="fa fa-download"></i>
