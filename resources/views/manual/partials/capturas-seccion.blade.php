@php
    $configKey = $configKey ?? 'manual_gastronomia';
    $imgPublicPrefix = $imgPublicPrefix ?? 'docs/manual-gastronomia/img';
    $capturasCfg = config($configKey . '.capturas', []);
    $ids = [];
    ...
    $imgDir = public_path($imgPublicPrefix);
@endphp
@foreach ($ids as $capKey)
    ...
    <figure class="mc-figure">
        <img src="{{ asset($imgPublicPrefix . '/' . $imgFile) }}" alt="{{ $cap['titulo'] }}" loading="lazy">
