    ...
    <div class="card-body">
    	<table class="table" id="archivo-table">
    		...
    		<tbody id="tbody-tabla-archivo">
		 		@if ($data->cobranza_archivos ?? '') 
					@foreach (old('archivos', $data->cobranza_archivos->count() ? $data->cobranza_archivos : ['']) as $archivo)
            			<tr class="item-archivo">
                			<td>
								<input type="file" name="nombrearchivos[]" class="form-control nombrearchivos" 
									onchange='actualizaArchivo(this)'
									data-initial-preview="{{isset($archivo->nombrearchivo) ? asset("storage/archivos/cobranzas/$data->id/$archivo->nombrearchivo") : ''}}" \>
                			...
                			<td>
								@if ($archivo->nombrearchivo ?? '')
                                	...
                                	@else
										<img heigh=100px width=100px src="{{ asset("storage/archivos/cobranzas/$data->id/$archivo->nombrearchivo") }}" alt="image">
									@endif
                			...
                			<td>
								@if ($archivo->nombrearchivo ?? '')
									<a download="{{$archivo->nombrearchivo}}" href="{{ asset("storage/archivos/cobranzas/$data->id/$archivo->nombrearchivo") }}" title='Descargar' /><i class="fa fa-download"></i>
