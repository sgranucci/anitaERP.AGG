@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? BingoCartonListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'codigo';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (BingoCartonListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = BingoCartonListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = BingoCartonListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('bingo_carton');
@endphp
<div class="collapse border-bottom" id="panel-filtros-bingo-carton" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ BingoCartonListadoFiltros::MODO_TODOS }}" {{ $modo === BingoCartonListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ BingoCartonListadoFiltros::MODO_CAMPO }}" {{ $modo === BingoCartonListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== BingoCartonListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? BingoCartonListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(BingoCartonListadoFiltros::operadoresParaCampo($modo === BingoCartonListadoFiltros::MODO_CAMPO ? $campoActivo : 'codigo') as $opKey => $opLabel)
