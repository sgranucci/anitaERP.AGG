@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? BingoConceptoRendicionListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'codigo';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (BingoConceptoRendicionListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = BingoConceptoRendicionListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = BingoConceptoRendicionListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('bingo_concepto_rendicion');
@endphp
<div class="collapse border-bottom" id="panel-filtros-bingo-concepto-rendicion" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ BingoConceptoRendicionListadoFiltros::MODO_TODOS }}" {{ $modo === BingoConceptoRendicionListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ BingoConceptoRendicionListadoFiltros::MODO_CAMPO }}" {{ $modo === BingoConceptoRendicionListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== BingoConceptoRendicionListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? BingoConceptoRendicionListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(BingoConceptoRendicionListadoFiltros::operadoresParaCampo($modo === BingoConceptoRendicionListadoFiltros::MODO_CAMPO ? $campoActivo : 'codigo') as $opKey => $opLabel)
