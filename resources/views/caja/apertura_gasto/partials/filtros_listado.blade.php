@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? AperturaGastoListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'codigo';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (AperturaGastoListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = AperturaGastoListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = AperturaGastoListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('apertura_gasto');
@endphp
<div class="collapse border-bottom" id="panel-filtros-apertura-gasto" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ AperturaGastoListadoFiltros::MODO_TODOS }}" {{ $modo === AperturaGastoListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ AperturaGastoListadoFiltros::MODO_CAMPO }}" {{ $modo === AperturaGastoListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== AperturaGastoListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? AperturaGastoListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(AperturaGastoListadoFiltros::operadoresParaCampo($modo === AperturaGastoListadoFiltros::MODO_CAMPO ? $campoActivo : 'codigo') as $opKey => $opLabel)
