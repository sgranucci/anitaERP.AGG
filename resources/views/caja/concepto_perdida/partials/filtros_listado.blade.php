@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? ConceptoPerdidaListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'codigo';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (ConceptoPerdidaListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = ConceptoPerdidaListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = ConceptoPerdidaListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('concepto_perdida');
@endphp
<div class="collapse border-bottom" id="panel-filtros-concepto-perdida" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ ConceptoPerdidaListadoFiltros::MODO_TODOS }}" {{ $modo === ConceptoPerdidaListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ ConceptoPerdidaListadoFiltros::MODO_CAMPO }}" {{ $modo === ConceptoPerdidaListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== ConceptoPerdidaListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? ConceptoPerdidaListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(ConceptoPerdidaListadoFiltros::operadoresParaCampo($modo === ConceptoPerdidaListadoFiltros::MODO_CAMPO ? $campoActivo : 'codigo') as $opKey => $opLabel)
