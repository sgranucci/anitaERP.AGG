@php
    $archivosList = isset($data) && $data && ($data->caja_movimiento_archivos ?? null)
        ? $data->caja_movimiento_archivos
        : collect();
    $ocultarInputsConservar = $ocultarInputsConservar ?? false;
    $movimientoId = (int) ($data->id ?? 0);
@endphp

@if ($archivosList->count())
    <div class="ie-archivos-preview row">
        @foreach ($archivosList as $arch)
            @php
                $safeName = $arch->nombrearchivo;
                $ext = strtolower(pathinfo($safeName, PATHINFO_EXTENSION));
                $urlInline = \App\Support\Archivos\ArchivoAdjuntoCacheSupport::urlStoragePublico('archivos/caja_movimientos/'.$movimientoId.'/'.$safeName);
                $urlDescarga = $urlInline;
                $esImagen = in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp'], true);
                $esPdf = $ext === 'pdf';
            @endphp
            <div class="col-md-6 col-lg-4 mb-3 ie-archivo-item">
                <div class="card card-outline card-secondary h-100 mb-0">
                    <div class="card-body p-2 d-flex flex-column">
                        ...
                        @if (! $ocultarInputsConservar)
                            ...
                            <button type="button" class="btn btn-sm btn-outline-danger mt-2 eliminar-archivo-ie" title="Quitar de la lista; se elimina al guardar">
