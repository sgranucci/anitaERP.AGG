...
<head>
    ...
    <title>Facturas estacionamiento del día</title>
    <style>
    ...
    <table class="listado-header">
        <tr>
            ...
            <td style="width: 40%; text-align: center;">
                <h2 style="margin: 0; font-size: 18px; font-weight: bold;">Facturas estacionamiento del día</h2>
                <div class="meta">
        ...
        <tbody>
            @foreach ($registros as $r)
                ...
                <tr>
                    ...
                    <td>{{ $v?->codigo ?? '—' }}</td>
                    <td>{{ $v ? \App\Support\Caja\Estacionamiento\EstacionamientoVentaDisplaySupport::nombreReceptorFactura($v) : '—' }}</td>
                    <td>{{ $r->cuenta?->item?->nombre ?? '—' }}</td>
                    <td>{{ $pvTxt !== '' ? $pvTxt : '—' }}</td>
                    <td class="num">{{ number_format((float) ($v?->total ?? 0), 2, ',', '.') }}</td>
                    <td>{{ $r->cuenta_estacionamiento_id ?? '—' }}</td>
