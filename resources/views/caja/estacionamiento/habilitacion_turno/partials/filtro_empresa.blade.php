...
<div class="alert alert-info py-2 small mb-3">
    ...
    <strong>configuración de punto de venta estacionamiento</strong> para la terminal
...
@if (collect($empresa_query ?? [])->isEmpty())
    <div class="alert alert-warning mb-3">
        Ninguna de las empresas asignadas a su usuario tiene punto de venta estacionamiento configurado
        para la terminal <code>{{ $identificador_pc !== '' ? $identificador_pc : '(vacío)' }}</code>.
        @php
            $configsPv = collect($configs_pv_asignadas ?? []);
        @endphp
        @if ($configsPv->isNotEmpty())
            <br><br>
            <strong>Configuraciones PV existentes para sus empresas:</strong>
            <ul class="mb-0 mt-1">
            ...
            <small class="d-block mt-2 text-muted">
                El identificador de esta sesión debe coincidir con <code>identificador_pc</code> de la fila PV.
                Use la IP de la caja en el ABM y <code>ESTACIONAMIENTO_IDENTIFICADOR_USAR_IP_CLIENTE=true</code>,
                o fije <code>ESTACIONAMIENTO_IDENTIFICADOR_PC</code> en <code>.env</code> con el mismo valor.
            </small>
        @else
            Revise la configuración en
            <strong>Caja → Estacionamiento → Configuración punto de venta</strong>.
        @endif
    </div>
@else
    <form method="get"
          action="{{ route('estacionamiento_habilitacion_turno') }}"
