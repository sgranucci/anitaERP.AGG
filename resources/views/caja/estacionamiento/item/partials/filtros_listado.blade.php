@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? ItemEstacionamientoListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (ItemEstacionamientoListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = ItemEstacionamientoListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = ItemEstacionamientoListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('estacionamiento_item');
@endphp
<div class="collapse border-bottom" id="panel-filtros-estacionamiento-item" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ ItemEstacionamientoListadoFiltros::MODO_TODOS }}" {{ $modo === ItemEstacionamientoListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ ItemEstacionamientoListadoFiltros::MODO_CAMPO }}" {{ $modo === ItemEstacionamientoListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== ItemEstacionamientoListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? ItemEstacionamientoListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(ItemEstacionamientoListadoFiltros::operadoresParaCampo($modo === ItemEstacionamientoListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
