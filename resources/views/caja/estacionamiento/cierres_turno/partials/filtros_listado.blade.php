@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? EstacionamientoCierresTurnoListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'referencia';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (EstacionamientoCierresTurnoListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = EstacionamientoCierresTurnoListadoFiltros::operadoresParaCampo($key);
    }
@endphp
        ...
        <div class="form-row align-items-end">
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ EstacionamientoCierresTurnoListadoFiltros::MODO_TODOS }}" {{ $modo === EstacionamientoCierresTurnoListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ EstacionamientoCierresTurnoListadoFiltros::MODO_CAMPO }}" {{ $modo === EstacionamientoCierresTurnoListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== EstacionamientoCierresTurnoListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? EstacionamientoCierresTurnoListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(EstacionamientoCierresTurnoListadoFiltros::operadoresParaCampo($campoActivo) as $opKey => $opLabel)
