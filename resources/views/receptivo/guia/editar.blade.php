    ...
    <div class="col-lg-12">
        ...
        <div class="card card-danger">
            <div class="card-header">
                <h3 class="card-title">Editar Guía - Id {{$guia->id}}</h3>
                <div class="card-tools">
                    <a href="{{route('guia')}}" class="btn btn-outline-info btn-sm">
                        <i class="fa fa-fw fa-reply-all"></i> Volver al listado
                    </a>
                </div>
            </div>
            <form action="{{route('actualizar_guia', ['id' => $guia->id])}}" id="form-general" class="form-horizontal form--label-right" method="POST" autocomplete="off">
                @csrf @method("put")
                <div class="card-body">
                    @include('receptivo.guia.form')
                </div>
