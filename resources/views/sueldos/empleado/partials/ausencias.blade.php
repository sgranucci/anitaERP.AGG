...
@endphp
<style>
    #ausencias-panel .ausencias-toggle { cursor: pointer; }
    #ausencias-panel .ausencias-toggle .fa-chevron-up { transition: transform .15s ease-in-out; }
    #ausencias-panel .ausencias-toggle.collapsed .fa-chevron-up { transform: rotate(180deg); }
    #ausencias-panel .ausencias-scroll { overflow-y: auto; }
    #ausencias-panel .ausencias-scroll thead th { position: sticky; top: 0; z-index: 2; }
    #ausencias-panel .small-box { margin-bottom: 0; }
    #ausencias-panel .small-box .inner { padding: 8px 10px; }
    #ausencias-panel .small-box .inner h3 { font-size: 1.5rem; margin: 0 0 2px; }
    #ausencias-panel .small-box .inner p { margin: 0; font-size: .8rem; }
    #ausencias-panel .small-box .icon { font-size: 2rem; top: 6px; right: 8px; }
</style>
<div id="ausencias-panel"
    ...
    <div class="card card-outline card-info mb-3">
        <div class="card-header py-2 d-flex align-items-center justify-content-between ausencias-toggle"
             data-toggle="collapse" data-target="#ausencias-seccion-saldos" role="button" aria-expanded="true">
            <h6 class="card-title mb-0">
                <i class="fa fa-calculator"></i> Saldos
                <span class="badge badge-info ml-1">Saldo {{ number_format($resumen['saldo'], 2, ',', '.') }}</span>
                <span class="badge badge-success ml-1">Deveng. {{ number_format($resumen['devengado'], 2, ',', '.') }}</span>
                <span class="badge badge-warning ml-1">Consum. {{ number_format($resumen['consumido'], 2, ',', '.') }}</span>
            </h6>
            <i class="fa fa-chevron-up text-muted"></i>
        </div>
        <div id="ausencias-seccion-saldos" class="collapse show">
            <div class="card-body">
                ...
                <div class="table-responsive ausencias-scroll mb-0" style="max-height: 190px;">
    ...
    <div class="card card-outline card-info mb-3">
        <div class="card-header py-2 d-flex align-items-center justify-content-between ausencias-toggle"
             data-toggle="collapse" data-target="#ausencias-seccion-grilla" role="button" aria-expanded="true">
            <h6 class="card-title mb-0">
        ...
        <div id="ausencias-seccion-grilla" class="collapse show">
            <div class="card-body p-0">
                <div class="table-responsive ausencias-scroll mb-0" style="max-height: 320px;">
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-3">
                                        Sin eventos. Abrí <strong>Registrar ausencia</strong>, completá el formulario y pulsá
                                        <strong>Guardar ausencia</strong>; aparece acá.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    @if ($puedeEditar)
        <div class="card card-outline card-primary mb-0">
            <div class="card-header py-2 d-flex align-items-center justify-content-between ausencias-toggle collapsed"
                 data-toggle="collapse" data-target="#ausencias-seccion-form" role="button" aria-expanded="false">
                <h6 class="card-title mb-0" id="ausencia-form-titulo">Registrar ausencia</h6>
                <i class="fa fa-chevron-up text-muted"></i>
            </div>
            <div id="ausencias-seccion-form" class="collapse">
                <div class="card-body">
