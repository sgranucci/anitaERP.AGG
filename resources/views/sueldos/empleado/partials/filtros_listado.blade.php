@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? EmpleadoSueldosListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (EmpleadoSueldosListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = EmpleadoSueldosListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = EmpleadoSueldosListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('consultar_empleado_sueldos');
@endphp
@php
    $fEstado = $f['estado'] ?? 'A';
...
@endphp
<div class="collapse border-bottom" id="panel-filtros-empleado-sueldos" data-listado-filtros-panel>
    <input type="hidden" name="filtro_busqueda_rapida" id="filtro_busqueda_rapida" value="">
    {{-- Persistencia de los filtros externos (estado / empresa) al buscar por texto o aplicar el panel --}}
    @if ($fScope === 'todas')
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ EmpleadoSueldosListadoFiltros::MODO_TODOS }}" {{ $modo === EmpleadoSueldosListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ EmpleadoSueldosListadoFiltros::MODO_CAMPO }}" {{ $modo === EmpleadoSueldosListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== EmpleadoSueldosListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? EmpleadoSueldosListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(EmpleadoSueldosListadoFiltros::operadoresParaCampo($modo === EmpleadoSueldosListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <input type="text" id="filtro_valor_panel" class="form-control form-control-sm"
                       value="{{ $f['valor'] ?? '' }}" placeholder="Texto o número" autocomplete="off">
            </div>
