@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? PrendaSueldosListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'descripcion';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (PrendaSueldosListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = PrendaSueldosListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = PrendaSueldosListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('consultar_prenda_sueldos');
@endphp
<div class="collapse border-bottom" id="panel-filtros-prenda-sueldos" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ PrendaSueldosListadoFiltros::MODO_TODOS }}" {{ $modo === PrendaSueldosListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ PrendaSueldosListadoFiltros::MODO_CAMPO }}" {{ $modo === PrendaSueldosListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== PrendaSueldosListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? PrendaSueldosListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(PrendaSueldosListadoFiltros::operadoresParaCampo($modo === PrendaSueldosListadoFiltros::MODO_CAMPO ? $campoActivo : 'descripcion') as $opKey => $opLabel)
