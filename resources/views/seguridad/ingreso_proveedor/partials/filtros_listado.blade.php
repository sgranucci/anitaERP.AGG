@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? IngresoProveedorListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'proveedor';
...
@endphp
<div class="collapse border-bottom" id="panel-filtros-ingreso-proveedor" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ IngresoProveedorListadoFiltros::MODO_TODOS }}" {{ $modo === IngresoProveedorListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ IngresoProveedorListadoFiltros::MODO_CAMPO }}" {{ $modo === IngresoProveedorListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== IngresoProveedorListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? IngresoProveedorListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(IngresoProveedorListadoFiltros::operadoresParaCampo($modo === IngresoProveedorListadoFiltros::MODO_CAMPO ? $campoActivo : 'proveedor') as $opKey => $opLabel)
