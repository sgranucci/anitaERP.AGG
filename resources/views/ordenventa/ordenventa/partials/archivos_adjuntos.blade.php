@php
    $archivosList = isset($data) && $data && ($data->ordenventa_archivos ?? null)
        ? $data->ordenventa_archivos
        : collect();
    $ocultarInputsConservar = $ocultarInputsConservar ?? false;
@endphp

@if ($archivosList->count())
    <div class="ordenventa-archivos-preview row">
        @foreach ($archivosList as $arch)
            @php
                $safeName = $arch->nombrearchivo;
                $ext = strtolower(pathinfo($safeName, PATHINFO_EXTENSION));
                $urlInline = \App\Support\Archivos\ArchivoAdjuntoCacheSupport::urlStoragePublico('archivos/ordenventas/'.$data->id.'/'.$safeName);
                $urlDescarga = $urlInline;
                $esImagen = in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp'], true);
                $esPdf = $ext === 'pdf';
            @endphp
            <div class="col-md-6 col-lg-4 mb-3 ordenventa-archivo-item">
                <div class="card card-outline card-secondary h-100 mb-0">
                    <div class="card-body p-2 d-flex flex-column">
                        ...
                        @if (! $ocultarInputsConservar)
                            ...
                            <button type="button" class="btn btn-sm btn-outline-danger mt-2 eliminar-archivo-ordenventa" title="Quitar de la lista; se elimina de la orden de venta al guardar">
