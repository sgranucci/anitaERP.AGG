@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? ProvinciaListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (ProvinciaListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = ProvinciaListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = ProvinciaListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('provincia');
@endphp
<div class="collapse border-bottom" id="panel-filtros-provincia" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ ProvinciaListadoFiltros::MODO_TODOS }}" {{ $modo === ProvinciaListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ ProvinciaListadoFiltros::MODO_CAMPO }}" {{ $modo === ProvinciaListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== ProvinciaListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? ProvinciaListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(ProvinciaListadoFiltros::operadoresParaCampo($modo === ProvinciaListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
            ...
            <div class="form-group col-md-3 col-sm-6 mb-2">
                ...
                <input type="text"
                       ...
                       value="{{ $f['valor'] ?? '' }}"
                       placeholder="Nombre, código o jurisdicción"
                       autocomplete="off">
            </div>
