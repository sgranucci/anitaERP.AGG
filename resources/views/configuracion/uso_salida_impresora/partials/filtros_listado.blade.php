@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? UsoSalidaImpresoraListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (UsoSalidaImpresoraListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = UsoSalidaImpresoraListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = UsoSalidaImpresoraListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('uso_salida_impresora');
@endphp
<div class="collapse border-bottom" id="panel-filtros-uso-salida-impresora" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ UsoSalidaImpresoraListadoFiltros::MODO_TODOS }}" {{ $modo === UsoSalidaImpresoraListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ UsoSalidaImpresoraListadoFiltros::MODO_CAMPO }}" {{ $modo === UsoSalidaImpresoraListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== UsoSalidaImpresoraListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? UsoSalidaImpresoraListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(UsoSalidaImpresoraListadoFiltros::operadoresParaCampo($modo === UsoSalidaImpresoraListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
