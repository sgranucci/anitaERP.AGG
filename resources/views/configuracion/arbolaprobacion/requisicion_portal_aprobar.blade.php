...
@section('titulo_pagina', 'Aprobar requisición '.$requisicion->numerorequisicion)

@section('content')
@include('configuracion.arbolaprobacion.partials.requisicion_portal_resumen', ['modoPortal' => 'aprobar'])

<div class="card card-success portal-card">
    <div class="card-header">
        <h2 class="card-title mb-0 h6">Confirmar aprobación</h2>
    </div>
    <form action="{{ route('aprobar_requisicion_externo') }}" method="post" class="card-body">
        @csrf
        <input type="hidden" name="comprobante_id" value="{{ $requisicion->id }}">
        ...
        <button type="submit" class="btn btn-success btn-block btn-lg">Aprobar requisición</button>
    </form>
</div>
@endsection
