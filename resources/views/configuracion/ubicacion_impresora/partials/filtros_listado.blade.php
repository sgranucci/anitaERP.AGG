@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? UbicacionImpresoraListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (UbicacionImpresoraListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = UbicacionImpresoraListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = UbicacionImpresoraListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('ubicacion_impresora');
@endphp
<div class="collapse border-bottom" id="panel-filtros-ubicacion-impresora" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ UbicacionImpresoraListadoFiltros::MODO_TODOS }}" {{ $modo === UbicacionImpresoraListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ UbicacionImpresoraListadoFiltros::MODO_CAMPO }}" {{ $modo === UbicacionImpresoraListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== UbicacionImpresoraListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? UbicacionImpresoraListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(UbicacionImpresoraListadoFiltros::operadoresParaCampo($modo === UbicacionImpresoraListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
