@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? OrdentrabajoListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'cliente';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (OrdentrabajoListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = OrdentrabajoListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = OrdentrabajoListadoFiltros::tieneCriteriosTexto($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('ordentrabajo');
@endphp
<div class="collapse border-bottom" id="panel-filtros-ordentrabajo" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ OrdentrabajoListadoFiltros::MODO_TODOS }}" {{ $modo === OrdentrabajoListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ OrdentrabajoListadoFiltros::MODO_CAMPO }}" {{ $modo === OrdentrabajoListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== OrdentrabajoListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach ($camposFiltro ?? OrdentrabajoListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach (OrdentrabajoListadoFiltros::operadoresParaCampo($modo === OrdentrabajoListadoFiltros::MODO_CAMPO ? $campoActivo : 'cliente') as $opKey => $opLabel)
