{{--
    ...
    y desde el detalle (ver comprobante). Requiere que se incluya en la misma
    vista el partial `partials/modal_generar_nc.blade.php`.

    Botones disparadores: cualquier elemento con clase `.js-fd-generar-nc`
    y atributos `data-venta-id` (obligatorio) y `data-codigo` (opcional).

    Durante el procesamiento muestra:
      - Spinner + texto «Procesando…» en el botón Confirmar.
      - Overlay fullscreen (`#fd-nc-procesando-overlay`) como aviso visible
        en móvil y desktop, ya que la NC es irrevertible.
      - Bloqueo del cierre del modal (backdrop / X / Escape / Cancelar).
--}}
<script>
(function () {
