@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? PedidoInterformingListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'codigo';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (PedidoInterformingListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = PedidoInterformingListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = PedidoInterformingListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('pedido');
@endphp
<div class="collapse border-bottom" id="panel-filtros-pedido-interforming" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ PedidoInterformingListadoFiltros::MODO_TODOS }}" {{ $modo === PedidoInterformingListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ PedidoInterformingListadoFiltros::MODO_CAMPO }}" {{ $modo === PedidoInterformingListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== PedidoInterformingListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? PedidoInterformingListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(PedidoInterformingListadoFiltros::operadoresParaCampo($modo === PedidoInterformingListadoFiltros::MODO_CAMPO ? $campoActivo : 'codigo') as $opKey => $opLabel)
