@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? ConceptoVentaListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (ConceptoVentaListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = ConceptoVentaListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = ConceptoVentaListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('concepto_venta');
@endphp
<div class="collapse border-bottom" id="panel-filtros-concepto-venta" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ ConceptoVentaListadoFiltros::MODO_TODOS }}" {{ $modo === ConceptoVentaListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ ConceptoVentaListadoFiltros::MODO_CAMPO }}" {{ $modo === ConceptoVentaListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== ConceptoVentaListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? ConceptoVentaListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(ConceptoVentaListadoFiltros::operadoresParaCampo($modo === ConceptoVentaListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
