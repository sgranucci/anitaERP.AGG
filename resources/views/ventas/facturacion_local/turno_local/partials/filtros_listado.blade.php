@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? TurnoLocalListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (TurnoLocalListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = TurnoLocalListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = TurnoLocalListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('facturacion_local_turno');
@endphp
<div class="collapse border-bottom" id="panel-filtros-turno-local" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ TurnoLocalListadoFiltros::MODO_TODOS }}" {{ $modo === TurnoLocalListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ TurnoLocalListadoFiltros::MODO_CAMPO }}" {{ $modo === TurnoLocalListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== TurnoLocalListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach ($camposFiltro ?? TurnoLocalListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach (TurnoLocalListadoFiltros::operadoresParaCampo($modo === TurnoLocalListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
