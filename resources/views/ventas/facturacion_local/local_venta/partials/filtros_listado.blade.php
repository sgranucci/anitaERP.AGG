@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? LocalVentaListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (LocalVentaListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = LocalVentaListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = LocalVentaListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('facturacion_local_locales');
@endphp
<div class="collapse border-bottom" id="panel-filtros-local-venta" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ LocalVentaListadoFiltros::MODO_TODOS }}" {{ $modo === LocalVentaListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ LocalVentaListadoFiltros::MODO_CAMPO }}" {{ $modo === LocalVentaListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== LocalVentaListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach ($camposFiltro ?? LocalVentaListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach (LocalVentaListadoFiltros::operadoresParaCampo($modo === LocalVentaListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
