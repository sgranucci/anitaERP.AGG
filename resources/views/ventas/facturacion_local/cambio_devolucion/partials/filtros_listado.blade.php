@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? CambioDevolucionMarketplaceListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'numero';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (CambioDevolucionMarketplaceListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = CambioDevolucionMarketplaceListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = CambioDevolucionMarketplaceListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('facturacion_local_cambios_devolucion');
@endphp
<div class="collapse border-bottom" id="panel-filtros-cdm" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ CambioDevolucionMarketplaceListadoFiltros::MODO_TODOS }}" {{ $modo === CambioDevolucionMarketplaceListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ CambioDevolucionMarketplaceListadoFiltros::MODO_CAMPO }}" {{ $modo === CambioDevolucionMarketplaceListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== CambioDevolucionMarketplaceListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach ($camposFiltro ?? CambioDevolucionMarketplaceListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach (CambioDevolucionMarketplaceListadoFiltros::operadoresParaCampo($modo === CambioDevolucionMarketplaceListadoFiltros::MODO_CAMPO ? $campoActivo : 'receptor_nombre') as $opKey => $opLabel)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                <label class="small mb-1" for="filtro_canal">Canal</label>
                <select name="filtro_canal" id="filtro_canal" class="form-control form-control-sm">
                    <option value="">Todos</option>
                    @foreach ($canales ?? [] as $cKey => $cLabel)
                        <option value="{{ $cKey }}" {{ ($f['canal'] ?? '') === $cKey ? 'selected' : '' }}>{{ $cLabel }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2">
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                <div class="custom-control custom-checkbox mt-4">
                    <input type="checkbox" class="custom-control-input" id="filtro_solo_pendientes"
                           name="filtro_solo_pendientes" value="1" {{ ! empty($f['solo_pendientes']) ? 'checked' : '' }}>
                    <label class="custom-control-label" for="filtro_solo_pendientes">Solo pendientes</label>
                </div>
            </div>
