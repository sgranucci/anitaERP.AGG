{{--
    JS para flujo «Generar nota de crédito» desde Facturas Local.
--}}
<script>
(function () {
    var csrfMeta = document.querySelector('meta[name="csrf-token"]');
    var token = csrfMeta ? csrfMeta.getAttribute('content') : '';
    var baseUrl = '{{ url('ventas/facturacion-local/facturas') }}';
    ...
    function postNotaCreditoBody(ventaId, tokenActual, payload) {
        return fetch(baseUrl + '/' + ventaId + '/generar-nota-credito', {
            method: 'POST',
            headers: {
        ...
        postNotaCreditoBody(ventaId, token, payload)
            ...
            .then(function (res) {
                if (res.ok && res.body && res.body.ok) {
                    ...
                    var pdfUrls = res.body.pdf_urls || [];
                    if (pdfUrls.length) {
                        setTimeout(function () { window.open(pdfUrls[0], '_blank'); }, 400);
                    }
                    ...
                    console.error('NC Facturación Local falló', res);
                    if (typeof toastr !== 'undefined') toastr.error(err, '', { timeOut: 12000, extendedTimeOut: 4000 });
                    else alert(err);
                }
            })
            .catch(function (e) {
                console.error('NC Facturación Local error de red', e);
