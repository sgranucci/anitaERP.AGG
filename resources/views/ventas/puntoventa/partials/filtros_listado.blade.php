@php
    ...
    $f = $filtros ?? [];
    $modo = $f['modo'] ?? PuntoventaListadoFiltros::MODO_TODOS;
    $campoActivo = $f['campo'] ?? 'nombre';
    $operadorActivo = $f['operador'] ?? 'contiene';
    $operadoresJson = [];
    foreach (PuntoventaListadoFiltros::CAMPOS as $key => $meta) {
        $operadoresJson[$key] = PuntoventaListadoFiltros::operadoresParaCampo($key);
    }
    $tieneCriteriosPanel = PuntoventaListadoFiltros::tieneCriteriosAplicados($f);
    $limpiarUrlPanel = $limpiarUrl ?? route('puntoventa');
@endphp
<div class="collapse border-bottom" id="panel-filtros-puntoventa" data-listado-filtros-panel>
        ...
        <div class="form-row align-items-end">
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_modo" id="filtro_modo" class="form-control form-control-sm">
                    <option value="{{ PuntoventaListadoFiltros::MODO_TODOS }}" {{ $modo === PuntoventaListadoFiltros::MODO_TODOS ? 'selected' : '' }}>Cualquier campo</option>
                    <option value="{{ PuntoventaListadoFiltros::MODO_CAMPO }}" {{ $modo === PuntoventaListadoFiltros::MODO_CAMPO ? 'selected' : '' }}>Campo determinado</option>
                </select>
            </div>
            <div class="form-group col-md-2 col-sm-6 mb-2 filtro-campo-wrap" style="{{ $modo !== PuntoventaListadoFiltros::MODO_CAMPO ? 'display:none' : '' }}">
                ...
                <select name="filtro_campo" id="filtro_campo" class="form-control form-control-sm">
                    @foreach($camposFiltro ?? PuntoventaListadoFiltros::CAMPOS as $key => $meta)
            ...
            <div class="form-group col-md-2 col-sm-6 mb-2">
                ...
                <select name="filtro_operador" id="filtro_operador" class="form-control form-control-sm"
                    ...
                    @foreach(PuntoventaListadoFiltros::operadoresParaCampo($modo === PuntoventaListadoFiltros::MODO_CAMPO ? $campoActivo : 'nombre') as $opKey => $opLabel)
            ...
            <div class="form-group col-md-3 col-sm-6 mb-2">
                ...
                <input type="text"
                       ...
                       value="{{ $f['valor'] ?? '' }}"
                       placeholder="Código, nombre, empresa…"
                       autocomplete="off">
            </div>
