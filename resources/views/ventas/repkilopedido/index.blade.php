...
<script src="{{ asset('assets/pages/scripts/ventas/repkilopedido/filtro.js') }}" type="text/javascript"></script>
    ...
    <div class="col-lg-12">
        ...
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title">Reporte de kilos pedidos</h3>
                <div class="card-tools">
                    <a href="{{ route('rep_kilopedido') }}" class="btn btn-outline-secondary btn-sm" title="Limpiar filtros">
                        <i class="fa fa-eraser"></i> Limpiar
                    </a>
                </div>
            </div>
            <form method="get" action="{{ route('rep_kilopedido') }}" id="form-kilo-pedido" class="mb-0">
                <div class="card-body pb-2">
                    <p class="text-muted small mb-3">
                        Kilos y piezas por reparto y fecha de entrega.
                        ...
                        <div class="col-lg-9">
                            <div class="row">
                                <div class="col-md-5">
                                    ...
                                    <div class="kilo-pedido-reparto-campo" data-campo="desde">
                                ...
                                <div class="col-md-5">
                                    ...
                                    <div class="kilo-pedido-reparto-campo" data-campo="hasta">
                                        <div class="input-group input-group-sm mb-1">
                    ...
                    <div class="form-group row">
                        <label for="tipolistado" class="col-lg-2 control-label requerido">Tipo de listado</label>
                        <div class="col-lg-4">
                            <select name="tipolistado" id="tipolistado" class="form-control" required>
                                @foreach ($tipolistado_enum as $value => $label)
                                    <option value="{{ $value }}" @selected(($filtros['tipolistado'] ?? 'TOTAL') === $value)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="card-body p-0 border-top">
                    <div class="px-3 py-2 border-bottom bg-light">
                        <p class="mb-0 small">
                            ...
                            · <strong>Listado:</strong> {{ $tipolistado_enum[$filtros['tipolistado'] ?? 'TOTAL'] ?? '' }}
                    ...
                    <div class="d-flex flex-wrap align-items-center justify-content-between px-3 py-2 border-bottom bg-light">
                        ...
                        @if (!empty($totales))
                            <div class="small mb-1 mb-md-0 text-md-right">
                                ...
                                <strong>{{ (int) ($totales['cantidad_detalle'] ?? 0) }}</strong> registros
                                · Piezas <strong>{{ number_format((float) ($totales['total_pieza'] ?? 0), 2, ',', '.') }}</strong>
                                · Kilos teóricos <strong>{{ number_format((float) ($totales['total_kilo'] ?? 0), 2, ',', '.') }}</strong>
                                · Kilos pesados <strong>{{ number_format((float) ($totales['total_pesada'] ?? 0), 2, ',', '.') }}</strong>
                    ...
                    <style>
                        #tabla-kilo-pedido thead tr { background-color: #85C1E9; color: #17202A; }
                        #tabla-kilo-pedido thead th { font-weight: 600; border-color: #7fb3d5; }
                    </style>
                    <div class="table-responsive">
                        <table id="tabla-kilo-pedido" class="table table-striped table-bordered table-hover table-sm mb-0" style="font-size: 0.78rem;">
                            @include('ventas.repkilopedido.partials.tabla_datos', [
                                'filas' => $filasVista ?? [],
                                'filtros' => $filtros,
                                'puede_ver_pedido' => $puede_ver_pedido ?? false,
                                'puede_ver_cliente' => $puede_ver_cliente ?? false,
                                'puede_ver_articulo' => $puede_ver_articulo ?? false,
                                'puede_ver_transporte' => $puede_ver_transporte ?? false,
                            ])
                        </table>
                    </div>
