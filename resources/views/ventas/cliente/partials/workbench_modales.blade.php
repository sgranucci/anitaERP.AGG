...
@php
    ...
    $layout = $grillaLayout ?? [];
    $catalogo = $catalogoColumnas ?? ClienteListadoColumnas::catalogoActivo();
        ...
        <div class="modal-content">
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane fade show active" id="pane-grilla-columnas" role="tabpanel">
                    <form method="post" action="{{ route('guardar_columnas_listado_cliente') }}" id="form-lw-grilla-aplicar">
                        @csrf
                ...
                <div class="tab-pane fade" id="pane-grilla-vista" role="tabpanel">
                    <form method="post" action="{{ route('guardar_vista_listado_cliente') }}" id="form-lw-grilla-vista">
                        @csrf
                        @foreach ($filtrosHidden as $hk => $hv)
                            @if (is_array($hv))
                                @foreach ($hv as $sk => $sv)
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="nombre" id="vista_nombre_grilla" class="form-control" required maxlength="120"
                                       ...
                                       placeholder="Ej. Clientes activos — vendedores">
                            </div>
                            @if ($vistaActiva)
                        ...
                        <div class="modal-footer justify-content-between">
                            @if ($vistaActiva && (int) ($vistaActiva->usuario_id ?? 0) === (int) auth()->id())
                                <button type="button" class="btn btn-outline-danger btn-sm" id="btn-lw-eliminar-vista-ref"
                                        data-action="{{ route('eliminar_vista_listado_cliente', $vistaActiva->id) }}">
                                    Eliminar esta vista
                                </button>
                            @else
                                <span></span>
                    ...
                    @if ($vistaActiva && (int) ($vistaActiva->usuario_id ?? 0) === (int) auth()->id())
                        <form method="post" action="{{ route('eliminar_vista_listado_cliente', $vistaActiva->id) }}" id="form-lw-eliminar-vista" class="d-none"
...
<div class="modal fade lw-modal" id="modal-lw-etiquetas" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <form method="post" action="{{ route('guardar_etiquetas_listado_cliente') }}">
                @csrf
                @foreach ($filtrosHidden as $hk => $hv)
