...
class Pedido_ArticuloRepository implements Pedido_ArticuloRepositoryInterface
{
    protected $model;
     ...
     * @param Post $post
     */
    public function __construct(Pedido_Articulo $pedido_articulo,
								PedidoQueryInterface $pedidoquery,
								ClienteQueryInterface $clientequery)
    {
        $this->model = $pedido_articulo;
    ...
    {
		$pedido_articulo = $this->model->create($data);
    ...
    {
		$pedido_articulo = $this->model->find($id)->first();
		
		$pedido_articulo = $pedido_articulo->update($data);
    ...
    {
    	$pedido_articulo = $this->model->find($id);
		
		$pedido_articulo->update(['ot_id'=>0]);
	...
	public function leePedidosSinOtPorFecha($hastafecha) {
		return $this->model->select('pedido_articulo.id as id',
									'pedido_articulo.pedido_id as pedido_id')
			->join('pedido', 'pedido_articulo.pedido_id', 'pedido.id')
			->where('pedido.fecha','<=',$hastafecha)
			->where('pedido_articulo.ot_id',0)
			->get();
	}
	...
	private function guardarPedidoEnAnita(array $data, $articulo_id, $combinacion_id, $numeroitem,
	  								$modulo_id, $cantidad, $precio, $listaprecio_id, $incluyeimpuesto, 
	  								$moneda_id, $descuento, $medidas, $observacion, $funcion) 
	{		
        $apiAnita = new ApiAnita();
       			...
       			$data = array( 'tabla' => $this->tableAnita, 'acc' => 'insert',
           			'campos' => ' 
		    			penv_cliente, penv_tipo, penv_letra, penv_sucursal, penv_nro, penv_orden, penv_articulo, penv_desc,
   						...
   						penv_cod_mon, penv_vendedor, penv_zonavta, penv_zonamult, penv_partida, penv_fecha_ent,'.
						(strtoupper(config('app.empresa')) == "EL BIERZO" ? ' penv_pieza, penv_fl_bonif, penv_reparto, penv_kilos_reales,
						penv_piezas_reales ' :
						'penv_color,
