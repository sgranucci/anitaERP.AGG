...
final class GastronomiaCuentacajaTotem
{
    ...
    {
        return strtoupper(trim((string) config('gastronomia.cuentacaja_totem_codigo', 'TOTEM')));
    }
    ...
    {
        if ($empresaId <= 0) {
            return 'Empresa no válida para resolver cuenta TOTEM.';
        }

        $codigo = self::codigo();
        if ($codigo === '') {
            return 'No está configurado el código de cuenta de caja TOTEM (GASTRONOMIA_CUENTACAJA_TOTEM_CODIGO).';
        }
