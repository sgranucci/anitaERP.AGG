...
class VentaSerieNumeradorListadoFiltros
{
    ...
    public const CAMPOS = [
        'codigo_pv' => ['column' => 'puntoventa.codigo', 'type' => 'texto', 'label' => 'Código PV'],
        'nombre_pv' => ['column' => 'puntoventa.nombre', 'type' => 'texto', 'label' => 'Nombre PV'],
        'codigo_afip' => ['column' => 'venta_serie_numerador.codigo_afip', 'type' => 'entero', 'label' => 'Tipo ARCA'],
        'ultimo_numero' => ['column' => 'venta_serie_numerador.ultimo_numero', 'type' => 'entero', 'label' => 'Último número'],
        'piso' => ['column' => 'venta_serie_numerador.piso', 'type' => 'entero', 'label' => 'Piso'],
        'modofacturacion' => ['column' => 'puntoventa.modofacturacion', 'type' => 'texto', 'label' => 'Modo PV'],
    ];
    ...
    public static function resolverDesdeRequest(Request $request, ?string $busquedaRuta = null): array
    {
        ...
        $campo = (string) $request->input('filtro_campo', 'codigo_pv');
        ...
        $operador = self::normalizarOperador($operador, $modo === self::MODO_CAMPO ? $campo : 'codigo_pv');
    ...
    public static function paraQueryString(array $filtros): array
    {
        ...
        if (($filtros['modo'] ?? '') === self::MODO_CAMPO) {
            $params['filtro_campo'] = $filtros['campo'] ?? 'codigo_pv';
    ...
    /**
     * @param  Builder<\App\Models\Ventas\Venta_Serie_Numerador>  $query
     */
    public static function aplicar(Builder $query, array $filtros): void
    {
        ...
        if ($modo === self::MODO_CAMPO) {
            self::aplicarEnCampo($query, $filtros['campo'] ?? 'codigo_pv', $operador, $valor);
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('venta_serie_numerador.codigo_afip', (int) $id);
                $q->orWhere('venta_serie_numerador.ultimo_numero', (int) $id);
            }
            foreach (['puntoventa.codigo', 'puntoventa.nombre', 'puntoventa.modofacturacion'] as $col) {
    ...
    {
        $def = self::CAMPOS[$campoKey] ?? self::CAMPOS['codigo_pv'];
        if (($def['type'] ?? '') === 'entero') {
