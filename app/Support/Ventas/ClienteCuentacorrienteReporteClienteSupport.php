...
/**
 * Resuelve clientes del reporte CC según alcance: todos / puntuales / rango.
 */
final class ClienteCuentacorrienteReporteClienteSupport
{
    /**
    ...
    {
        $alcance = (string) ($filtros['alcance_clientes'] ?? ClienteCuentacorrienteReporteFiltros::ALCANCE_TODOS);
    ...
    private static function desdePuntuales(array $filtros): array
    {
        $ids = array_values(array_filter(
            array_map('intval', $filtros['cliente_ids'] ?? []),
    ...
    private static function desdeRango(array $filtros): array
    {
        ...
        $iniciales = $clientes->map(static fn (Cliente $c) => [
            'id' => (int) $c->id,
            'codigo' => trim((string) $c->codigo),
            'nombre' => (string) $c->nombre,
        ])->values()->all();
