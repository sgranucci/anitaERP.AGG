...
/**
 * Filtros del listado maestro de turnos (Facturación Local).
 */
class TurnoLocalListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'turno_local.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'turno_local.codigo', 'type' => 'texto', 'label' => 'Código'],
        'nombre' => ['column' => 'turno_local.nombre', 'type' => 'texto', 'label' => 'Nombre'],
        'empresa' => ['column' => 'empresa.nombre', 'type' => 'texto', 'label' => 'Empresa'],
        'activo' => ['column' => 'turno_local.activo', 'type' => 'texto', 'label' => 'Activo'],
    ];
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                foreach (['turno_local.codigo', 'turno_local.nombre'] as $col) {
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('turno_local.id', (int) $id);
            }
            foreach (['turno_local.codigo', 'turno_local.nombre', 'empresa.nombre'] as $col) {
            ...
            if ($activoNorm !== null) {
                $q->orWhere('turno_local.activo', $activoNorm);
            }
        });
    }
    ...
    private static function aplicarEnCampo(Builder $query, string $campoKey, string $operador, string $valor): void
    {
        ...
        if ($campoKey === 'activo') {
            ...
            if ($operador === 'vacio') {
                $query->whereNull('turno_local.activo');

                return;
            }
            if ($operador === 'distinto') {
                $query->where('turno_local.activo', '!=', $norm);
