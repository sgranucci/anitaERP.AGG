...
/**
 * Filtros del listado de usuarios de vianda (index / export).
 *
 * Sigue el patrón estándar del sistema: búsqueda inteligente (modo/campo/operador)
 * con tolerancia a errores de tipeo, más selects dedicados de empresa, estado y tipo.
 */
final class ViandaUsuarioListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'vianda_usuario.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'vianda_usuario.codigo_usuario', 'type' => 'texto', 'label' => 'Código'],
        'nombre' => ['column' => 'vianda_usuario.nombre', 'type' => 'texto', 'label' => 'Nombre'],
        'empresa' => ['column' => 'empresa.nombre', 'type' => 'texto', 'label' => 'Empresa'],
        'tipo_menu' => ['column' => 'vianda_tipo_menu.nombre', 'type' => 'texto', 'label' => 'Tipo menú'],
        'centrocosto' => ['column' => 'centrocosto.nombre', 'type' => 'texto', 'label' => 'Centro de costo'],
    ];

    /** @var list<string> */
    private const COLUMNAS_COINCIDENCIA_FLEXIBLE = [
        'vianda_usuario.nombre',
        'empresa.nombre',
        'vianda_tipo_menu.nombre',
        'centrocosto.nombre',
    ];
    ...
    public static function resolverDesdeRequest(Request $request, ?string $busquedaRuta = null): array
    {
        ...
        return [
            ...
            'estado' => self::normalizarEstado($request->input('estado')),
            'tipo_usuario' => self::normalizarTipo($request->input('tipo_usuario')),
            'empresas_asignadas' => [],
        ];
    }
    ...
    public static function paraQueryString(array $filtros): array
    {
        ...
        if (($filtros['tipo_usuario'] ?? '') !== '') {
            $params['tipo_usuario'] = (string) $filtros['tipo_usuario'];
        }
    ...
    /**
     * Limita el listado a las empresas del usuario (sesión). Si hay empresa_id, acota a esa.
     *
     * @param  Builder<\App\Models\Ventas\ViandaUsuario>  $query
     * @param  array<string, mixed>  $filtros
     */
    public static function aplicarScopeEmpresasAsignadas(Builder $query, array $filtros): void
    {
        ...
        if ($empresaId > 0) {
            $query->where('vianda_usuario.empresa_id', $empresaId);
        ...
        $query->whereIn('vianda_usuario.empresa_id', $asignadas);
    }

    /**
     * Filtros de select dedicados (estado / tipo de usuario). Se aplican siempre.
     *
     * @param  Builder<\App\Models\Ventas\ViandaUsuario>  $query
     * @param  array<string, mixed>  $filtros
     */
    public static function aplicarFiltrosDirectos(Builder $query, array $filtros): void
    {
        $estado = (string) ($filtros['estado'] ?? '');
        if (in_array($estado, ['A', 'I'], true)) {
            $query->where('vianda_usuario.estado', $estado);
        }

        $tipo = (string) ($filtros['tipo_usuario'] ?? '');
        if ($tipo !== '' && ViandaUsuarioTipoSupport::tipoValido($tipo)) {
            $query->where('vianda_usuario.tipo_usuario', strtoupper($tipo));
        }
    }

    /**
     * Búsqueda inteligente de texto (modo global o campo determinado).
     *
     * @param  Builder<\App\Models\Ventas\ViandaUsuario>  $query
     * @param  array<string, mixed>  $filtros
     */
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                foreach (['vianda_usuario.nombre', 'vianda_usuario.codigo_usuario'] as $col) {
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('vianda_usuario.id', (int) $id);
            }
            $textCols = [
                'vianda_usuario.codigo_usuario',
                'vianda_usuario.nombre',
                'empresa.nombre',
                'vianda_tipo_menu.nombre',
                'centrocosto.nombre',
    ...
    private static function normalizarEstado(mixed $value): string
    {
        $estado = strtoupper(trim((string) ($value ?? '')));

        return in_array($estado, ['A', 'I'], true) ? $estado : '';
    }

    private static function normalizarTipo(mixed $value): string
    {
        $tipo = strtoupper(trim((string) ($value ?? '')));

        return ViandaUsuarioTipoSupport::tipoValido($tipo) ? $tipo : '';
    }
}
