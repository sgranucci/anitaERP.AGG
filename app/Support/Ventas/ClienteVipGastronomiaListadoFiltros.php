...
/**
 * Filtros del listado de clientes VIP gastronomía.
 */
class ClienteVipGastronomiaListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'cliente_vip_gastronomia.id', 'type' => 'entero', 'label' => 'ID'],
        'numeroid' => ['column' => 'cliente_vip_gastronomia.numeroid', 'type' => 'entero', 'label' => 'Número Anita'],
        'nrodocumento' => ['column' => 'cliente_vip_gastronomia.nrodocumento', 'type' => 'texto', 'label' => 'Nro. documento'],
        'apellido' => ['column' => 'cliente_vip_gastronomia.apellido', 'type' => 'texto', 'label' => 'Apellido'],
        'nombre' => ['column' => 'cliente_vip_gastronomia.nombre', 'type' => 'texto', 'label' => 'Nombre'],
        'nickname' => ['column' => 'cliente_vip_gastronomia.nickname', 'type' => 'texto', 'label' => 'Nickname'],
        'localidad' => ['column' => 'cliente_vip_gastronomia.localidad', 'type' => 'texto', 'label' => 'Localidad'],
        'empresa' => ['column' => 'empresa.nombre', 'type' => 'texto', 'label' => 'Empresa'],
    ];

    /** @var list<string> */
    private const COLUMNAS_COINCIDENCIA_FLEXIBLE = [
        'cliente_vip_gastronomia.apellido',
        'cliente_vip_gastronomia.nombre',
        'cliente_vip_gastronomia.nickname',
        'cliente_vip_gastronomia.localidad',
        'empresa.nombre',
    ];
    ...
    /**
     * @param  Builder<\App\Models\Ventas\ClienteVipGastronomia>  $query
     */
    public static function aplicarScopeEmpresasAsignadas(Builder $query, array $filtros): void
    {
        $empresaId = (int) ($filtros['empresa_id'] ?? 0);
        if ($empresaId > 0) {
            $query->where('cliente_vip_gastronomia.empresa_id', $empresaId);
        ...
        $query->whereIn('cliente_vip_gastronomia.empresa_id', $asignadas);
    }
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                foreach (['cliente_vip_gastronomia.apellido', 'cliente_vip_gastronomia.nombre', 'cliente_vip_gastronomia.nrodocumento'] as $col) {
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('cliente_vip_gastronomia.id', (int) $id);
                $q->orWhere('cliente_vip_gastronomia.numeroid', (int) $id);
            }
            $textCols = [
                'cliente_vip_gastronomia.nrodocumento',
                'cliente_vip_gastronomia.apellido',
                'cliente_vip_gastronomia.nombre',
                'cliente_vip_gastronomia.nickname',
                'cliente_vip_gastronomia.localidad',
