...
/**
 * Filtros del listado de locales de venta (Facturación Local).
 */
class LocalVentaListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'local_venta.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'local_venta.codigo', 'type' => 'texto', 'label' => 'Código'],
        'nombre' => ['column' => 'local_venta.nombre', 'type' => 'texto', 'label' => 'Nombre'],
        'activo' => ['column' => 'local_venta.activo', 'type' => 'texto', 'label' => 'Activo'],
        'empresa' => ['column' => 'empresa.nombre', 'type' => 'texto', 'label' => 'Empresa'],
        'puntoventa' => ['column' => 'puntoventa.nombre', 'type' => 'texto', 'label' => 'Punto de venta'],
        'puntoventa_codigo' => ['column' => 'puntoventa.codigo', 'type' => 'texto', 'label' => 'Cód. PV'],
    ...
    /**
     * @param  Builder<\App\Models\Ventas\LocalVenta>  $query
     */
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                foreach (['local_venta.codigo', 'local_venta.nombre'] as $col) {
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('local_venta.id', (int) $id);
            }
            $textCols = [
                'local_venta.codigo',
                'local_venta.nombre',
            ...
            $activoNorm = self::normalizarActivoBusqueda($valor);
            if ($activoNorm !== null) {
                $q->orWhere('local_venta.activo', $activoNorm);
            }
        });
    }
    ...
    private static function aplicarEnCampo(Builder $query, string $campoKey, string $operador, string $valor): void
    {
        ...
        if ($campoKey === 'activo') {
            self::aplicarActivo($query, $operador, $valor);
     ...
     * @param  Builder<\App\Models\Ventas\LocalVenta>  $query
     */
    private static function aplicarActivo(Builder $query, string $operador, string $valor): void
    {
        $norm = self::normalizarActivoBusqueda($valor);
        if ($norm === null && $operador !== 'vacio') {
            return;
        }
        if ($operador === 'vacio') {
            $query->whereNull('local_venta.activo');

            return;
        }
        if ($operador === 'distinto') {
            $query->where('local_venta.activo', '!=', $norm);
    ...
    private static function normalizarActivoBusqueda(string $valor): ?int
    {
        $v = mb_strtolower(trim($valor));
        if (in_array($v, ['1', 'si', 'sí', 'true', 'activo', 's'], true)) {
            return 1;
        }
        if (in_array($v, ['0', 'no', 'false', 'inactivo', 'n'], true)) {
            return 0;
        }
