...
/**
 * Motor de sugerencia y validación de aplicaciones CC cliente (sin I/O).
 *
 * Créditos = NC / pagos a cuenta (saldo &gt; 0 ya en valor absoluto).
 * Deudas   = facturas / ND con saldo pendiente.
 */
final class ClienteCuentacorrienteAplicacionMatcherSupport
{
    ...
    /**
     * @param  array<int, array{id:int,saldo:float,moneda_id:int,empresa_id?:int,cliente_id?:int,fecha:?string}>  $creditosById
     * @param  array<int, array{id:int,saldo:float,moneda_id:int,empresa_id?:int,cliente_id?:int,fecha:?string}>  $deudasById
    ...
    public static function validarLineas(array $creditosById, array $deudasById, array $lineas, ?string $fechaAplicacion = null): array
    {
        ...
        foreach ($lineas as $i => $linea) {
            ...
            if (isset($credito['cliente_id'], $deuda['cliente_id'])
                && (int) $credito['cliente_id'] !== (int) $deuda['cliente_id']) {
                $errores[] = "Línea {$n}: crédito y deuda deben ser del mismo cliente.";
            }
            if ($fechaAplicacion) {
