...
/**
 * Filtros del listado de apertura de gastos (rendición de máquinas).
 */
class AperturaGastoListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'apertura_gasto.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'apertura_gasto.codigo', 'type' => 'entero', 'label' => 'Código'],
        'nombre' => ['column' => 'apertura_gasto.nombre', 'type' => 'texto', 'label' => 'Nombre'],
        ...
        'estado' => ['column' => 'apertura_gasto.estado', 'type' => 'texto', 'label' => 'Estado'],
    ];

    /** @var list<string> */
    private const COLUMNAS_COINCIDENCIA_FLEXIBLE = [
        'apertura_gasto.nombre',
        'empresa.nombre',
     ...
     *
     * @param  Builder<\App\Models\Caja\AperturaGasto>  $query
    ...
    public static function aplicarJoinsListado(Builder $query, array $filtros): void
    {
        ...
        $query
            ->leftJoin('apertura_gasto_empresa', 'apertura_gasto_empresa.apertura_gasto_id', '=', 'apertura_gasto.id')
            ->leftJoin('empresa', 'empresa.id', '=', 'apertura_gasto_empresa.empresa_id')
            ->leftJoin('cuentacontable', 'cuentacontable.id', '=', 'apertura_gasto_empresa.cuentacontable_id')
            ->distinct();
    }
    ...
    public static function aplicarScopeEmpresasAsignadas(Builder $query, array $filtros): void
    {
        ...
        if ($empresaId > 0) {
            $query->whereExists(function ($q) use ($empresaId) {
                $q->selectRaw('1')
                    ->from('apertura_gasto_empresa')
                    ->whereColumn('apertura_gasto_empresa.apertura_gasto_id', 'apertura_gasto.id')
                    ->where('apertura_gasto_empresa.empresa_id', $empresaId);
            });
        ...
        $query->whereExists(function ($q) use ($asignadas) {
            $q->selectRaw('1')
                ->from('apertura_gasto_empresa')
                ->whereColumn('apertura_gasto_empresa.apertura_gasto_id', 'apertura_gasto.id')
                ->whereIn('apertura_gasto_empresa.empresa_id', $asignadas);
        });
    }
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                foreach (['apertura_gasto.nombre', 'apertura_gasto.estado'] as $col) {
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('apertura_gasto.id', (int) $id)
                    ->orWhere('apertura_gasto.codigo', (int) $id);
            }
