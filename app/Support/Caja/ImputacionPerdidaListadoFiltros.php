...
/**
 * Filtros del listado de imputaciones de pérdida.
 */
class ImputacionPerdidaListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'imputacion_perdida.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'imputacion_perdida.codigo', 'type' => 'entero', 'label' => 'Código'],
        'nombre' => ['column' => 'imputacion_perdida.nombre', 'type' => 'texto', 'label' => 'Nombre'],
        'empresa' => ['column' => 'empresa.nombre', 'type' => 'texto', 'label' => 'Empresa'],
        'cuenta' => ['column' => 'cuentacontable.nombre', 'type' => 'texto', 'label' => 'Cuenta contable'],
    ];
    ...
    /**
     * Joins a hija/empresa/cuenta solo cuando el filtro o la búsqueda lo requieren.
     *
     * @param  Builder<\App\Models\Caja\ImputacionPerdida>  $query
    ...
    public static function aplicarJoinsListado(Builder $query, array $filtros): void
    {
        ...
        $query
            ->leftJoin('imputacion_perdida_empresa', 'imputacion_perdida_empresa.imputacion_perdida_id', '=', 'imputacion_perdida.id')
            ->leftJoin('empresa', 'empresa.id', '=', 'imputacion_perdida_empresa.empresa_id')
            ->leftJoin('cuentacontable', 'cuentacontable.id', '=', 'imputacion_perdida_empresa.cuentacontable_id')
            ->distinct();
    }
    ...
    private static function requiereJoinEmpresas(array $filtros): bool
    {
        ...
        return in_array($campo, ['empresa', 'cuenta'], true);
    }
    ...
    public static function aplicarScopeEmpresasAsignadas(Builder $query, array $filtros): void
    {
        ...
        if ($empresaId > 0) {
            $query->whereExists(function ($q) use ($empresaId) {
                $q->selectRaw('1')
                    ->from('imputacion_perdida_empresa')
                    ->whereColumn('imputacion_perdida_empresa.imputacion_perdida_id', 'imputacion_perdida.id')
                    ->where('imputacion_perdida_empresa.empresa_id', $empresaId);
            });
        ...
        $query->whereExists(function ($q) use ($asignadas) {
            $q->selectRaw('1')
                ->from('imputacion_perdida_empresa')
                ->whereColumn('imputacion_perdida_empresa.imputacion_perdida_id', 'imputacion_perdida.id')
                ->whereIn('imputacion_perdida_empresa.empresa_id', $asignadas);
        });
    }
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                $q->where(function ($w) {
                    $w->whereNull('imputacion_perdida.nombre')->orWhere('imputacion_perdida.nombre', '');
                });
            });
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('imputacion_perdida.id', (int) $id)
                    ->orWhere('imputacion_perdida.codigo', (int) $id);
            }
