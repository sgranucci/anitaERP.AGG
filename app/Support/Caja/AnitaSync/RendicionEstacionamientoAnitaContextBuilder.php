 ...
 */
final class RendicionEstacionamientoAnitaContextBuilder
{
    /**
     ...
     */
    public static function desdeRendicion(RendicionEstacionamientoCaja $rendicion, ?float $totalZForzado = null): array
    {
        ...
        $numeracion = EstacionamientoTurnoNumeracionComprobanteSupport::paraTurno(
            $turno,
        ...
        return [
            ...
            'nro_rend_vta' => (int) $rendicion->turno_operativo_estacionamiento_id,
    ...
    private static function totalFacturacionSinNotasCreditoTurno(
        ...
        if ($turno->habilitacion_en !== null && $turno->cierre_en !== null) {
            return EstacionamientoTurnoOperativoTotalesSupport::totalFacturasSinNotasCredito(
    ...
    private static function letraTurno(TurnoOperativoEstacionamiento $turno): string
    {
    ...
    public static function totalesFacturacionPuntoventaEnTurno(
        ...
        $filas = VentaEstacionamientoEmision::query()
            ->join('venta', 'venta.id', '=', 'venta_estacionamiento_emision.venta_id')
            ->where('venta_estacionamiento_emision.identificador_pc', (string) $turno->identificador_pc)
            ...
            ->select([
                DB::raw('SUM(CASE WHEN venta_estacionamiento_emision.venta_factura_origen_id IS NULL THEN venta.total ELSE 0 END) AS total_fc'),
                DB::raw('SUM(CASE WHEN venta_estacionamiento_emision.venta_factura_origen_id IS NOT NULL THEN venta.total ELSE 0 END) AS total_nc'),
            ])
            ->first();
