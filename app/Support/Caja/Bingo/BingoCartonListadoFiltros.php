...
/**
 * Filtros del listado de cartones de bingo.
 */
class BingoCartonListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'bingo_carton.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'bingo_carton.codigo', 'type' => 'texto', 'label' => 'Código'],
        'nombre' => ['column' => 'bingo_carton.nombre', 'type' => 'texto', 'label' => 'Nombre'],
        'empresa' => ['column' => 'empresa.nombre', 'type' => 'texto', 'label' => 'Empresa'],
        'precio_unitario' => ['column' => 'bingo_carton.precio_unitario', 'type' => 'decimal', 'label' => 'Precio unitario'],
        'lineas' => ['column' => 'bingo_carton.lineas', 'type' => 'entero', 'label' => 'Líneas'],
        'estado' => ['column' => 'bingo_carton.estado', 'type' => 'texto', 'label' => 'Estado'],
    ];
     ...
     *
     * @param  Builder<\App\Models\Caja\Bingo\BingoCarton>  $query
    ...
    public static function aplicarScopeEmpresasAsignadas(Builder $query, array $filtros): void
    {
        ...
        if ($empresaId > 0) {
            $query->where('bingo_carton.empresa_id', $empresaId);
        ...
        $query->whereIn('bingo_carton.empresa_id', $asignadas);
    }
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                foreach (['bingo_carton.codigo', 'bingo_carton.nombre', 'bingo_carton.estado'] as $col) {
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador, $esNumerico, $numero) {
            if ($id !== false) {
                $q->orWhere('bingo_carton.id', (int) $id);
                $q->orWhere('bingo_carton.lineas', (int) $id);
            }
            if ($esNumerico) {
                $q->orWhere('bingo_carton.precio_unitario', '=', (float) $numero);
            }
            $textCols = [
                'bingo_carton.codigo',
                'bingo_carton.nombre',
