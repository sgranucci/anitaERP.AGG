...
/**
 * Filtros del listado de conceptos de rendición de bingo.
 */
class BingoConceptoRendicionListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'bingo_concepto_rendicion.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'bingo_concepto_rendicion.codigo', 'type' => 'texto', 'label' => 'Código'],
        'detalle' => ['column' => 'bingo_concepto_rendicion.detalle', 'type' => 'texto', 'label' => 'Detalle'],
        'signo' => ['column' => 'bingo_concepto_rendicion.signo', 'type' => 'texto', 'label' => 'Signo'],
        'porcentaje' => ['column' => 'bingo_concepto_rendicion.porcentaje', 'type' => 'decimal', 'label' => 'Porcentaje'],
        'empresa' => ['column' => 'empresa.nombre', 'type' => 'texto', 'label' => 'Empresa'],
        'estado' => ['column' => 'bingo_concepto_rendicion.estado', 'type' => 'texto', 'label' => 'Estado'],
    ];

    /** @var list<string> */
    private const COLUMNAS_COINCIDENCIA_FLEXIBLE = [
        'bingo_concepto_rendicion.codigo',
        'bingo_concepto_rendicion.detalle',
        'empresa.nombre',
    ];
     ...
     *
     * @param  Builder<\App\Models\Caja\Bingo\BingoConceptoRendicion>  $query
    ...
    public static function aplicarScopeEmpresasAsignadas(Builder $query, array $filtros): void
    {
        ...
        if ($empresaId > 0) {
            $query->where('bingo_concepto_rendicion.empresa_id', $empresaId);
        ...
        $query->whereIn('bingo_concepto_rendicion.empresa_id', $asignadas);
    }
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                foreach (['bingo_concepto_rendicion.codigo', 'bingo_concepto_rendicion.detalle', 'bingo_concepto_rendicion.estado'] as $col) {
        ...
        $esNumerico = is_numeric($numero);

        $query->where(function ($q) use ($valor, $like, $id, $operador, $esNumerico, $numero) {
            if ($id !== false) {
                $q->orWhere('bingo_concepto_rendicion.id', (int) $id);
            }
            if ($esNumerico) {
                $q->orWhere('bingo_concepto_rendicion.porcentaje', '=', (float) $numero);
            }
            $textCols = [
                'bingo_concepto_rendicion.codigo',
                'bingo_concepto_rendicion.detalle',
                'bingo_concepto_rendicion.signo',
                'empresa.nombre',
                'bingo_concepto_rendicion.estado',
            ];
