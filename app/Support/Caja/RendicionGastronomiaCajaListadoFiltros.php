...
/**
 * Filtros del listado de rendiciones gastronomía en caja (index / exportaciones).
 */
class RendicionGastronomiaCajaListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'rendicion_gastronomia_caja.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'rendicion_gastronomia_caja.codigo', 'type' => 'texto', 'label' => 'Ticket / código'],
        'tipo' => ['column' => 'rendicion_gastronomia_caja.tipo', 'type' => 'texto', 'label' => 'Tipo (turno / jornada)'],
        ...
        'turno_operativo_id' => ['column' => 'rendicion_gastronomia_caja.turno_operativo_gastronomia_id', 'type' => 'entero', 'label' => 'Turno operativo'],
        'jornada_id' => ['column' => 'rendicion_gastronomia_caja.jornada_gastronomia_id', 'type' => 'entero', 'label' => 'Jornada'],
        'fecharendicion' => ['column' => 'rendicion_gastronomia_caja.fecharendicion', 'type' => 'fecha', 'label' => 'Fecha rendición'],
        'fecha_jornada' => ['type' => 'fecha_jornada', 'label' => 'Fecha jornada'],
        'waitry_hasta' => ['column' => 'rendicion_gastronomia_caja.waitry_order_id_hasta', 'type' => 'entero', 'label' => 'Waitry hasta'],
    ];

    /** @var list<string> */
    private const COLUMNAS_COINCIDENCIA_FLEXIBLE = [
        'rendicion_gastronomia_caja.codigo',
        'empresa.nombre',
        'caja.nombre',
        'puntoventa.codigo',
        'puntoventa.nombre',
     ...
     *
     * @param  Builder<RendicionGastronomiaCaja>  $query
    ...
    public static function aplicarScopeEmpresasAsignadas(Builder $query, array $filtros): void
    {
        ...
        if ($empresaId > 0) {
            $query->where('rendicion_gastronomia_caja.empresa_id', $empresaId);
        ...
        $query->whereIn('rendicion_gastronomia_caja.empresa_id', $asignadas);
    }
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('rendicion_gastronomia_caja.id', (int) $id)
                    ->orWhere('rendicion_gastronomia_caja.turno_operativo_gastronomia_id', (int) $id)
                    ->orWhere('rendicion_gastronomia_caja.jornada_gastronomia_id', (int) $id)
                    ->orWhere('rendicion_gastronomia_caja.waitry_order_id_hasta', (int) $id);
            }

            foreach (['rendicion_gastronomia_caja.codigo', 'rendicion_gastronomia_caja.tipo'] as $col) {
