...
/**
 * Filtros del listado de provincias (index / exportaciones del ABM).
 */
class ProvinciaListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'provincia.id', 'type' => 'entero', 'label' => 'ID'],
        'nombre' => ['column' => 'provincia.nombre', 'type' => 'texto', 'label' => 'Nombre'],
        'abreviatura' => ['column' => 'provincia.abreviatura', 'type' => 'texto', 'label' => 'Abreviatura'],
        'jurisdiccion' => ['column' => 'provincia.jurisdiccion', 'type' => 'texto', 'label' => 'Jurisdicción'],
        'codigo' => ['column' => 'provincia.codigo', 'type' => 'texto', 'label' => 'Código Anita'],
        'pais' => ['column' => 'pais.nombre', 'type' => 'texto', 'label' => 'País'],
        'codigoexterno' => ['column' => 'provincia.codigoexterno', 'type' => 'texto', 'label' => 'Código externo'],
        'minimocoeficientecm05' => ['column' => 'provincia.minimocoeficientecm05', 'type' => 'texto', 'label' => 'Mínimo coef. CM05'],
    ];
    ...
    /**
     * @param  Builder<\App\Models\Configuracion\Provincia>  $query
     */
    ...
    public static function textoTasas($provincia): string
    {
        $tasas = $provincia->provincia_tasaiibbs ?? collect();

        return $tasas->map(function ($tasa) {
            $condicion = $tasa->condicioniibbs->nombre ?? '';

            return trim(sprintf(
                '%s %s %% Min.Neto %s Min.Perc. %s',
                $condicion,
                number_format((float) ($tasa->tasa ?? 0), 2, ',', '.'),
                number_format((float) ($tasa->minimoneto ?? 0), 2, ',', '.'),
                number_format((float) ($tasa->minimopercepcion ?? 0), 2, ',', '.')
            ));
        })->filter()->implode("\n");
    }

    public static function textoCuentas($provincia): string
    {
        $cuentas = $provincia->provincia_cuentacontableiibbs ?? collect();

        return $cuentas->map(function ($fila) {
            $empresa = $fila->empresas->nombre ?? '';
            $codigo = $fila->cuentacontables->codigo ?? '';
            $nombre = $fila->cuentacontables->nombre ?? '';

            return trim($empresa.' '.$codigo.($nombre !== '' ? '-'.$nombre : ''));
        })->filter()->implode("\n");
    }
}
