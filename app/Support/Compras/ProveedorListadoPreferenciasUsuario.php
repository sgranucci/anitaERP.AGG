...
/**
 * Preferencias de grilla del listado de proveedores (por usuario).
 *
 ...
 */
final class ProveedorListadoPreferenciasUsuario
{
    private const CACHE_GRILLA = 'proveedor-listado-grilla-estandar-v3';

    private const CACHE_COLUMNAS = 'proveedor-listado-columnas';
    ...
    {
        $normalizadas = ProveedorListadoColumnas::normalizarVisibles($columnas);
    ...
    public static function resolverColumnas(?array $desdeRequest = null): array
    {
        if (is_array($desdeRequest) && $desdeRequest !== []) {
            return ProveedorListadoColumnas::normalizarVisibles($desdeRequest);
        }
