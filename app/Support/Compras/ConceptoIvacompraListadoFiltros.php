...
/**
 * Filtros del listado de conceptos del Libro de IVA Compras.
 */
class ConceptoIvacompraListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'concepto_ivacompra.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'concepto_ivacompra.codigo', 'type' => 'texto', 'label' => 'Código'],
        'nombre' => ['column' => 'concepto_ivacompra.nombre', 'type' => 'texto', 'label' => 'Nombre'],
        'nombre_ia' => ['column' => 'concepto_ivacompra.nombre_ia', 'type' => 'texto', 'label' => 'Nombre IA'],
        'tipoconcepto' => ['column' => 'concepto_ivacompra.tipoconcepto', 'type' => 'texto', 'label' => 'Tipo'],
        'empresa' => ['column' => 'empresa.nombre', 'type' => 'texto', 'label' => 'Empresa'],
        'cuenta_debe' => ['column' => 'cta_debe.nombre', 'type' => 'texto', 'label' => 'Cuenta Debe'],
        'cuenta_haber' => ['column' => 'cta_haber.nombre', 'type' => 'texto', 'label' => 'Cuenta Haber'],
    ];

    /** @var list<string> */
    private const COLUMNAS_COINCIDENCIA_FLEXIBLE = [
        'concepto_ivacompra.nombre',
        'concepto_ivacompra.nombre_ia',
        'empresa.nombre',
        'cta_debe.nombre',
        'cta_haber.nombre',
    ];
    ...
    /**
     * @param  Builder<\App\Models\Compras\Concepto_Ivacompra>  $query
     * @param  array<string, mixed>  $filtros
     */
    public static function aplicarJoinsListado(Builder $query, array $filtros): void
    {
        if (! self::requiereJoinEmpresas($filtros)) {
            return;
        }

        $query
            ->leftJoin('concepto_ivacompra_empresa', 'concepto_ivacompra_empresa.concepto_ivacompra_id', '=', 'concepto_ivacompra.id')
            ->leftJoin('empresa', 'empresa.id', '=', 'concepto_ivacompra_empresa.empresa_id')
            ->leftJoin('cuentacontable as cta_debe', 'cta_debe.id', '=', 'concepto_ivacompra_empresa.cuentacontabledebe_id')
            ->leftJoin('cuentacontable as cta_haber', 'cta_haber.id', '=', 'concepto_ivacompra_empresa.cuentacontablehaber_id')
            ->distinct();
    }
     ...
     * @param  array<string, mixed>  $filtros
     */
    private static function requiereJoinEmpresas(array $filtros): bool
    {
        ...
        return in_array($campo, ['empresa', 'cuenta_debe', 'cuenta_haber'], true);
    }
    ...
    public static function aplicarScopeEmpresasAsignadas(Builder $query, array $filtros): void
    {
        ...
        if ($empresaId > 0) {
            $query->where(function ($q) use ($empresaId) {
                $q->whereExists(function ($sub) use ($empresaId) {
                    $sub->selectRaw('1')
                        ->from('concepto_ivacompra_empresa')
                        ->whereColumn('concepto_ivacompra_empresa.concepto_ivacompra_id', 'concepto_ivacompra.id')
                        ->where('concepto_ivacompra_empresa.empresa_id', $empresaId);
                })->orWhereDoesntHave('concepto_ivacompra_empresas');
            });
        ...
        $query->where(function ($q) use ($asignadas) {
            $q->whereExists(function ($sub) use ($asignadas) {
                $sub->selectRaw('1')
                    ->from('concepto_ivacompra_empresa')
                    ->whereColumn('concepto_ivacompra_empresa.concepto_ivacompra_id', 'concepto_ivacompra.id')
                    ->whereIn('concepto_ivacompra_empresa.empresa_id', $asignadas);
            })->orWhereDoesntHave('concepto_ivacompra_empresas');
        });
    }
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                foreach (['concepto_ivacompra.nombre', 'concepto_ivacompra.codigo'] as $col) {
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('concepto_ivacompra.id', (int) $id);
            }

            $textCols = [
                'concepto_ivacompra.nombre',
                'concepto_ivacompra.nombre_ia',
                'concepto_ivacompra.codigo',
                'concepto_ivacompra.tipoconcepto',
                'empresa.nombre',
                'cta_debe.nombre',
                'cta_haber.nombre',
            ];
