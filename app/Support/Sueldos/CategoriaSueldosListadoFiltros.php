...
/**
 * Filtros del listado de categorías de sueldos (index paginado).
 */
class CategoriaSueldosListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'categoria_sueldos.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'categoria_sueldos.codigo', 'type' => 'entero', 'label' => 'Código'],
        'descripcion' => ['column' => 'categoria_sueldos.descripcion', 'type' => 'texto', 'label' => 'Descripción'],
        'origen_bases' => ['column' => 'categoria_sueldos.origen_bases', 'type' => 'texto', 'label' => 'Origen de bases'],
    ];

    /** @var list<string> */
    private const COLUMNAS_COINCIDENCIA_FLEXIBLE = [
        'categoria_sueldos.descripcion',
    ];
    ...
    /**
     * @param  Builder<\App\Models\Sueldos\Categoria_Sueldos>  $query
     */
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                $q->whereNull('categoria_sueldos.descripcion')->orWhere('categoria_sueldos.descripcion', '');
            });
        ...
        $query->where(function ($q) use ($valor, $like, $id, $operador) {
            if ($id !== false) {
                $q->orWhere('categoria_sueldos.id', (int) $id);
                $q->orWhere('categoria_sueldos.codigo', (int) $id);
            }
            $q->orWhere('categoria_sueldos.descripcion', 'like', $like);
            ...
            self::aplicarCoincidenciaOrigen($q, $valor);
        });
    }
     ...
     * @param  Builder<\App\Models\Sueldos\Categoria_Sueldos>  $query
     */
    private static function aplicarCoincidenciaOrigen(Builder $query, string $valor): void
    {
        $codigo = self::codigoOrigenDesdeTexto($valor);
        if ($codigo !== null) {
            $query->orWhere('categoria_sueldos.origen_bases', $codigo);
        }
    }

    private static function codigoOrigenDesdeTexto(string $valor): ?string
    {
        $v = mb_strtolower(trim($valor));
        if ($v === '') {
            return null;
        }
        if (str_contains($v, 'emple')) {
            return CategoriaOrigenBases::EMPLEADO;
        }
        if (str_contains($v, 'categor') || str_contains($v, 'tabla')) {
            return CategoriaOrigenBases::TABLA;
        }
        if (in_array(mb_strtoupper($v), [CategoriaOrigenBases::TABLA, CategoriaOrigenBases::EMPLEADO], true)) {
    ...
    private static function aplicarOrigen(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                $q->whereNull('categoria_sueldos.origen_bases')->orWhere('categoria_sueldos.origen_bases', '');
            });
        ...
        if ($codigo !== null && in_array($operador, ['contiene', 'igual', 'empieza', 'termina'], true)) {
            $query->where('categoria_sueldos.origen_bases', $codigo);

            return;
        }

        self::aplicarTexto($query, 'categoria_sueldos.origen_bases', $operador, $valor);
    }
