...
/**
 * Filtros del listado de grupos de conceptos (index paginado + filtro externo empresa).
 */
class GrupoConceptoSueldosListadoFiltros
{
    ...
    public const CAMPOS = [
        'id' => ['column' => 'grupo_concepto_sueldos.id', 'type' => 'entero', 'label' => 'ID'],
        'codigo' => ['column' => 'grupo_concepto_sueldos.codigo', 'type' => 'entero', 'label' => 'Código'],
        'descripcion' => ['column' => 'grupo_concepto_sueldos.descripcion', 'type' => 'texto', 'label' => 'Descripción'],
        'origen' => ['column' => 'grupo_concepto_sueldos.origen', 'type' => 'texto', 'label' => 'Origen'],
        'empresa' => ['column' => 'empresa.nombre', 'type' => 'texto', 'label' => 'Empresa'],
    ];

    /** @var list<string> */
    private const COLUMNAS_COINCIDENCIA_FLEXIBLE = [
        'grupo_concepto_sueldos.descripcion',
        'empresa.nombre',
    ];
    ...
    /**
     * @param  Builder<\App\Models\Sueldos\Grupo_Concepto_Sueldos>  $query
     */
    public static function aplicar(Builder $query, array $filtros): void
    {
        if (! empty($filtros['empresa_id'])) {
            // Incluye grupos sin empresa (válidos para todas).
            $eid = (int) $filtros['empresa_id'];
            $query->where(function ($q) use ($eid) {
                $q->where('grupo_concepto_sueldos.empresa_id', $eid)
                    ->orWhereNull('grupo_concepto_sueldos.empresa_id');
            });
        }
    ...
    private static function aplicarBusquedaGlobal(Builder $query, string $operador, string $valor): void
    {
        if ($operador === 'vacio') {
            $query->where(function ($q) {
                $q->whereNull('grupo_concepto_sueldos.descripcion')->orWhere('grupo_concepto_sueldos.descripcion', '');
            });
        ...
        $query->where(function ($q) use ($valor, $like, $id) {
            if ($id !== false) {
                $q->orWhere('grupo_concepto_sueldos.id', (int) $id);
                $q->orWhere('grupo_concepto_sueldos.codigo', (int) $id);
            }
            $q->orWhere('grupo_concepto_sueldos.descripcion', 'like', $like);
            $q->orWhere('grupo_concepto_sueldos.origen', 'like', $like);
