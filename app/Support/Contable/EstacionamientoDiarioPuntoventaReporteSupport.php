...
use App\Services\Caja\Estacionamiento\EstacionamientoFacturacionService;
...
/**
 * Reporte Contable: ventas estacionamiento por jornada y punto de venta,
 * con apertura por medio de pago (cobros de facturas vs devoluciones NC).
 */
final class EstacionamientoDiarioPuntoventaReporteSupport
{
    ...
    public function generar(int $empresaId, string $fechaDesde, string $fechaHasta, ?int $puntoventaId = null): array
    {
        ...
        /** @var array<string, array<int, list<VentaEstacionamientoEmision>>> $porDiaPv */
        $porDiaPv = [];
    ...
    private function cargarEmisiones(int $empresaId, string $desde, string $hasta, ?int $puntoventaId): Collection
    {
        return VentaEstacionamientoEmision::query()
            ->whereHas('venta', function ($v) use ($empresaId, $desde, $hasta, $puntoventaId) {
                $v->where(function ($fecha) use ($desde, $hasta) {
                    ...
                    ->whereHas('puntoventas', function ($pv) use ($empresaId, $puntoventaId) {
    ...
    /**
     * @param  list<VentaEstacionamientoEmision>  $emisiones
     * @return array<string, mixed>
     */
    private function armarFilaPuntoventa(array $emisiones): array
    {
        $importeMinimo = EstacionamientoFacturacionService::IMPORTE_MINIMO_FACTURA;
        $ventaBruta = 0.0;
        $ventaNeto = 0.0;
        $ventaIva = 0.0;
        $totalNc = 0.0;
        ...
        foreach ($emisiones as $em) {
            ...
            $lineasMedios = EstacionamientoVentaDetalleSupport::mediosPagoPorCobranza($cobranzas);
            $cobradoVenta = 0.0;
