...
/**
 * Backfill: centro de costo en asientos del cierre contable estacionamiento (ERP + Anita ctamov).
 */
final class CorregirCentrocostoAsientoCierreEstacionamientoSupport
{
    /**
    ...
    {
        $ids = $this->asientoIdsPorFecha($fecha, $empresaId);
        if ($ids === []) {
            return collect();
        }
    ...
    {
        $rendiciones = $this->queryRendicionesCerradas($fecha, $empresaId)->get();
     ...
     * @return list<int>
     */
    public function asientoIdsPorFecha(string $fecha, ?int $empresaId = null): array
    {
        $idsRendicion = $this->queryRendicionesCerradas($fecha, $empresaId)
            ->pluck('asiento_id')
            ->map(fn ($id) => (int) $id)
            ->filter(fn (int $id) => $id > 0);
        ...
        $obsQuery = Asiento::query()
            ->where('observacion', 'like', CierreRendicionEstacionamientoAsientoSupport::DESCRIPCION_ASIENTO.' — '.$fechaFmt.'%');
    ...
    public function requiereActualizacionLinea(Asiento_Movimiento $linea, array $config): bool
    {
        $ccObjetivo = CierreRendicionEstacionamientoCentrocostoSupport::resolverCentrocostoIdParaCuentacontable(
    ...
    public function lineaLlevaCentrocosto(Asiento_Movimiento $linea, array $config): bool
    {
        return CierreRendicionEstacionamientoCentrocostoSupport::resolverCentrocostoIdParaCuentacontable(
            (int) ($linea->cuentacontable_id ?? 0),
            $config,
        ) !== null;
    }
    ...
    private function queryRendicionesCerradas(string $fecha, ?int $empresaId = null)
    {
        $query = RendicionEstacionamientoCaja::query()
            ->whereNotNull('asiento_id')
            ->where('asiento_id', '>', 0)
            ->where(function ($q) use ($fecha) {
                $q->whereDate('cierre_contable_en', $fecha)
                    ->orWhereDate('fecharendicion', $fecha);
            });
    ...
    private function ejecutarSobreColeccion(Collection $asientos, bool $dryRun): array
    {
        ...
        $codigoCc = CierreRendicionEstacionamientoCentrocostoSupport::codigoCentrocostoConfigurado();
        /** @var array<int, array<string, mixed>> $configPorEmpresa */
        $configPorEmpresa = [];

        foreach ($asientos as $asiento) {
            $empresaId = (int) $asiento->empresa_id;
            if (! isset($configPorEmpresa[$empresaId])) {
                $configPorEmpresa[$empresaId] = CierreRendicionEstacionamientoConfigSupport::paraEmpresa($empresaId);
            }
            $config = $configPorEmpresa[$empresaId];
            ...
            foreach ($lineas as $linea) {
                if (! $this->requiereActualizacionLinea($linea, $config)) {
                    continue;
                }
    ...
    private function contarLineasAnitaPendientes(
        ...
        foreach ($lineasErp->values() as $idx => $linea) {
            if (! $this->lineaLlevaCentrocosto($linea, $config)) {
                continue;
            }
    ...
    private function actualizarLineasAnita(
        ...
        foreach ($lineasErp->values() as $idx => $linea) {
            ...
            ], 'ctamov update ccosto cierre estacionamiento');
