...
class PuntoventaListadoExport implements FromView, ShouldAutoSize, WithColumnFormatting, WithColumnWidths, WithEvents, WithStyles, WithTitle
{
    use Exportable;
    ...
    public function view(): View
    {
        if ($this->flDesdeIndex) {
            $datas = $this->repository->leePuntoventa($this->filtros, false);
            ...
            return view('exports.ventas.puntoventaindex', [
                'datas' => $datas,
    ...
    /**
     * @param  \Illuminate\Support\Collection<int, \App\Models\Ventas\Puntoventa>|\Illuminate\Database\Eloquent\Collection  $datas
     */
    private static function enriquecerNombreEmpresa($datas): void
    {
        foreach ($datas as $row) {
            $row->nombreempresa = (string) ($row->empresas->nombre ?? config('app.empresa'));
        }
    }
}
