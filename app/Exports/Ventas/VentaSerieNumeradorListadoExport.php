...
class VentaSerieNumeradorListadoExport implements FromView, ShouldAutoSize, WithColumnFormatting, WithColumnWidths, WithEvents, WithStyles, WithTitle
{
    use Exportable;

    private const COL_ULTIMA = 'I';

    private VentaSerieNumeradorRepositoryInterface $repository;
    ...
    public function __construct(VentaSerieNumeradorRepositoryInterface $repository)
    {
    ...
    public function view(): View
    {
        if ($this->flDesdeIndex) {
            $datas = $this->repository->leeVentaSerieNumerador($this->filtros, false);
            foreach ($datas as $row) {
                $row->nombreempresa = (string) ($row->empresa->nombre ?? $row->puntoventa->empresa->nombre ?? config('app.empresa'));
            }
            ...
            return view('exports.ventas.venta_serie_numeradorindex', [
                'datas' => $datas,
