...
class ParametroSueldosListadoExport implements FromView, ShouldAutoSize, WithColumnFormatting, WithColumnWidths, WithEvents, WithStyles, WithTitle
{
    use Exportable;
    ...
    public function view(): View
    {
        if ($this->flDesdeIndex) {
            $datas = $this->repository->leeParametro($this->filtros, false);
            ...
            return view('exports.sueldos.parametroindex', [
                'datas' => $datas,
    ...
    {
        return 'Parámetros de liquidación';
    }
    ...
    public static function valorVigenteEtiqueta(Parametro_Sueldos $parametro): string
    {
        $ultimo = $parametro->valores->first();
        if ($ultimo === null) {
            return '';
        }

        if (($parametro->tipo ?? 'numero') === 'texto') {
            return (string) ($ultimo->valor_texto ?? '');
        }

        return $ultimo->valor !== null ? (string) $ultimo->valor : '';
    }
}
