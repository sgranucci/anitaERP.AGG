...
class MaquinavendingDiarioPuntoventaExport implements FromView, WithColumnFormatting, WithColumnWidths, WithEvents, WithStyles, WithTitle
{
    ...
    public function view(): View
    {
        ...
        return view('contable.cierre_rendicion_maquinavending.diario_puntoventa_listado', [
    ...
    /**
     * Matriz a lo ancho: FECHA + por cada PV (medios + Venta total), una fila por jornada.
     * Vending no maneja notas de crédito: la venta total ya incluye lo que hubiera.
     *
     * @param  array<string, mixed>  $resultado
     * @return array{
    ...
    public static function matrizAncha(array $resultado): array
    {
        ...
        uasort($pvMap, static fn (array $a, array $b): int => strcmp($a['pv_codigo'], $b['pv_codigo']));
        ...
        foreach ($pvMap as $pv) {
            ...
            $labelsFila2[] = 'Venta total';

            $colsBloque = count($medios) + 1; // medios + Venta total
            $bloquesPv[] = [
        ...
        $labelsFila2[] = 'Venta total';
        $colsTotalDia = count($mediosTotales) + 1;
        $bloquesPv[] = [
            'puntoventa_id' => 0,
            'pv_codigo' => '',
            'pv_nombre' => '',
        ...
        foreach ($resultado['dias'] ?? [] as $dia) {
            ...
            foreach ($bloquesPv as $bloque) {
                ...
                $totalPv = round((float) ($pvDia['venta_bruta'] ?? 0), 2);
                $valores[] = $totalPv;
                $ventaTotalDia += $totalPv;
            }
            ...
            $valores[] = round((float) ($totalesDia['venta_bruta'] ?? $ventaTotalDia), 2);
