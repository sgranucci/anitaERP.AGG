...
class IngresoProveedorControlListadoExport implements FromView, ShouldAutoSize, WithColumnFormatting, WithColumnWidths, WithEvents, WithStyles, WithTitle
{
    use Exportable;
    ...
    public function view(): View
    {
        ...
        if ($this->flDesdeIndex) {
            $personas = IngresoProveedorControlSupport::grillaDelDia($this->filtros);
            $filas = $personas
                ->map(fn ($p) => IngresoProveedorControlSupport::payloadPersona($p))
                ->values();
            $paraLogos = $personas->map(function ($p) {
                $ingreso = $p->ingreso;
                if ($ingreso) {
                    $ingreso->nombreempresa = $ingreso->empresas->nombre ?? '';
                }
        ...
        return view('exports.seguridad.ingreso_proveedor_controlindex', [
            'filas' => $filas,
