...
class AperturaGastoListadoExport implements FromView, ShouldAutoSize, WithColumnFormatting, WithColumnWidths, WithEvents, WithStyles, WithTitle
{
    use Exportable;

    private const COL_ULTIMA = 'H';

    private AperturaGastoRepositoryInterface $repository;
    ...
    public function __construct(AperturaGastoRepositoryInterface $repository)
    {
    ...
    public function view(): View
    {
        if ($this->flDesdeIndex) {
            $datas = $this->repository->leeAperturaGasto($this->filtros, false);
            ...
            return view('exports.caja.apertura_gastoindex', [
                'datas' => $datas,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle('C'.$primera.':D'.$sheet->getHighestRow())
                    ->getAlignment()
                    ->setWrapText(true)
    ...
    /**
     * @param  \Illuminate\Support\Collection<int, \App\Models\Caja\AperturaGasto>|\Illuminate\Database\Eloquent\Collection  $datas
     */
    private static function enriquecerNombreEmpresa($datas): void
    {
        foreach ($datas as $row) {
            $row->nombreempresa = $row->empresas_resumen ?: '';
        }
    }
}
