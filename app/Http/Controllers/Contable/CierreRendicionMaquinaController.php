...
class CierreRendicionMaquinaController extends Controller
{
    public function __construct(
        private readonly CierreRendicionMaquinaService $service,
    ...
    public function index(Request $request)
    {
        ...
        return view('contable.cierre_rendicion_maquina.index', [
            'grupos' => $grupos,
            'filtros' => $filtros,
            'filtrosQuery' => CierreRendicionMaquinaListadoFiltros::paraQueryString($filtros),
            'camposFiltro' => CierreRendicionMaquinaListadoFiltros::CAMPOS,
    ...
    public function listar(Request $request, ?string $formato = null, ?string $busqueda = null)
    {
        ...
        switch ($formato) {
            case 'PDF':
                $view = \View::make('contable.cierre_rendicion_maquina.listado', compact('rendiciones'))->render();
                $path = storage_path('pdf/listados');
                $nombrePdf = 'listado_cierre_rendicion_maquina';
            ...
            case 'CSV':
                ...
                return \Maatwebsite\Excel\Facades\Excel::download(
                    new CierreRendicionMaquinaListadoExport($rendiciones, $formato === 'CSV'),
                    'cierre_rendicion_maquina.'.$ext,
                    $mime,
                );
        }
        ...
        return redirect()->route(
            ...
            CierreRendicionMaquinaListadoFiltros::paraQueryString($filtros),
        );
    }
    ...
    public function conciliacionFlash(Request $request)
    {
        ...
        return view('contable.cierre_rendicion_maquina.conciliacion_flash', [
    ...
    public function listarConciliacionFlash(Request $request, ?string $formato = null)
    {
        ...
        if ($empresaId <= 0 || $fechaDesde === '' || $fechaHasta === '') {
            return redirect()
                ->route('cierre_rendicion_maquina_conciliacion_flash', $redirectQuery)
        ...
        switch ($formato) {
            case 'PDF':
                $view = \View::make('contable.cierre_rendicion_maquina.conciliacion_flash_listado', [
                    'resultado' => $resultado,
                    'esExcel' => false,
                    'filas' => CierreRendicionMaquinaConciliacionFlashExport::aplanarFilas($resultado),
                ])->render();
                $path = storage_path('pdf/listados');
                if (! is_dir($path)) {
                    mkdir($path, 0755, true);
                }
                $nombrePdf = 'listado_conciliacion_flash_maquina';
            ...
            case 'CSV':
                ...
                return \Maatwebsite\Excel\Facades\Excel::download(
                    new CierreRendicionMaquinaConciliacionFlashExport($resultado, $formato === 'CSV'),
                    'conciliacion_flash_maquina.'.$ext,
                    $mime,
                );
        }
    ...
    {
        can('ejecutar-cierre-rendicion-maquina-contable');
    ...
    {
        can('anular-cierre-rendicion-maquina-contable');
    ...
    private function resolverFiltrosListado(Request $request, ?string $busquedaRuta = null): array
    {
        ...
        $filtros = CierreRendicionMaquinaListadoFiltros::resolverDesdeRequest(
            $request,
            $busquedaRuta,
            $empresaDefault,
        );
    ...
    private function resolverRetornoListadoQuery(Request $request): array
    {
        ...
        return QueryRetornoListado::desdeRequest($request, CierreRendicionMaquinaListadoFiltros::class);
    }
}
