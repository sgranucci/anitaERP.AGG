...
class Tipotransaccion_StockController extends Controller
{
    ...
    public function consultaTipotransaccionStock(Request $request)
    {
        ...
        $query = Tipotransaccion_Stock::query()
            ->select('id', 'abreviatura', 'nombre', 'operacion', 'maneja_contabilidad', 'origen_bien_uso', 'destino_bien_uso', 'requiere_aprobacion', 'aviso_opcional', 'baja_npu', 'alta_npu', 'pide_precio')
            ->where('estado', 'A')
        ...
        if ($data->isEmpty()) {
            ...
            foreach ($data as $row) {
                ...
                $output['data'] .= '<td class="pide-precio d-none">'.e($row->pide_precio ? '1' : '0').'</td>';
    ...
    public function leeUnTipotransaccionPorAbreviatura(Request $request, string $abreviatura)
    {
        ...
        return response()->json([
            ...
            'pide_precio' => (bool) $tipo->pide_precio,
        ]);
    }
    ...
    private function datosNormalizados(ValidacionTipotransaccion_Stock $request): array
    {
        ...
        $data['pide_precio'] = $request->boolean('pide_precio');
