...
use App\Models\Sueldos\Cierrefallo_Sueldos;
...
class DtoFalloProcesoService
{
     ...
     * }  $input
     * @return array{cierre: Cierrefallo_Sueldos, resumen: array<string, mixed>}
     */
    public function generar(array $input): array
    {
        ...
        return DB::transaction(function () use (
            ...
            $cierre = Cierrefallo_Sueldos::create([
                'nro_cierre' => $this->siguienteNroCierre(),
                ...
                'usuario_id' => Auth::id(),
                'estado' => Cierrefallo_Sueldos::ESTADO_GENERADO,
                'observacion' => sprintf(
                    'Proceso dto. fallo %s a %s · período %s',
                    $desde->format('d/m/Y'),
                    $hasta->format('d/m/Y'),
                    $periodo
                ),
    ...
    public function anularCierre(int $cierreId): Cierrefallo_Sueldos
    {
        return DB::transaction(function () use ($cierreId) {
            /** @var Cierrefallo_Sueldos $cierre */
            $cierre = Cierrefallo_Sueldos::query()->lockForUpdate()->findOrFail($cierreId);
            if ($cierre->estaAnulado()) {
                return $cierre;
            }

            $movs = Dtofallo_Sueldos::query()
                ->where('cierrefallo_id', $cierre->id)
                ->get();

            foreach ($movs as $mov) {
                if ($mov->novedad_id && Schema::hasColumn('novedad_sueldos', 'dtofallo_id')) {
                    Novedad_Sueldos::query()
                        ...
                        ->update([
                            ...
                            'observacion' => trim((string) ($mov->observacion ?? '')).' · Anulado cierre #'.$cierre->nro_cierre,
                        ]);
                }
                $mov->delete();
            }
            ...
            $cierre->update([
                'estado' => Cierrefallo_Sueldos::ESTADO_ANULADO,
    ...
    private function procesarEmpleado(
        ...
        $totDescuentoPrevio = (float) Dtofallo_Sueldos::query()
            ->where('empleado_sueldos_id', $empleado->id)
            ->where('tipo_oper', DtoFalloTipoOper::DESCUENTO)
        ...
        if ($totADescontar > 0.009) {
            ...
            while (abs($totDescontado - $totADescontar) > 1 && $totADescontar > 0) {
                ...
                $yaDescontadoMes = (float) Dtofallo_Sueldos::query()
                    ->where('empleado_sueldos_id', $empleado->id)
                    ->where('tipo_oper', DtoFalloTipoOper::DESCUENTO)
    ...
    private function generarNovedad(
        ...
        if ($mov->tipo_oper !== DtoFalloTipoOper::DESCUENTO) {
            return false;
        }

        $existente = Novedad_Sueldos::query()->where('dtofallo_id', $mov->id)->first();
        ...
        $payload = [
            ...
            'periodo' => $periodo,
            'origen' => NovedadSueldosCatalogo::ORIGEN_DTOFALLO,
            'dtofallo_id' => (int) $mov->id,
            'usuario_id' => Auth::id(),
    ...
    private function siguienteNroCierre(): int
    {
        $ultimo = (int) (Cierrefallo_Sueldos::query()->max('nro_cierre') ?? 0);
