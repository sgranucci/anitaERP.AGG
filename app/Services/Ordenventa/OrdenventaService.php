<?php
namespace App\Services\Ordenventa;

use Symfony\Component\Process\Process;
use Symfony\Component\Process\Exception\ProcessFailedException;
use App\Repositories\Ordenventa\OrdenventaRepositoryInterface;
use App\Repositories\Ordenventa\Ordenventa_EstadoRepositoryInterface;
use App\Repositories\Ordenventa\Ordenventa_ArchivoRepositoryInterface;
use App\Repositories\Ordenventa\Ordenventa_CuotaRepositoryInterface;
use App\Repositories\Ordenventa\Ordenventa_ConceptoRepositoryInterface;
use App\Repositories\Ventas\VentaRepositoryInterface;
use App\Services\Configuracion\ArbolaprobacionService;
use App\Models\Ordenventa\Ordenventa_Estado;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Str;
use Carbon\Carbon;
use App;
use Auth;
use DB;
use Exception;

class OrdenventaService 
{
	private $ordenventaRepository;
    private $ordenventa_estadoRepository;
    private $ordenventa_archivoRepository;
	private $ordenventa_cuotaRepository;
	private $ordenventa_conceptoRepository;
	private $ventaRepository;
	private $arbolaprobacionService;

    public function __construct(OrdenventaRepositoryInterface $ordenventarepository,
                                Ordenventa_EstadoRepositoryInterface $ordenventa_estadorepository,
                                Ordenventa_ArchivoRepositoryInterface $ordenventa_archivorepository,
								Ordenventa_CuotaRepositoryInterface $ordenventa_cuotarepository,
								Ordenventa_ConceptoRepositoryInterface $ordenventa_conceptorepository,
								VentaRepositoryInterface $ventarepository,
								ArbolaprobacionService $arbolaprobacionservice
								)
    {
		$this->ordenventaRepository = $ordenventarepository;
        $this->ordenventa_estadoRepository = $ordenventa_estadorepository;
        $this->ordenventa_archivoRepository = $ordenventa_archivorepository;
		$this->ordenventa_cuotaRepository = $ordenventa_cuotarepository;
		$this->ordenventa_conceptoRepository = $ordenventa_conceptorepository;
		$this->ventaRepository = $ventarepository;
		$this->arbolaprobacionService = $arbolaprobacionservice;
    }

	public function guardaOrdenventa($request, $origen = null)
	{
		$data = $request->all();

   		// Crea estado
	   	$data['fechas'][] = Carbon::now();
	   	$data['estados'][] = Ordenventa_Estado::$enumEstado[array_search('S', array_column(Ordenventa_Estado::$enumEstado, 'valor'))]['nombre'];
		$data['usuario_ids'][] = Auth::user()->id;
	   	$data['observacionestados'][] = "Alta de Orden de Venta";

		$data['creousuario_id'] = Auth::user()->id;

		DB::beginTransaction();
		try
		{
			$ordenventa = $this->ordenventaRepository->create($data);

			if ($ordenventa == 'Error')
				throw new Exception('Error en grabacion');

			// Guarda tablas asociadas
			if ($ordenventa)
				Self::agrega($data, $ordenventa, $request);

			DB::commit();
		} catch (\Exception $e) {
			DB::rollback();
			dd($e->getMessage());
			return ['mensaje' => 'error', 'errores' => $e->getMessage()];
		}
        return ['mensaje' => 'ok'];
	}

	// Agrega tablas asociadas
	private function agrega($data, $ordenventa, $request)
	{
		$ordenventa_estado = $this->ordenventa_estadoRepository->create($data, $ordenventa->id);
		$ordenventa_archivo = $this->ordenventa_archivoRepository->create($request, $ordenventa->id);

		// Si existen las tareas asume que graba desde administracion de ordenventas
		if (isset($data['montofacturas']))
			$ordenventa_cuota = $this->ordenventa_cuotaRepository->create($data, $ordenventa->id);

		// Si existen las tareas asume que graba desde administracion de ordenventas
		if (isset($data['montoconceptos']))
			$ordenventa_concepto = $this->ordenventa_conceptoRepository->create($data, $ordenventa->id);

		// Llama al arbol de aprobacion
		$this->arbolaprobacionService->procesaArbolaprobacion('OV', $ordenventa->id, 'insert');
	}

    public function actualizaOrdenventa($request, $id, $origen = null)
    {
		$data = $request->all();

		DB::beginTransaction();
		try
		{
			Self::actualiza($data, $id, $request);

			DB::commit();
		} catch (\Exception $e) {
			DB::rollback();

			return ['mensaje' => 'error', 'errores' => $e->getMessage()];
		}
        return ['mensaje' => 'ok'];
    }

	private function actualiza($data, $id, $request)
	{
		// Graba ordenventa
		$ordenventa = $this->ordenventaRepository->update($data, $id);

		if ($ordenventa === 'Error')
			throw new Exception('Error en grabacion ordenventa.');

		// Graba movimientos de estados y archivos
		$this->ordenventa_archivoRepository->update($request, $id);
		$this->ordenventa_cuotaRepository->update($data, $id);
		$this->ordenventa_conceptoRepository->update($data, $id);

		// Llama al arbol de aprobacion
		$this->arbolaprobacionService->procesaArbolaprobacion('OV', $id, 'insert');
	}

	public function actualizaSoloOrdenventa($estado, $id)
	{
		DB::beginTransaction();
		try
		{
			$ordenventa = $this->ordenventaRepository->update($estado, $id);

			// Crea estado
			if (isset($estado['estadoordenventa']))
			{
				$data = [];
				$data['fechas'][] = Carbon::now();
				$data['usuario_ids'][] = Auth::user()->id;

				if ($estado['estadoordenventa'] == 'ANULADA')
				{
					$data['observacionestados'][] = "Anulación de Orden de Venta";
					$data['estados'][] = Ordenventa_Estado::$enumEstado[array_search('A', array_column(Ordenventa_Estado::$enumEstado, 'valor'))]['nombre'];
				}
				else
				{
					$data['observacionestados'][] = "Activación de Orden de Venta";
					$data['estados'][] = Ordenventa_Estado::$enumEstado[array_search('P', array_column(Ordenventa_Estado::$enumEstado, 'valor'))]['nombre'];
				}

				$data['creousuario_id'][] = Auth::user()->id;

				$ordenventa_estado = $this->ordenventa_estadoRepository->create($data, $id);
			}			

			DB::commit();
		} catch (\Exception $e) {
			DB::rollback();

			return ['mensaje' => 'error', 'errores' => $e->getMessage()];
		}		
	}

	public function leeHistoriaOrdenventa($ordenventa_id)
	{
		return $this->ordenventa_estadoRepository->leeHistoriaOrdenventa($ordenventa_id);
	}

	public function leeOrdenVenta($ordenventa_id)
	{
        return $this->ordenventaRepository->find($ordenventa_id);
	}

	public function marcaOrdenVentaFacturada($ordenventa_id, $tipo, $letra, $sucursal, $numero, $venta_id, $ordenventa_cuota_id)
	{
		// Crea estado
		$data = [];
	   	$data['fechas'][] = Carbon::now();
	   	$data['estados'][] = Ordenventa_Estado::$enumEstado[array_search('F', array_column(Ordenventa_Estado::$enumEstado, 'valor'))]['nombre'];
		$data['usuario_ids'][] = Auth::user()->id;
	   	$data['observacionestados'][] = "Orden de Venta Facturada ($tipo $letra $sucursal-$numero)";
		$data['creousuario_id'] = Auth::user()->id;

		$ordenventa_estado = $this->ordenventa_estadoRepository->create($data, $ordenventa_id);

		// Lee las cuotas
		$flPendiente = false;
		if ($ordenventa_cuota_id > 0)
		{
			$ordenventa_cuota = $this->ordenventa_cuotaRepository->findPorOrdenventa($ordenventa_id);

			foreach($ordenventa_cuota as $cuota)
			{
				if ($cuota->id == $ordenventa_cuota_id)
				{
					$ovcta = $this->ordenventa_cuotaRepository->update(["venta_id" => $venta_id, $ordenventa_cuota_id]);
					$cuota->venta_id = $venta_id;
				}
				if ($cuota->venta_id == null)
					$flPendiente = true;
			}
		}

		$ordenventa = $this->ordenventaRepository->update(['estado' => ($flPendiente ? 'FACTPARCIAL' : 'FACTURADA')], $ordenventa_id);
	}

	public function anulaMarcaOrdenVentaFacturada($ordenventa_id, $tipo, $letra, $sucursal, $numero, $referenciaventa_id)
	{
		// Crea estado
		$data = [];
	   	$data['fechas'][] = Carbon::now();
	   	$data['estados'][] = Ordenventa_Estado::$enumEstado[array_search('N', array_column(Ordenventa_Estado::$enumEstado, 'valor'))]['nombre'];
		$data['usuario_ids'][] = Auth::user()->id;
	   	$data['observacionestados'][] = "Nota de Crédito ($tipo $letra $sucursal-$numero) Cancela OV";
		$data['creousuario_id'] = Auth::user()->id;

		$ordenventa_estado = $this->ordenventa_estadoRepository->create($data, $ordenventa_id);

		// Lee las cuotas
		//$flPendiente = true;
		//$ordenventa_cuota = $this->ordenventa_cuotaRepository->findPorOrdenventa($ordenventa_id);
		//foreach($ordenventa_cuota as $cuota)
		//{
			// Si encuentra la cuota facturada la desmarca
		//	if ($cuota->venta_id == $referenciaventa_id)
		//	{
		//		$ovcta = $this->ordenventa_cuotaRepository->update(["venta_id" => null], $cuota-id);
		//		$cuota->venta_id = null;
		//	}
		//	if ($cuota->venta_id != null)
		//		$flPendiente = false;
		//}

		//$ordenventa = $this->ordenventaRepository->update(['estado' => ($flPendiente ? 'PENDIENTE' : 'FACTPARCIAL')], $ordenventa_id);
		$ordenventa = $this->ordenventaRepository->update(['estado' => 'CANCELADA | NOTA DE CREDITO'], $ordenventa_id);
	}	


	public function leeComprobantesOrdenventa($ordenventa_id)
	{
		return $this->ventaRepository->leeComprobantePorOrdenVenta($ordenventa_id);
	}

	public function marcaOrdenVentaCobrada($ordenventa_id)
	{
		// Lee los comprobantes de la orden de venta
		$comprobantes = $this->ventaRepository->leeComprobantePorOrdenVenta($ordenventa_id);

		// Verifica si hay algun comprobante impago de la orden de venta
		$flPendiente = false;
		foreach($comprobantes as $comprobante)
		{
			if (abs($comprobante->aplicado) < $comprobante->total)
				$flPendiente = true;
		}

		if (!$flPendiente)
			$ordenventa = $this->ordenventaRepository->update(['estado' => 'COBRADA'], $ordenventa_id);
	}
}
