...
final class GastronomiaFacturaMedioPagoService
{
     ...
     *     }>
     *   }>,
     *   cuentacajas?: list<array{id:int,codigo:string,nombre:string,moneda_id:int}>
     * }
     */
    public function datosParaCambio(int $ventaId): array
    {
        ...
        return [
            ...
            'cuentas_caja' => $this->listarCuentasCajaGastronomia($empresaId),
        ];
    }
    ...
    public function cuentaPorCodigo(int $ventaId, string $codigo): array
    {
        $venta = $this->resolverVentaGastronomia($ventaId);
        if ($venta === null) {
            return ['id' => 0, 'error' => 'La venta no corresponde a una emisión gastronomía.'];
        }

        $empresaId = (int) ($venta->puntoventas?->empresa_id ?? 0);
        $usoId = $this->usoCuentacajaGastronomiaId();
        if (! $usoId) {
            return ['id' => 0, 'error' => 'Uso de cuenta de caja gastronomía no configurado.'];
        }
        ...
        if ($cuenta === null) {
            return [
                'id' => 0,
                'error' => 'No existe cuenta de caja con uso Gastronomía para ese código.',
            ];
        }
    ...
    public function aplicarCambio(int $ventaId, array $cambios): array
    {
        ...
        $empresaId = (int) ($venta->puntoventas?->empresa_id ?? 0);
        $cobranzaIds = GastronomiaVentaDetalleSupport::cobranzasDeVenta($venta)->pluck('id')->map(fn ($id) => (int) $id)->all();
    ...
    private function resolverVentaGastronomia(int $ventaId): ?Venta
    {
     ...
     * }>
     */
    private function listarCuentasCajaGastronomia(int $empresaId): array
    {
        if ($empresaId <= 0) {
            return [];
        }
