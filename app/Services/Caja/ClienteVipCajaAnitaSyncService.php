...
use App\Support\Caja\ClivipAnitaBridgeSupport;
use App\Support\Caja\ClivipFieldMapper;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class ClienteVipCajaAnitaSyncService
{
    /**
    ...
    public function sincronizarEmpresaDesdeAnita(int $empresaId): array
    {
        ...
        $lista = ClivipAnitaBridgeSupport::listar($empresaId);
        $ret['en_anita'] = count($lista);

        foreach ($lista as $row) {
            $numeroid = ClivipFieldMapper::mapNumeroid($row);
            if ($numeroid === null) {
                $ret['omitidos']++;
            ...
            try {
                ...
                $msg = "Cliente VIP caja Anita empresa {$empresaId} numeroid={$numeroid}: ".$e->getMessage();
                $ret['errores'][] = $msg;
                Log::warning('ClienteVipCajaAnitaSync: '.$msg, ['exception' => $e]);
            }
        }
    ...
    {
        $filas = ClivipAnitaBridgeSupport::listar($empresaId, ' WHERE inumeroid = '.(int) $numeroid);
        if ($filas === []) {
            return 'omitido';
        }
    ...
    {
        $orden = (array) config('caja.cliente_vip_anita_empresas_sync', [1, 2, 3]);
    ...
    {
        $payload = ClivipFieldMapper::mapAll($row, $empresaId);
        $this->validarPayloadMinimo($payload, $empresaId, $numeroid);

        $existente = ClienteVipCaja::query()
        ...
        try {
            ...
            ClienteVipCaja::query()->create($payload);
            DB::commit();
