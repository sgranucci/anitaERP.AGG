...
final class RendicionGastronomiaAuditoriaAnitaService
{
    ...
    public function auditarFechaJornada(
        ...
        foreach ($puntosVenta as $pv) {
            ...
            if ($estado === 'excluido_estacionamiento') {
                $conteo['excluido_estacionamiento'] = ($conteo['excluido_estacionamiento'] ?? 0) + 1;
            } elseif (isset($conteo[$estado])) {
                $conteo[$estado]++;
            }
        }
        ...
        return [
            ...
            'resumen' => [
                ...
                'filtro_erp' => 'venta.fechajornada + venta_gastronomia_emision (facturas sin NC en Z; NC en tot_nc)',
    ...
    private function auditarPuntoventa(
        ...
        $erpZ = GastronomiaTurnoOperativoTotalesSupport::totalFacturasSinNotasCreditoPorPuntoventa(
            (int) $pv->id,
            $empresaId,
            $fechaJornada,
        );
        $erpNc = GastronomiaTurnoOperativoTotalesSupport::totalNotasCreditoPorPuntoventa(
            (int) $pv->id,
            $empresaId,
            $fechaJornada,
        );
        $totalesErp = GastronomiaTurnoOperativoTotalesSupport::totalesDiaPorPuntoventa(
            (int) $pv->id,
            $empresaId,
            $fechaJornada,
        );
        ...
        if ($this->esPuntoventaSoloAnita($pv) && $erpZ <= $tolerancia && $erpNc <= $tolerancia) {
            return [
                'puntoventa' => $pv->codigo,
                'sucursal' => $sucursal,
                ...
                'cantidad_nc_erp' => $cantNc,
                'cabeceras' => 0,
                'mensaje' => 'Estacionamiento: factura en Anita, sin comparar con ERP gastronomía',
            ];
        }
        ...
        if ($this->esPuntoventaSoloAnita($pv) && $erpZ <= $tolerancia && $erpNc <= $tolerancia) {
            ...
            return [
                ...
                'erp_z' => $erpZ,
                'erp_nc' => $erpNc,
                'anita_z' => round((float) ($portadora->rendg_total_z ?? 0), 2),
                'anita_nc' => round((float) ($portadora->rendg_tot_nc ?? 0), 2),
                'diff_z' => null,
                'diff_nc' => null,
                'cantidad_facturas_erp' => $cantFacturas,
                'cantidad_nc_erp' => $cantNc,
    ...
    private function esPuntoventaSoloAnita(Puntoventa $pv): bool
    {
        $codigos = config('rendicion_gastronomia_anita.auditoria_diaria.puntoventa_codigos_solo_anita', []);
        if (in_array(trim((string) $pv->codigo), $codigos, true)) {
            return true;
        }
