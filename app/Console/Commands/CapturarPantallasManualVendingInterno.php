...
class CapturarPantallasManualVendingInterno extends Command
{
    protected $signature = 'manual:capturar-vending-interno
                            {--usuario=admin : Usuario ERP}
                            {--rol= : ID de rol (opcional)}';

    protected $description = 'Captura pantallas del módulo Vending (render interno + Ghostscript)';
    ...
    {
        $maquinaId = (int) (DB::table('maquinavending')->orderBy('id')->value('id') ?? 0);
        $rendicionId = (int) (DB::table('maquinavending_rendicion')->orderBy('id')->value('id') ?? 0);
        $cajaRow = DB::table('rendicion_maquinavending_caja')->orderBy('id')->first(['id', 'caja_id']);
        $cajaId = (int) ($cajaRow->id ?? 0);
        $cajaAsignadaId = (int) ($cajaRow->caja_id ?? 0);

        $this->pantallas = [
            'maquinas-listado' => ['path' => '/ventas/gastronomia/maquinas-vending', 'auth' => true],
            'maquinas-form' => [
                'path' => $maquinaId > 0
                    ? '/ventas/gastronomia/maquinas-vending/' . $maquinaId . '/editar'
                    : '/ventas/gastronomia/maquinas-vending/crear',
                'auth' => true,
            ],
            'rendicion-ventas-listado' => ['path' => '/ventas/gastronomia/maquinas-vending/rendiciones', 'auth' => true],
            'rendicion-ventas-form' => ['path' => '/ventas/gastronomia/maquinas-vending/rendiciones/crear', 'auth' => true],
            'rendicion-ventas-comprobante' => [
                'path' => $rendicionId > 0
                    ? '/ventas/gastronomia/maquinas-vending/rendiciones/' . $rendicionId . '/comprobante?inline=1'
                    : '/ventas/gastronomia/maquinas-vending/rendiciones/crear',
                'auth' => true,
            ],
            'caja-listado' => ['path' => '/caja/rendicionmaquinavending', 'auth' => true],
            'caja-form' => [
                'path' => $cajaAsignadaId > 0
                    ? '/caja/rendicionmaquinavending/crear/' . $cajaAsignadaId
                    : '/caja/rendicionmaquinavending/crear',
                'auth' => true,
            ],
            'caja-editar' => [
                'path' => $cajaId > 0
                    ? '/caja/rendicionmaquinavending/' . $cajaId . '/editar'
                    : '/caja/rendicionmaquinavending/crear',
                'auth' => true,
            ],
            'caja-comprobante' => [
                'path' => $cajaId > 0
                    ? '/caja/rendicionmaquinavending/' . $cajaId . '/imprimir?inline=1'
                    : '/caja/rendicionmaquinavending/crear',
                'auth' => true,
            ],
        ];

        $outDir = public_path('docs/manual-vending/img');
        ...
        $this->info('Capturando Vending → ' . $outDir);
        $this->info('Máquina ID: ' . ($maquinaId ?: '—') . ' | Rendición: ' . ($rendicionId ?: '—') . ' | Caja: ' . ($cajaId ?: '—'));
        ...
        $this->info('Listo. Ejecute: php docs/manual-vending/generar.php');
    ...
    private function htmlAPng(string $html, string $pngPath): void
    {
        ...
        $tmpPdf = storage_path('app/tmp-manual-vending-captura.pdf');
        Pdf::loadHTML($html)
    ...
    private function prepararHtmlParaCaptura(string $html): string
    {
        ...
        return '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">'
            ...
            . '.modal.show{display:block!important;opacity:1!important;position:relative!important;}'
