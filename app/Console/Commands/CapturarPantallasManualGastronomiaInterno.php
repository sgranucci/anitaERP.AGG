...
class CapturarPantallasManualGastronomiaInterno extends Command
{
    protected $signature = 'manual:capturar-gastronomia-interno
                            {--usuario=admin : Usuario ERP}
                            {--rol= : ID de rol (opcional)}
                            {--empresa= : empresa_id para query string (default: primera con PV gastronomía)}';

    protected $description = 'Captura pantallas reales del módulo Gastronomía (render interno + Ghostscript)';
    ...
    public function handle(): int
    {
        ...
        if ($empresaId <= 0) {
            $empresaId = (int) (DB::table('configuracion_puntoventa_gastronomia')->value('empresa_id') ?: 1);
        }

        $q = '?empresa_id=' . $empresaId;

        $this->pantallas = [
            'jornada' => ['path' => '/ventas/gastronomia/jornada' . $q, 'auth' => true],
            'habilitacion-turno' => ['path' => '/ventas/gastronomia/habilitacion-turno' . $q, 'auth' => true],
            'proceso-facturacion' => ['path' => '/ventas/gastronomia/proceso-facturacion', 'auth' => true],
            'facturas-dia' => ['path' => '/ventas/gastronomia/facturas-dia' . $q, 'auth' => true],
            'cierres-turno' => ['path' => '/ventas/gastronomia/cierres-turno' . $q, 'auth' => true],
            'informe-gerente' => ['path' => '/ventas/gastronomia/informe-gerente' . $q, 'auth' => true],
            'articulos-vendidos' => ['path' => '/ventas/gastronomia/articulos-vendidos' . $q, 'auth' => true],
            'saneamiento-turno' => ['path' => '/ventas/gastronomia/saneamiento-turno' . $q, 'auth' => true],
            'configuracion-pv' => ['path' => '/ventas/configuracion-puntoventa-gastronomia' . $q, 'auth' => true],
            'totem-waitry' => ['path' => '/ventas/totem-waitry-gastronomia' . $q, 'auth' => true],
        ];

        $outDir = public_path('docs/manual-gastronomia/img');
        ...
        $this->info('Capturando pantallas gastronomía → ' . $outDir);
        $this->info('Empresa en query: ' . $empresaId);
        ...
        $this->info('Listo. Ejecute: php docs/manual-gastronomia/generar.php');
    ...
    private function htmlAPng(string $html, string $pngPath): void
    {
        ...
        $tmpPdf = storage_path('app/tmp-manual-gastronomia-captura.pdf');
        Pdf::loadHTML($html)
    ...
    private function prepararHtmlParaCaptura(string $html): string
    {
        ...
        // DomPDF falla con <template> (tr/td fuera de table) y celdas vacías en AdminLTE
