...
class CapturarPantallasManualCanjesMarketingInterno extends Command
{
    protected $signature = 'manual:capturar-canjes-marketing-interno
                            {--usuario=admin : Usuario ERP}
                            {--rol= : ID de rol (opcional)}
                            {--empresa= : empresa_id (default: primera con PV gastronomía)}';

    protected $description = 'Captura pantallas del módulo Canjes Marketing (render interno + Ghostscript)';
    ...
    public function handle(): int
    {
        ...
        $inicioMes = Carbon::today()->startOfMonth()->format('Y-m-d');
        $q = '?empresa_id=' . $empresaId;

        $this->pantallas = [
            'cliente-vip-listado' => ['path' => '/ventas/gastronomia/canjes/cliente-vip' . $q, 'auth' => true],
            'cliente-vip-crear' => ['path' => '/ventas/gastronomia/canjes/cliente-vip/crear' . $q, 'auth' => true],
            'facturador-canjes-login' => ['path' => '/ventas/gastronomia/canjes/proceso-facturacion', 'auth' => true],
            'facturador-canjes-pantalla' => ['path' => '/ventas/gastronomia/canjes/proceso-facturacion', 'auth' => true],
            'listado-marketing' => [
                'path' => '/ventas/gastronomia/canjes/listado-marketing'
                    . $q . '&fecha_desde=' . $inicioMes . '&fecha_hasta=' . $hoy,
                'auth' => true,
            ],
        ];

        $outDir = public_path('docs/manual-canjes-marketing/img');
        ...
        $this->info('Capturando Canjes Marketing → ' . $outDir);
        ...
        $this->info('Listo. Ejecute: php docs/manual-canjes-marketing/generar.php');
    ...
    private function htmlAPng(string $html, string $pngPath): void
    {
        ...
        $tmpPdf = storage_path('app/tmp-manual-canjes-captura.pdf');
        Pdf::loadHTML($html)
    ...
    private function prepararHtmlParaCaptura(string $html): string
    {
        ...
        return '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">'
            ...
            . '#cm-pos-shell.cm-bloqueado{filter:none!important;pointer-events:auto!important;}'
            . '</style></head><body>'
            . $body . '</body></html>';
    }
