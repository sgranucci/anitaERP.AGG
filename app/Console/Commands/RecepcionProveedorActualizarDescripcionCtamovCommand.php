...
class RecepcionProveedorActualizarDescripcionCtamovCommand extends Command
{
    protected $signature = 'recepcion-proveedor:actualizar-descripcion-ctamov
                            ...
                            {--desde= : Fecha recepción desde (Y-m-d)}
    ...
    protected $description = 'Actualiza leyenda COM + proveedor en ctav_desc_mov y cabecera asiento ERP';
    ...
    public function handle(RecepcionProveedorAsientoDescripcionCtamovService $service): int
    {
        $opciones = [
            ...
            'fecha_desde' => $this->option('desde') ? (string) $this->option('desde') : null,
        ...
        if ($opciones['fecha_desde']) {
            $this->info('Filtro fecha recepción >= '.$opciones['fecha_desde']);
        }
        ...
        $total = $service->contarCandidatas($opciones);
