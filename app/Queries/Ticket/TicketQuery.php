...
class TicketQuery implements TicketQueryInterface
{
    ...
    public function leeTicketAdministracion($busqueda, $filtros, $flPaginando = null)
    {
        ini_set('memory_limit', '-1');
        ini_set('max_execution_time', '0');
		...
		if ($filtros != '' && isset($filtros['filter_column']))
		{
			for ($ii = 0; $ii < count($filtros['filter_column']); $ii++)
			{
				...
				if ($filtros['filter_column'][$ii]['column'] == 'fechaticket' &&
					$filtros['filter_column'][$ii]['value'][0] != '')
				{
					$fechaTicket = $filtros['filter_column'][$ii]['value'][0].'/'.
        ...
        if ($flTecnico) // Tecnico ve solo sus tickets de su area
        {
            ...
            $tickets->where(function ($query) use ($usuario_id) {
                //$query->orWhere('tecnico_usuario_id', $usuario_id);
                $query->orWhere('estado_ticket', "Sin Asignar");
                $query->orwhereHas('ticket_tareas.tecnicos', function ($query) use ($usuario_id) {
        ...
        if ($estado != '')
            $tickets->where('estado_ticket', $estado);
        else
        {
            $tickets->where('estado_ticket', "!=", "Finalizado");
            $tickets->where('estado_ticket', "!=", "Suspendido");
        }

        if ($fechaTicket != '')
        {
            $fechasTicket = explode("/", $fechaTicket);
            $tickets->whereBetween('ticket.fecha', $fechasTicket);
        }
        ...
        $tickets->where(function ($query) use ($count, $busqueda, $columns, $flSupervisor, $flTecnico, 
                        			...
                        			for ($i = 0; $i < $count; $i++)
                                    {
                                        if ($columns[$i]['columna'] == 'ticket.fecha' && !ctype_alpha($busqueda))
                                        {
                                            $query->orWhere($columns[$i]['columna'], $columns[$i]['clausula'], $busqueda);
                                        }

                                        if ($columns[$i]['columna'] != 'ticket.fecha')
                                        {
                                            if ($columns[$i]['clausula'] == 'LIKE')
                                                $query->orWhere($columns[$i]['columna'], "LIKE", '%'. $busqueda . '%');
                                            else
