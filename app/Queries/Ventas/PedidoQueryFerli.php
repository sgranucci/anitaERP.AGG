...
/**
 * Consultas de pedido del circuito Ferli (pedido_combinacion / talles / OT).
 * No implementar PedidoQueryInterface: esa interfaz es del circuito std (pedido_articulo).
 */
class PedidoQueryFerli
{
    protected $model;
    ...
    public function allPedidoIndex($cliente_id, $opcion)
    {
        ...
        return $this->model->with('clientes:id,nombre')->with('mventas:id,nombre')
                                ->orderBy('id','desc')
    ...
    public function allPedidoIndexPaginando($busqueda)
    {
        ...
        return $this->model->select('pedido.id as id',
    ...
    {
        if ($cliente_id) {
            return $this->model->with('clientes:id,nombre')->with('mventas:id,nombre')->with('transportes:id,nombre')->with('pedido_combinaciones')->where('estado','0')->where('cliente_id',$cliente_id)->get();
        }

        return $this->model->with('clientes:id,nombre')->with('mventas:id,nombre')->with('transportes:id,nombre')->with('pedido_combinaciones')->where('estado','0')->get();
    }
    ...
    public function allProduccionIndex($cliente_id)
    {
        return collect();
    }
