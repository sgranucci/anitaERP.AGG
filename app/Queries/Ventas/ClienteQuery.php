...
class ClienteQuery implements ClienteQueryInterface
{
    ...
    {
	  	if ($campos)
        	return $this->model->with('condicionivas')->with('condicionventas')->with('abastos')->with('coeficientes')->select($campos)->where('id',$id)->first();
		else
        	return $this->model->with('condicionivas')->where('id',$id)->with('paises')->with('abastos')->with('coeficientes')->with('condicionventas')->first();
    }
