(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-proveedor').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-proveedor');
        ...
        $('#form-filtros-proveedor').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-proveedor').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-proveedor'), {
            selectorPanel: '#panel-filtros-proveedor'
        });
