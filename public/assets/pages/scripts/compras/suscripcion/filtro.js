(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-suscripcion').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-suscripcion');
        ...
        $('#form-filtros-suscripcion').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-suscripcion').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-suscripcion'), {
            selectorPanel: '#panel-filtros-suscripcion'
        });
