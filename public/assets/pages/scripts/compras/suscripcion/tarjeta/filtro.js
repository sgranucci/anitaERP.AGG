(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (tipo === 'entero') {
            ...
            setPlaceholderValor('Activa / Inactiva');
        } else if (tipo === 'imputacion') {
            setPlaceholderValor('Lista / Incompleta');
        } else {
    ...
    $(function () {
        if (!$('#form-filtros-tarjeta-suscripcion').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-tarjeta-suscripcion');
        ...
        $('#form-filtros-tarjeta-suscripcion').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-tarjeta-suscripcion').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-tarjeta-suscripcion'), {
            selectorPanel: '#panel-filtros-tarjeta-suscripcion'
        });
