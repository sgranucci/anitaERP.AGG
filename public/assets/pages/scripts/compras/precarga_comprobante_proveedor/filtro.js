(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (modo === MODO_CAMPO && operador === 'entre' && tipo === 'fecha') {
    ...
    $(function () {
        if (!$('#form-filtros-precarga-comprobante-proveedor').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-precarga-comprobante-proveedor');
        ...
        $('#form-filtros-precarga-comprobante-proveedor').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-precarga-comprobante-proveedor').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-precarga-comprobante-proveedor'), {
            selectorPanel: '#panel-filtros-precarga-comprobante-proveedor'
        });
