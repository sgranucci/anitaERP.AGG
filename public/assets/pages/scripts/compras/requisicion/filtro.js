(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-requisicion').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-requisicion');
        ...
        $('#form-filtros-requisicion').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-requisicion').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-requisicion'), {
            selectorPanel: '#panel-filtros-requisicion'
        });
