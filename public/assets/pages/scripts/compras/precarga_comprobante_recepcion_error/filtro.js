(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-precarga-recepcion-error').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-precarga-recepcion-error');
        ...
        $('#form-filtros-precarga-recepcion-error').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-precarga-recepcion-error').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-precarga-recepcion-error'), {
            selectorPanel: '#panel-filtros-precarga-recepcion-error'
        });
