(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-bien-uso').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-bien-uso');
        ...
        $('#form-filtros-bien-uso').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-bien-uso').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-bien-uso'), {
            selectorPanel: '#panel-filtros-bien-uso'
        });
