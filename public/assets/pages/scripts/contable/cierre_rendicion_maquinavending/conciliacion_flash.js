(function () {
    ...
    var cfg = window.CIERRE_REND_VEND_CONC || {};
    ...
    function ejecutarCierreJornada(btn) {
        ...
        var fechaFmt = btn.getAttribute('data-fecha-fmt') || fechaJornada;
        ...
        var msg = '¿Confirmar cierre contable de la jornada ' + fechaFmt + '?\n'
            + pendientes + ' rendición(es) pendiente(s) en ' + grupos + ' asiento(s) (fecha + PV).';
        if (! confirm(msg)) {
            return;
        }
    ...
    function ejecutarCierrePeriodo(btn) {
        ...
        var grupos = parseInt(btn.getAttribute('data-grupos') || '0', 10);
        var pendientes = parseInt(btn.getAttribute('data-pendientes') || '0', 10);
        var jornadas = parseInt(btn.getAttribute('data-jornadas') || '0', 10);
        ...
        var msg = '¿Confirmar cierre contable del periodo completo?\n'
            + jornadas + ' jornada(s), ' + pendientes + ' rendición(es) pendiente(s), '
            + grupos + ' asiento(s) a generar (fecha jornada + PV).';
        if (! confirm(msg)) {
            return;
        }
