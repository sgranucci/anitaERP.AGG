(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (tipo === 'entero') {
            ...
            setPlaceholderValor('Fecha d/m/Y o Y-m-d');
        } else {
    ...
    $(function () {
        if (!$('#form-filtros-asiento').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-asiento');
        ...
        $('#form-filtros-asiento').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-asiento').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-asiento'), {
            selectorPanel: '#panel-filtros-asiento'
        });
