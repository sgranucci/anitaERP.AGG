(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-cierres-gastro-contable').length) {
            return;
        }
        ...
        $('#todas_terminales').on('change', function () {
            ...
            $('#form-filtros-cierres-gastro-contable').trigger('submit');
        });
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-cierres-gastro-contable');
        ...
        $('#form-filtros-cierres-gastro-contable').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-cierres-gastro-contable').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        if (LF && LF.initSubmitBusquedaRapida) {
            LF.initSubmitBusquedaRapida($('#form-filtros-cierres-gastro-contable'), {
                selectorPanel: '#panel-filtros-cierres-gastro-contable'
            });
        }
