(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-ingreso-proveedor').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-ingreso-proveedor');
        ...
        $('#form-filtros-ingreso-proveedor').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-ingreso-proveedor').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-ingreso-proveedor'), {
            selectorPanel: '#panel-filtros-ingreso-proveedor'
        });
