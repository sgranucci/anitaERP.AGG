(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        setPlaceholderValor(tipo === 'entero' ? 'Número entero' : 'Texto o número');
    }
    ...
    function actualizarOperadores(mantenerSeleccion) {
        ...
        if (modo === MODO_CAMPO) {
            mapa = operadoresPorCampo[$('#filtro_campo').val()] || LF.operadoresModoTodos();
        } else {
    ...
    $(function () {
        if (!$('#form-filtros-ordentrabajo').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-ordentrabajo');
        ...
        $('#form-filtros-ordentrabajo').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-ordentrabajo').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-ordentrabajo'), {
            selectorPanel: '#panel-filtros-ordentrabajo'
        });
