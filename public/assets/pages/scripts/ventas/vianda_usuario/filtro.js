(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-vianda-usuario').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-vianda-usuario');
        ...
        $('#form-filtros-vianda-usuario').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-vianda-usuario').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-vianda-usuario'), {
            selectorPanel: '#panel-filtros-vianda-usuario'
        });
