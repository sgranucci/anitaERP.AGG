(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (tipo === 'entero') {
            ...
            setPlaceholderValor('Localidad, zona o código SENASA');
        }
    }
    ...
    $(function () {
        if (!$('#form-filtros-destino').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-destino');
        ...
        $('#form-filtros-destino').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-destino').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-destino'), {
            selectorPanel: '#panel-filtros-destino'
        });
