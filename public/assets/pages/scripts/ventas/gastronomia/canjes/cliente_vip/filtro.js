(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-cliente-vip').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-cliente-vip');
        ...
        $('#form-filtros-cliente-vip').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-cliente-vip').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-cliente-vip'), {
            selectorPanel: '#panel-filtros-cliente-vip'
        });
