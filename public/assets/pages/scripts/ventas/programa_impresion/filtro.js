(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-programa-impresion').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-programa-impresion');
        ...
        $('#form-filtros-programa-impresion').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-programa-impresion').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-programa-impresion'), {
            selectorPanel: '#panel-filtros-programa-impresion'
        });
