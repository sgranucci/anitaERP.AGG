(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-concepto-venta').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-concepto-venta');
        ...
        $('#form-filtros-concepto-venta').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-concepto-venta').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-concepto-venta'), {
            selectorPanel: '#panel-filtros-concepto-venta'
        });
