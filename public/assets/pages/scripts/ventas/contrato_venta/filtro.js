(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (tipo === 'entero') {
            ...
            setPlaceholderValor('Código, cliente o concepto');
        }
    }
    ...
    $(function () {
        if (!$('#form-filtros-contrato-venta').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-contrato-venta');
        ...
        $('#form-filtros-contrato-venta').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-contrato-venta').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-contrato-venta'), {
            selectorPanel: '#panel-filtros-contrato-venta'
        });
