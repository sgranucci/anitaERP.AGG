(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (esFecha) {
            ...
            setPlaceholderValor('Código o descripción de estado');
        } else {
    ...
    $(function () {
        if (!$('#form-filtros-pedido-interforming').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-pedido-interforming');
        ...
        $('#form-filtros-pedido-interforming').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-pedido-interforming').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-pedido-interforming'), {
            selectorPanel: '#panel-filtros-pedido-interforming'
        });
