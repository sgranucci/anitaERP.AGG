(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (tipo === 'entero') {
            ...
            setPlaceholderValor('Código, nombre, empresa…');
        }
    }
    ...
    $(function () {
        if (!$('#form-filtros-puntoventa').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-puntoventa');
        ...
        $('#form-filtros-puntoventa').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-puntoventa').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-puntoventa'), {
            selectorPanel: '#panel-filtros-puntoventa'
        });
