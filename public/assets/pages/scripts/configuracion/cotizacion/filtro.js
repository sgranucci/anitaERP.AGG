(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-cotizacion').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-cotizacion');
        ...
        $('#form-filtros-cotizacion').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-cotizacion').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-cotizacion'), {
            selectorPanel: '#panel-filtros-cotizacion'
        });
