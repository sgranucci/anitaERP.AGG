(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-ubicacion-impresora').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-ubicacion-impresora');
        ...
        $('#form-filtros-ubicacion-impresora').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-ubicacion-impresora').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-ubicacion-impresora'), {
            selectorPanel: '#panel-filtros-ubicacion-impresora'
        });
