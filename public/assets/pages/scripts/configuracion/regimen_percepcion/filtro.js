(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (tipo === 'entero') {
            ...
            setPlaceholderValor('Código o nombre');
        }
    }
    ...
    $(function () {
        if (!$('#form-filtros-regimen-percepcion').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-regimen-percepcion');
        ...
        $('#form-filtros-regimen-percepcion').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-regimen-percepcion').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-regimen-percepcion'), {
            selectorPanel: '#panel-filtros-regimen-percepcion'
        });
