(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-provincia').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-provincia');
        ...
        $('#form-filtros-provincia').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-provincia').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-provincia'), {
            selectorPanel: '#panel-filtros-provincia'
        });
