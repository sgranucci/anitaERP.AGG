(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-feriado').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-feriado');
        ...
        $('#form-filtros-feriado').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-feriado').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-feriado'), {
            selectorPanel: '#panel-filtros-feriado'
        });
