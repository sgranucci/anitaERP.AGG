(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-ingresoegreso').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-ingresoegreso');
        ...
        $('#form-filtros-ingresoegreso').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-ingresoegreso').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-ingresoegreso'), {
            selectorPanel: '#panel-filtros-ingresoegreso'
        });
