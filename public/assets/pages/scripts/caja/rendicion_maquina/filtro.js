(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-rendicion-maquina').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-rendicion-maquina');
        ...
        $('#form-filtros-rendicion-maquina').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-rendicion-maquina').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-rendicion-maquina'), {
            selectorPanel: '#panel-filtros-rendicion-maquina'
        });
