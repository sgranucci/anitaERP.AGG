(function () {
    ...
    var app = document.getElementById('rendicion-estacionamiento-app');
    if (!app) {
        return;
    }
    ...
    var inpTurnoId = document.getElementById('turno_operativo_estacionamiento_id');
    var inpTurnoNumero = document.getElementById('turno_operativo_numero');
    var inpJornadaId = document.getElementById('jornada_estacionamiento_id');
    var inpJornadaNumero = document.getElementById('jornada_estacionamiento_numero');
    ...
    var chkVerificacionEstacionamiento = document.getElementById('chk_verificacion_estacionamiento');
    ...
    var cierreEstacionamientoCargado = !!(inicial.cierre_cargado);
    ...
    var formEl = document.getElementById('form-rendicion-estacionamiento');
    ...
    var alertErroresEl = document.getElementById('alert-errores-rendicion-estacionamiento');
    ...
    var R = window.EstacionamientoTotalesTurnoRender || {};
    ...
    window.rendicionEstacionamientoMostrarError = mostrarErrorRendicion;
    window.rendicionEstacionamientoOcultarError = ocultarErrorRendicion;
    ...
    function actualizarAvisoSinCierre() {
        ...
        avisoSinCierreCargado.classList.toggle('d-none', cierreEstacionamientoCargado);
    }
    ...
    function actualizarEstadoBotonGuardar() {
        ...
        var puedeGuardar = cierreEstacionamientoCargado
            && chkVerificacionEstacionamiento
            && chkVerificacionEstacionamiento.checked;
        btnGuardarRendicion.disabled = !puedeGuardar;
        if (hintVerificacionFooter) {
            if (!cierreEstacionamientoCargado) {
            ...
            } else if (!chkVerificacionEstacionamiento || !chkVerificacionEstacionamiento.checked) {
    ...
    function habilitarVerificacionCajero(esJornadaCierre) {
        cierreEstacionamientoCargado = true;
        actualizarAvisoSinCierre();
        if (chkVerificacionEstacionamiento) {
            chkVerificacionEstacionamiento.disabled = false;
        }
    ...
    function deshabilitarVerificacionCajero() {
        cierreEstacionamientoCargado = false;
        actualizarAvisoSinCierre();
        if (chkVerificacionEstacionamiento) {
            chkVerificacionEstacionamiento.disabled = true;
            chkVerificacionEstacionamiento.checked = false;
        }
        actualizarEstadoBotonGuardar();
        if (verifItemJornadaWaitry) {
    ...
    function renderNumeracionPorPuntoventa(d) {
        ...
        if (!filas || !filas.length) {
            cont.innerHTML = '<p class="text-muted mb-0">'
                + escHtml(d.numeracion_resumen || 'Sin comprobantes o sin PV CAE/CAEA configurados en estacionamiento.')
                + '</p>';
            return;
        }
    ...
    function renderInformeZJornada(d) {
        ...
        if (!cargado || !conciliacion || !conciliacion.totems || conciliacion.totems.length === 0) {
            ...
            if (d.cierre_totem_habilitado === false) {
                ...
                msg += 'Informe Z no cargado. Regístrelo desde <strong>Ventas → Estacionamiento → Jornada</strong> antes de cerrar la jornada.';
            }
            msg += '</p>';
    ...
    function aplicarDatosJornada(d) {
        ...
        var etiqueta = 'Jornada #' + d.jornada_estacionamiento_id
            + ' — ' + (d.fecha_jornada || '')
            + ' — cierre ' + (d.cierre_en || '');
        if (inpJornadaId) {
            inpJornadaId.value = d.jornada_estacionamiento_id;
        }
        if (inpJornadaNumero) {
            inpJornadaNumero.value = d.jornada_estacionamiento_id;
        }
        if (lblJornadaSeleccionada) {
    ...
    function aplicarDatosTurno(d) {
        ...
        var etiqueta = 'Op. #' + d.turno_operativo_estacionamiento_id
            + ' — ' + (d.turno_nombre || '')
            + ' — ' + (d.identificador_pc || '')
            + ' — cierre ' + (d.cierre_en || '');
        setTurnoSeleccionado(d.turno_operativo_estacionamiento_id, etiqueta);
    ...
    function cargarTurno(turnoId) {
        ...
        body.append('turno_operativo_estacionamiento_id', turnoId);
        if (rendicionId) {
    ...
    function cargarJornada(jornadaId) {
        ...
        var body = new FormData();
        body.append('jornada_estacionamiento_id', jornadaId);
        if (rendicionId) {
            body.append('excepto_rendicion_id', rendicionId);
        }
        var token = document.querySelector('input[name="_token"]');
    ...
    window.rendicionEstacionamientoCargarTurno = cargarTurno;
    window.rendicionEstacionamientoCargarJornada = cargarJornada;
    window.rendicionEstacionamientoFijarTipo = fijarTipoUiSinLimpiar;
    ...
    function initEdicionLocal() {
        if (!inicial.turno_operativo_estacionamiento_id) {
            return;
        }
        panelDatos.classList.remove('d-none');
    ...
    if (formEl) {
        formEl.addEventListener('keydown', function (ev) {
            if ((ev.key === 'Enter' || ev.keyCode === 13) && ev.target
                && (ev.target.id === 'turno_operativo_numero' || ev.target.id === 'jornada_estacionamiento_numero')) {
                ev.preventDefault();
                ev.stopPropagation();
            }
        });
        ...
        formEl.addEventListener('submit', function (ev) {
            ...
            if (modo === 'crear' && !cierreEstacionamientoCargado) {
                ev.preventDefault();
                mostrarErrorRendicion('Debe cargar y revisar el cierre de estacionamiento antes de registrar la rendición.');
                return;
            }
            if (modo === 'crear' && chkVerificacionEstacionamiento && !chkVerificacionEstacionamiento.checked) {
                ev.preventDefault();
                actualizarEstadoBotonGuardar();
                if (bloqueVerificacionFooter) {
                    bloqueVerificacionFooter.scrollIntoView({ behavior: 'smooth', block: 'center' });
                }
                if (chkVerificacionEstacionamiento && !chkVerificacionEstacionamiento.disabled) {
                    chkVerificacionEstacionamiento.focus();
                }
                return;
            }
        });
    ...
    if (chkVerificacionEstacionamiento) {
        chkVerificacionEstacionamiento.addEventListener('change', actualizarEstadoBotonGuardar);
    }
