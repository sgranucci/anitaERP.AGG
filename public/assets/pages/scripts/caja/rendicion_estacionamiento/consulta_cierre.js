...
(function ($) {
    ...
    function csrfToken() {
        ...
        var $form = $('#form-rendicion-estacionamiento');
        if ($form.length) {
    ...
    function urlConsultaCierre() {
        ...
        return (typeof carpetaBase !== 'undefined' ? carpetaBase : '') + '/caja/rendicionestacionamiento/api/consulta-cierre';
    }
    ...
    function notificarError(msg) {
        if (typeof window.rendicionEstacionamientoMostrarError === 'function') {
            window.rendicionEstacionamientoMostrarError(msg);
            return;
        }
        alert(msg);
    }
    ...
    function actualizarTituloModal() {
        ...
        if (esJornada) {
            $thead.html(
                '<th>Nº jorn.</th><th>Fecha jornada</th><th>Cierre</th><th>Usuario cierre</th><th class="width120">Acciones</th>'
            );
        } else {
            $thead.html(
    ...
    function activarEventosConsultaCierre() {
        ...
        $(document)
            ...
            .on('click.consultaCierre', '.eligeconsultacierre', function (e) {
                ...
                $('#turno_operativo_estacionamiento_id').val(id);
                $('#turno_operativo_numero').val(id);
                $('#lbl-turno-seleccionado').text(etiqueta);

                $('#consultacierreModal').modal('hide');

                if (typeof window.rendicionEstacionamientoFijarTipo === 'function') {
                    window.rendicionEstacionamientoFijarTipo('turno');
                }
                $('#tipo_rendicion').val('turno');

                if (typeof window.rendicionEstacionamientoCargarTurno === 'function') {
                    window.rendicionEstacionamientoCargarTurno(id);
                } else {
            ...
            .on('click.consultaCierre', '.eligeconsultacierrejornada', function (e) {
                ...
                $('#jornada_estacionamiento_id').val(id);
                $('#jornada_estacionamiento_numero').val(id);
                $('#lbl-jornada-seleccionada').text(etiqueta);

                $('#consultacierreModal').modal('hide');

                if (typeof window.rendicionEstacionamientoFijarTipo === 'function') {
                    window.rendicionEstacionamientoFijarTipo('jornada');
                }
                $('#tipo_rendicion').val('jornada');

                if (typeof window.rendicionEstacionamientoCargarJornada === 'function') {
                    window.rendicionEstacionamientoCargarJornada(id);
                } else {
