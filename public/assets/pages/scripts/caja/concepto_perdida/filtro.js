(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-concepto-perdida').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-concepto-perdida');
        ...
        $('#form-filtros-concepto-perdida').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-concepto-perdida').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-concepto-perdida'), {
            selectorPanel: '#panel-filtros-concepto-perdida'
        });
