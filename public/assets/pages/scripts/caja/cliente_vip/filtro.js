(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-cliente-vip-caja').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-cliente-vip-caja');
        ...
        $('#form-filtros-cliente-vip-caja').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-cliente-vip-caja').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-cliente-vip-caja'), {
            selectorPanel: '#panel-filtros-cliente-vip-caja'
        });
