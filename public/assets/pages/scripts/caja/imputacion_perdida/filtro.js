(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-imputacion-perdida').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-imputacion-perdida');
        ...
        $('#form-filtros-imputacion-perdida').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-imputacion-perdida').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-imputacion-perdida'), {
            selectorPanel: '#panel-filtros-imputacion-perdida'
        });
