(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-apertura-gasto').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-apertura-gasto');
        ...
        $('#form-filtros-apertura-gasto').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-apertura-gasto').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-apertura-gasto'), {
            selectorPanel: '#panel-filtros-apertura-gasto'
        });
