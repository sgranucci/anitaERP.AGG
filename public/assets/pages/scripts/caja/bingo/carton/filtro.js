(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-bingo-carton').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-bingo-carton');
        ...
        $('#form-filtros-bingo-carton').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-bingo-carton').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-bingo-carton'), {
            selectorPanel: '#panel-filtros-bingo-carton'
        });
