(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (tipo === 'entero' || tipo === 'decimal') {
            setPlaceholderValor(tipo === 'decimal' ? 'Importe' : 'Número entero');
        } else {
    ...
    $(function () {
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-cheque');
        ...
        $('#form-filtros-cheque').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-cheque').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-cheque'), {
            selectorPanel: '#panel-filtros-cheque'
        });
