(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        if (tipo === 'entero') {
            ...
            setPlaceholderValor('Texto, ID, código…');
        }
    }
    ...
    $(function () {
        if (!$('#form-filtros-rendicion-bingo').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-rendicion-bingo');
        ...
        $('#form-filtros-rendicion-bingo').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-rendicion-bingo').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        if (LF && LF.initSubmitBusquedaRapida) {
            LF.initSubmitBusquedaRapida($('#form-filtros-rendicion-bingo'), {
                selectorPanel: '#panel-filtros-rendicion-bingo'
            });
        }
