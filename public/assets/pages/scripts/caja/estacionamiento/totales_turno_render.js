(function () {
    ...
    var CLS_PANEL = 'est-totales-panel';
    ...
    function renderConciliacionHtml(t) {
        ...
        var html = '<div class="' + CLS_PANEL + ' est-conciliacion border rounded p-3 mb-3 bg-white">';
        ...
        html += '</div>';
        html += '<p class="text-muted mb-2 est-totales-leyenda">Facturado bruto − Notas de crédito = <strong>Facturado final</strong>, que debe coincidir con lo cobrado (descontando invitaciones).</p>';
        ...
        html += '<span class="est-totales-monto font-weight-bold">$' + fmt(totalFacturasBruto) + '</span>';
        ...
        html += '<span class="est-totales-monto font-weight-bold">$' + fmt(t.total_invitaciones) + '</span>';
        ...
        html += '<span class="est-totales-monto font-weight-bold">$' + fmt(ncTotal) + '</span>';
        ...
        html += '<span class="est-totales-monto font-weight-bold">$' + fmt(totalFinal) + '</span>';
        ...
        html += '<span class="est-totales-monto font-weight-bold">$' + fmt(t.total_ventas_cobrables) + '</span>';
        html += '</div>';
        html += '<div class="col-6 col-md-6">';
        html += '<span class="text-muted d-block">Cobrado neto <em>(cobranzas − devoluciones NC)</em></span>';
        html += '<span class="est-totales-monto font-weight-bold">$' + fmt(t.total_cobrado) + '</span>';
        html += '</div>';
        html += '</div>';
    ...
    function renderMediosPagoTabla(medios, vacio, conTotalFinal, totalCobradoRef, opcionesConciliar, notasCredito, usuarioCtx, invitaciones, opcionesArqueo) {
        ...
        var usuarioId = usuarioCtx && (usuarioCtx.usuario_id != null ? usuarioCtx.usuario_id : usuarioCtx.usuario_habilitado_id) != null ? (usuarioCtx.usuario_id != null ? usuarioCtx.usuario_id : usuarioCtx.usuario_habilitado_id) : '';
        var usuarioNombre = usuarioCtx && (usuarioCtx.usuario_nombre || usuarioCtx.usuario_habilitado_nombre) ? (usuarioCtx.usuario_nombre || usuarioCtx.usuario_habilitado_nombre) : '';
        ...
        (medios || []).forEach(function (p) {
            ...
            var trClass = esEfectivo ? ' class="est-cierre-fila-efectivo"' : '';
            ...
            if (arqueoActivo) {
                ...
                var clsEsperado = esEfectivo
                    ? 'est-rendicion-esperado-efectivo font-weight-bold'
                    : 'font-weight-bold';
            ...
            if (conciliar && ccId > 0) {
                ...
                if (usuarioId !== '') {
                    html += ' data-usuario-id="' + esc(usuarioId) + '"';
                }
                if (usuarioNombre) {
                    html += ' data-usuario-nombre="' + esc(usuarioNombre) + '"';
                }
        ...
        if (conTotalFinal) {
            ...
            html += '<th class="text-right est-totales-monto font-weight-bold">$' + fmt(totalFinal) + '</th>';
            if (conciliar) {
                html += '<th></th>';
            }
            html += '</tr></tfoot>';
    ...
    function renderTotalMediosPagoFinalHtml(totales, opcionesConciliar, opcionesRender) {
        ...
        var html = '<div class="mt-3 pt-3 border-top est-totales-medios-final">';
    ...
    function renderPorUsuarioHabilitadoHtml(porUsuario, opcionesConciliar) {
        ...
        porUsuario.forEach(function (m) {
            ...
            html += '<strong class="est-item-nombre">' + esc(m.usuario_nombre || m.usuario_habilitado_nombre || 'Sin usuario') + '</strong>';
            ...
            html += renderMediosPagoTabla(
                ...
                {
                    usuario_id: m.usuario_id != null ? m.usuario_id : (m.usuario_habilitado_id != null ? m.usuario_habilitado_id : ''),
                    usuario_nombre: m.usuario_nombre || m.usuario_habilitado_nombre || '',
                },
    ...
    function renderGrillaConciliacionHtml(grilla) {
        ...
        if (pag) {
            ...
            html += '</span>';
            html += '<div class="est-grilla-paginacion" data-container=""></div>';
            html += '</div>';
        }

        html += '<table class="table table-bordered table-sm mb-0 est-totales-tabla">';
        html += '<thead><tr>';
        html += '<th>Comprobante</th><th>Hora</th><th>Cliente</th><th>Usuario</th>';
        ...
        filas.forEach(function (f) {
            ...
            html += '<td>' + esc(f.usuario_nombre || f.usuario_habilitado_nombre || '—') + '</td>';
            ...
            if (f.venta_id) {
                var baseVer = (typeof window !== 'undefined' && window.ESTACIONAMIENTO_FACTURA_VER_BASE) || '';
                if (baseVer) {
        ...
        if (pag && pag.total_pages > 1) {
            html += '<div class="d-flex justify-content-center py-2 border-top bg-light est-grilla-paginacion-footer" data-container=""></div>';
        }
    ...
    function renderTotalesHtml(totales, titulo, opciones) {
        ...
        var html = '<div class="' + CLS_PANEL + ' est-totales-bloque border rounded p-3 mb-3 bg-white">';
        ...
        var compacto = opciones && (opciones.soloPorUsuario || opciones.soloUsuarioHabilitado);
        ...
        if (!ocultarUsuario) {
            html += '<h6 class="font-weight-bold mt-1 mb-2">Detalle por usuario</h6>';
            html += renderPorUsuarioHabilitadoHtml(totales.por_usuario || totales.por_usuario_habilitado, optsConc);
        }
        if (!compacto) {
    ...
    function renderNumeracionFiscalHtml(numeracion) {
        ...
        var html = '<div class="card card-outline card-secondary mb-0 est-numeracion-fiscal">';
        ...
        html += '<div class="table-responsive"><table class="table table-sm table-bordered mb-0 est-totales-tabla">';
    ...
    function renderTotalesResumenHtml(bloques) {
        ...
        var html = '<div class="row est-totales-resumen">';
    ...
    window.EstacionamientoTotalesTurnoRender = {
        fmt: fmt,
        esc: esc,
