(function () {
    ...
    function aplicarCuenta(data) {
        ...
        var cuentaAnita = codigo.replace(/\D+/g, '');
        if (cuentaAnita !== '') {
            cuentaAnita = ('00000000' + cuentaAnita.slice(-8)).slice(-8);
        }

        $('#cuentacaja_id').val(id);
        $('#codigo_cuentacaja').val(codigo);
        $('#nombre_cuentacaja').val(nombre);
        $('#cuenta_anita').val(cuentaAnita);
        if ($('#cbu_origen').length) {
            $('#cbu_origen').val(cbu);
        }
        if ($('#cbu_origen_mostrar').length) {
            $('#cbu_origen_mostrar').val(cbu);
        }
        // Dispara recálculo cuenta débito Macro en la vista
        $(document).trigger('macro:cuenta-cambiada', [cbu]);
    }
    ...
    function abrirModalConsulta() {
        ...
        $('#consultacuentacajaModal').one('shown.bs.modal.macroArchivoPago', function () {
    ...
    function buscarCuentacajaPorCodigo() {
        ...
        var tpl = window.macroArchivoPagoCuentacajaPorCodigoUrl || '';
    ...
    $('#form-macro-archivo-pago').on('submit', function (e) {
        var id = parseInt($('#cuentacaja_id').val() || '0', 10) || 0;
        if (id <= 0) {
            e.preventDefault();
            alert('Elija una cuenta de caja Macro (consulta F1).');
            abrirModalConsulta();
            return false;
        }
    });
