(function ($) {
    ...
    $(function () {
        ...
        $('#form-filtros-capex').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-capex').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-capex'), {
            selectorPanel: '#panel-filtros-capex'
        });
