(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-partidagasto').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-partidagasto');
        ...
        $('#form-filtros-partidagasto').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-partidagasto').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-partidagasto'), {
            selectorPanel: '#panel-filtros-partidagasto'
        });
