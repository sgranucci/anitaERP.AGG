(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-presupuesto').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-presupuesto');
        ...
        $('#form-filtros-presupuesto').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-presupuesto').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-presupuesto'), {
            selectorPanel: '#panel-filtros-presupuesto'
        });
