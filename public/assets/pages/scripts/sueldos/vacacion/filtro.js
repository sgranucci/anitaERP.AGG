(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-vacacion-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-vacacion-sueldos');
        ...
        $('#form-filtros-vacacion-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-vacacion-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-vacacion-sueldos'), {
            selectorPanel: '#panel-filtros-vacacion-sueldos'
        });
