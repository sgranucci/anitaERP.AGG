(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-parametro-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-parametro-sueldos');
        ...
        $('#form-filtros-parametro-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-parametro-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-parametro-sueldos'), {
            selectorPanel: '#panel-filtros-parametro-sueldos'
        });
