(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-empleado-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-empleado-sueldos');
        ...
        $('#form-filtros-empleado-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-empleado-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-empleado-sueldos'), {
            selectorPanel: '#panel-filtros-empleado-sueldos'
        });
