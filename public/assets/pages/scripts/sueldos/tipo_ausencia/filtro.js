(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-tipo-ausencia-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-tipo-ausencia-sueldos');
        ...
        $('#form-filtros-tipo-ausencia-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-tipo-ausencia-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-tipo-ausencia-sueldos'), {
            selectorPanel: '#panel-filtros-tipo-ausencia-sueldos'
        });
