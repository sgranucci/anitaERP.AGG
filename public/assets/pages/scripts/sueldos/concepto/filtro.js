(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-concepto-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-concepto-sueldos');
        ...
        $('#form-filtros-concepto-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-concepto-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-concepto-sueldos'), {
            selectorPanel: '#panel-filtros-concepto-sueldos'
        });
