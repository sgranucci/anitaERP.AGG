(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-obrasocial-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-obrasocial-sueldos');
        ...
        $('#form-filtros-obrasocial-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-obrasocial-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-obrasocial-sueldos'), {
            selectorPanel: '#panel-filtros-obrasocial-sueldos'
        });
