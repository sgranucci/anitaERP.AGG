(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-grupo-concepto-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-grupo-concepto-sueldos');
        ...
        $('#form-filtros-grupo-concepto-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-grupo-concepto-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-grupo-concepto-sueldos'), {
            selectorPanel: '#panel-filtros-grupo-concepto-sueldos'
        });
