(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-art-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-art-sueldos');
        ...
        $('#form-filtros-art-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-art-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-art-sueldos'), {
            selectorPanel: '#panel-filtros-art-sueldos'
        });
