(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-agrupamiento-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-agrupamiento-sueldos');
        ...
        $('#form-filtros-agrupamiento-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-agrupamiento-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-agrupamiento-sueldos'), {
            selectorPanel: '#panel-filtros-agrupamiento-sueldos'
        });
