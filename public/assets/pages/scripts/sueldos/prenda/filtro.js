(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-prenda-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-prenda-sueldos');
        ...
        $('#form-filtros-prenda-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-prenda-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-prenda-sueldos'), {
            selectorPanel: '#panel-filtros-prenda-sueldos'
        });
