(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-novedad-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-novedad-sueldos');
        ...
        $('#form-filtros-novedad-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-novedad-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-novedad-sueldos'), {
            selectorPanel: '#panel-filtros-novedad-sueldos'
        });
