(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-fallocaja-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-fallocaja-sueldos');
        ...
        $('#form-filtros-fallocaja-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-fallocaja-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-fallocaja-sueldos'), {
            selectorPanel: '#panel-filtros-fallocaja-sueldos'
        });
