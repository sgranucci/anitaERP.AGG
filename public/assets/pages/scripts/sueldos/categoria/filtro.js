(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-categoria-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-categoria-sueldos');
        ...
        $('#form-filtros-categoria-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-categoria-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-categoria-sueldos'), {
            selectorPanel: '#panel-filtros-categoria-sueldos'
        });
