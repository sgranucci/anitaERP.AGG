(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-imputacion-concepto-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-imputacion-concepto-sueldos');
        ...
        $('#form-filtros-imputacion-concepto-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-imputacion-concepto-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-imputacion-concepto-sueldos'), {
            selectorPanel: '#panel-filtros-imputacion-concepto-sueldos'
        });
