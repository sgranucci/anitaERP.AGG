(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-liquidacion-sueldos').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-liquidacion-sueldos');
        ...
        $('#form-filtros-liquidacion-sueldos').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-liquidacion-sueldos').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-liquidacion-sueldos'), {
            selectorPanel: '#panel-filtros-liquidacion-sueldos'
        });
