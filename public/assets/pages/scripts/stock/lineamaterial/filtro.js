(function ($) {
    ...
    var formId = '#form-filtros-lineamaterial';
    var panelId = '#panel-filtros-lineamaterial';
    ...
    $(function () {
        ...
        $(formId).on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $(formId).on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($(formId), {
            selectorPanel: panelId
        });
