(function ($) {
    ...
    $(function () {
        if (!$('#form-filtros-movimientostock').length) {
            return;
        }
        ...
        function sincronizarValorAntesDeEnviar() {
            var $panel = $('#panel-filtros-movimientostock');
        ...
        $('#form-filtros-movimientostock').on('click', '[data-aplicar-filtros-panel]', function () {
            $valorPrincipal().val($valorPanel().val());
        });

        $('#form-filtros-movimientostock').on('submit.listadoFiltrosSync', function () {
            sincronizarValorAntesDeEnviar();
        });

        LF.initSubmitBusquedaRapida($('#form-filtros-movimientostock'), {
            selectorPanel: '#panel-filtros-movimientostock'
        });
