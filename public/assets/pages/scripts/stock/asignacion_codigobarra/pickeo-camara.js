(function ($) {
    ...
    function aplicarCodigo(codigo) {
        ...
        if (typeof window.acAplicarCodigoPickeo === 'function') {
            window.acAplicarCodigoPickeo(codigo);
        }
        return true;
    }

    function limpiarCodigosLeidos() {
        $('#ac_camara_codigos').empty().removeClass('tm-camara-codigos-visible');
    }

    function mostrarCodigosLeidos(codigos) {
        var $box = $('#ac_camara_codigos');
        $box.empty();
    ...
    function decodificarEnServidor(file) {
        var url = window.AC_URLS && window.AC_URLS.decodificarFoto;
        if (!url || !file) {
    ...
    function detenerStreamNativo() {
        ...
        $('#ac_camara_reader video').remove();
    }

    function detenerScanner() {
        window.acCamaraPickeoActiva = false;
        detenerStreamNativo();
        if (!scanner) {
            return $.Deferred().resolve().promise();
        }
    ...
    function cerrarCamara() {
        detenerScanner().always(function () {
            $('#ac_camara_overlay').removeClass('tm-camara-visible');
            $('#ac_camara_foto_wrap').addClass('d-none');
            $('#ac_camara_mira').addClass('d-none');
            mostrarPreview('');
    ...
    function abrirFotoNativa() {
        var el = document.getElementById('ac_camara_foto');
        if (el) {
            el.click();
        }
    }
    ...
    function mostrarFotoFallback(motivo) {
        window.acCamaraPickeoActiva = true;
        $('#ac_camara_overlay').addClass('tm-camara-visible');
        $('#ac_camara_mira').addClass('d-none');
        $('#ac_camara_foto_wrap').removeClass('d-none');
    ...
    function iniciarHtml5QrcodeVivo() {
        ...
        scanner = new Html5Qrcode('ac_camara_reader', {
    ...
    function iniciarCamaraViva() {
        ...
        detenerScanner().always(function () {
            ...
            $('#ac_camara_overlay').addClass('tm-camara-visible');
            $('#ac_camara_foto_wrap').removeClass('d-none');
            $('#ac_camara_mira').removeClass('d-none');
            $('#ac_camara_reader').find('video, canvas').remove();
            mostrarPreview('');
            limpiarCodigosLeidos();
            ultimoCodigo = '';
    ...
    function detectarHtml5Qrcode(file) {
        ...
        var lector = new Html5Qrcode('ac_camara_reader_foto', {
            formatsToSupport: formatosSoportados(),
            verbose: false,
        });
    ...
    window.acCerrarCamaraPickeo = cerrarCamara;
    window.acAbrirCamaraPickeo = iniciarCamaraViva;

    $(function () {
        $('#ac_btn_camara').on('click', iniciarCamaraViva);
        $('#ac_camara_cerrar').on('click', cerrarCamara);
        $('#ac_btn_otra_foto').on('click', function () {
            abrirFotoNativa();
        });
        $(document).on('change', '#ac_camara_foto', function () {
            var input = this;
