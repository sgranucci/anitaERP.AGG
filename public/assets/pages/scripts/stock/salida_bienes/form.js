/**
 * Formulario Salida de bienes: destinatario condicional, ítems con/sin artículo.
 */
(function () {
    'use strict';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const selDest = document.getElementById('sb_destinatario_tipo');
        const selTipo = document.getElementById('sb_tipo');
        const chkEspera = document.getElementById('sb_espera_devolucion');
        const inpFechaDev = document.getElementById('sb_fecha_devolucion');

        function togglePanelesDestino() {
            const tipo = selDest ? selDest.value : 'DEPOSITO';
            const panelDep = document.getElementById('sb_panel_deposito');
            const panelUsu = document.getElementById('sb_panel_usuario');
            const panelExt = document.getElementById('sb_panel_externo');
            if (panelDep) panelDep.style.display = tipo === 'DEPOSITO' ? '' : 'none';
            if (panelUsu) panelUsu.style.display = tipo === 'USUARIO' ? '' : 'none';
            if (panelExt) panelExt.style.display = tipo === 'EXTERNO' ? '' : 'none';
            actualizarBadges();
        }

        function syncEsperaDesdeTipo() {
            if (!selTipo || !chkEspera) return;
            if (selTipo.value === 'ENTREGA') {
                chkEspera.checked = false;
            } else if (!chkEspera.dataset.userTouched) {
                chkEspera.checked = true;
            }
            syncFechaDevolucionRequired();
        }

        function syncFechaDevolucionRequired() {
            if (!inpFechaDev || !chkEspera) return;
            inpFechaDev.required = !!chkEspera.checked;
        }
        ...
        function textoDeposito(inputId) {
            ...
            if (cod && cod.value.trim() !== '') return cod.value.trim();
            return '—';
        }
        ...
        function actualizarBadges() {
            ...
            if (badgeDestino) {
                const tipo = selDest ? selDest.value : 'DEPOSITO';
                if (tipo === 'DEPOSITO') {
                    badgeDestino.textContent = textoDeposito('prestamo_deposito_destino_id');
                } else if (tipo === 'USUARIO') {
                    const nom = document.getElementById('sb_destinatario_usuario_nombre');
                    badgeDestino.textContent = (nom && nom.value) ? nom.value : 'Usuario';
                } else {
                    const nom = document.querySelector('[name="externo_nombre"]');
                    badgeDestino.textContent = (nom && nom.value) ? nom.value : 'Externo';
                }
            }
        }
        ...
        async function refrescarSaldosFila(tr) {
            ...
            try {
                ...
                if (empresaId > 0) url.searchParams.append('empresa_id', String(empresaId));
                const res = await fetch(url.toString(), { credentials: 'same-origin', headers: { Accept: 'application/json' } });
                if (!res.ok) throw new Error('http ' + res.status);
                const data = await res.json();
                if (data && data.saldos) {
                    if (depOId > 0 && data.saldos[depOId] !== undefined) cacheSaldos['o:' + articuloId] = Number(data.saldos[depOId]);
                    if (depDId > 0 && data.saldos[depDId] !== undefined) cacheSaldos['d:' + articuloId] = Number(data.saldos[depDId]);
                }
            } catch (e) { /* silent */ }
            pintarSaldos(tr, articuloId);
        }
        ...
        function aplicarArticuloEnFila(tr, art) {
            ...
            if (desc) desc.value = art.descripcion || '';
        ...
        function onCambioFila(ev) {
            ...
            if (ev.target.classList.contains('codigoarticulo') && ev.target.value.trim() === '') {
        ...
        if (selDest) selDest.addEventListener('change', togglePanelesDestino);
        if (selTipo) selTipo.addEventListener('change', syncEsperaDesdeTipo);
        if (chkEspera) {
            chkEspera.addEventListener('change', function () {
                chkEspera.dataset.userTouched = '1';
                syncFechaDevolucionRequired();
            });
        }
        ...
        if (typeof activa_eventos_consultaarticulo === 'function') activa_eventos_consultaarticulo();
        if (typeof activa_eventos_consultadeposito === 'function') activa_eventos_consultadeposito();
        if (typeof activa_eventos_consultausuario === 'function') activa_eventos_consultausuario();
    });
})();
