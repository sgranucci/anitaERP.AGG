(function ($) {
    ...
    function actualizarVisibilidad() {
        ...
        var esFecha = tipo === 'fecha' || tipo === 'fecha_premio';
        ...
        if (tipo === 'telefono') {
            setPlaceholderValor('Solo dígitos (ej. 1144556677)');
        } else if (esFecha) {
            setPlaceholderValor('dd/mm/aaaa');
        } else if (tipo === 'entero') {
            setPlaceholderValor('Número entero');
        } else {
            setPlaceholderValor('Texto, número o fecha');
        }
    }
    ...
    $(function () {
        if (!$('#form-filtros-cliente-uif').length) {
            return;
        }
        ...
        $('#form-filtros-cliente-uif').on('submit.listadoFiltrosSync', function () {
            var activo = document.activeElement;
            if ($(activo).closest('#panel-filtros-cliente-uif').length) {
        ...
        LF.initSubmitBusquedaRapida($('#form-filtros-cliente-uif'), {
            selectorPanel: '#panel-filtros-cliente-uif'
        });
